...
操作系统、网络、存储、并发和 GPU 等基础章节，不能只罗列组件名。至少选择一个真实操作（例如 `write`、一次缺页、一次 TCP 发送或一次 GPU Kernel），从应用代码一直追到硬件完成，并回答：

- **讨论边界**：操作系统、文件类型、I/O 模式、硬件和故障模型是什么；哪些变体不走同一路径。
- **逐层输入输出**：这一层收到什么、修改什么状态、把所有权或完成责任交给谁。
- **软硬件分工**：语言运行库、内核、驱动、CPU、DMA、控制器和设备分别做了什么。
- **完成语义**：提交、接受、可见、完成、持久和业务确认分别发生在哪里。
- **慢路径与错误**：缺页、锁、排队、节流、资源耗尽、部分完成和延迟错误怎样出现。
- **可观察证据**：系统调用、内核事件、队列、设备和端到端指标怎样对齐，单个工具看不到什么。
- **验证与面试**：包含教学算例、故障实验、30 秒/90 秒答法、追问、易错句和带答案练习。

具体内核函数名和硬件内部步骤必须标明版本与适用范围。优先引用官方手册、内核文档、规范和源码，不把某个文件系统或设备实现写成普遍保证。
