...
1.  **取指 (Fetch)**: 从内存（或 L1 指令缓存）中读取指令。
2.  **译码 (Decode)**: 将指令翻译成 CPU 内部的微操作 (uOps)。
3.  **执行 (Execute)**: ALU 进行计算（加减乘除）。
4.  **访存 (Memory)**: 读写内存数据。
5.  **写回 (Writeback)**: 将结果写回寄存器。

### 1.1 流水线的威力
通过流水线，CPU 可以让不同指令处于不同的阶段。虽然单条指令的延迟（Latency）没有变（仍然需要 5 个时钟周期），但吞吐量（Throughput）变成了每个周期完成 1 条指令。
