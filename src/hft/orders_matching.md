...
- 区分限价、市价、IOC、FOK、Post-only、冰山等常见订单；
- 判断一个限价订单是否 marketable（可立即成交）；
- 手算价格—时间优先和简化 Pro-rata 分配；
- 解释部分成交、撤单竞态和改单丢失优先级；
- 在面试中先讲正确性，再谈低延迟实现。
...
业务系统还可能需要账户、交易场所、订单容量、客户标识、显示数量、自成交防护组等字段。面试时不必背所有协议字段，但必须明确：价格、数量、方向、标的和唯一订单标识是不同维度。
...
所以面试中可以手算“理想 Pro-rata”，但应补一句：最终分配必须复现具体场所算法，尤其是舍入和优先权部分。
...
“系统做得到”不等于“业务允许做”。面试中主动说明这一点通常比展示投机技巧更专业。
