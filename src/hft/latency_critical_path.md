...
- 区分 latency、throughput、jitter、p50、p99 和 worst case；
- 定义 tick-to-trade、wire-to-wire 等测量边界；
- 建立一张延迟预算表，并避免把百分位数直接相加；
- 识别排队、缓存未命中、分配、锁竞争、系统调用等延迟来源；
- 给出包含测量、优化、回归和降级的完整面试答案。
...
`L = 800,000 × 25 × 10^-6 = 20`，系统中平均约有 20 个事件。该结果只针对稳定状态的平均值，不能据此推断 p99 队列长度。
