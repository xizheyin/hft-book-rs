...
这个模型的最大好处是把缓存一致性开销压到最低。每个核心持续访问自己的一段状态，缓存命中率更高、伪共享风险更低，也更容易获得稳定的吞吐量。
...
- Hash 分区：`shard = hash(key) % N`，实现简单，平均意义上较均匀
...
这也是为什么在第三部分我们先讨论原子操作与无锁结构，再讨论 Ring Buffer 与队列：它们并不是孤立的数据结构题，而是多核扩展的基础语言。

## 5. NUMA：非一致内存访问带来的扩展边界

当机器进入多 socket 或多 NUMA 节点配置后，内存访问不再等价。访问本地 NUMA 节点内存与访问远端节点内存之间可能相差数倍延迟，且带宽上限也不同。扩展性问题会在此时变得更“物理”：
