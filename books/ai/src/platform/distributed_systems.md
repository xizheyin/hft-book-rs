...
1. 建身份表：task 表示用户意图，attempt 表示一次执行，operation 表示一次外部副作用，lease generation 表示当前所有权。重试 attempt 可以变化，operation ID 是否变化由业务意图决定。
2. 给工具分类为纯读、天然幂等、可用幂等键、不可安全自动重试。每类写 timeout 后的状态，不能统一用“失败重试”。
3. 画 `INTENT_RECORDED → SENT → SUCCEEDED/FAILED/UNKNOWN`。在远端执行、本地写结果、发送消息和 ACK 之间逐点崩溃；UNKNOWN 只能查询、对账或人工裁决。
4. 让 worker A 持 lease 暂停，B 获得更高 generation，再让 A 回写。数据库与自有资源接口应检查 fencing token；若第三方 API 不支持 token，就必须由唯一写网关串行化、用稳定 operation ID 去重或把该动作判为不可自动接管。只有控制面更新 owner 字段不能阻止迟到副作用。
5. 恢复题按数据库状态、outbox/inbox、对象存储 checkpoint、外部系统事实依次对账。RPO/RTO 结论必须绑定各存储的实际持久化与恢复位置。

验算点：同一业务意图不会因 attempt 重试产生第二次效果；旧 generation 不能提交；消息重复不改变本地最终状态；UNKNOWN 数量可监控且最终被裁决；恢复后各存储能按稳定 ID 对齐。
...
1. 在更新业务状态的同一数据库事务中写 Outbox 事件。事务提交后发布器持续重发未确认事件，消费者按 event ID 去重；这样提交后即使立即崩溃，事件仍在数据库中，不会永久丢失。它把“可能丢消息”变成“可能重复消息”，后者再由幂等消费处理。
2. 幂等键只有在第三方系统保存并校验它，而且键的保留期覆盖全部重试时才有效。若付款方不支持该键、请求已到达但查询接口不可用，或调用方错误地产生新键，本地记录不能阻止第二次扣款；此时只能用远端收据、对账、补偿和人工控制收口。
3. 状态转移使用版本和预期旧状态做条件更新，并提前规定优先规则。若完成结果已经由当前 epoch 持久化，迟到取消不能把它覆盖成 `CANCELLED`；若取消先成功进入 `CANCELLING`，执行器不得再开始新步骤，但已发生副作用仍需对账。并发者中只有一个条件更新成功，另一个重新读取最终状态。
4. 新 worker 获得更大的 fencing epoch，挂载服务/卷写代理在每次写或会话建立时验证 epoch，只接受当前值；旧 worker 的 lease 过期并不证明它停止，所以仅在控制面标记新 owner 不够。必要时先撤销旧会话和设备映射，再开放新写者。
5. 恢复点带全局时间/版本清单，列出当时数据库记录引用的对象、Checkpoint 和 active Attempt。恢复后先冻结新调度，比较数据库引用与对象存储实际对象，验证哈希并隔离孤儿；再查询运行中沙箱的 run/epoch，fence 掉数据库不再承认的实例。外部 operation 按稳定 ID 查询收据，完成对账后才恢复写流量。
