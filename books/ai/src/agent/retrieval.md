...
权限不是“相关性分数”，而是硬约束：无权读取的文档不应因为语义分高而进入候选。权限过滤通常应尽早做，防止未授权文档进入缓存、日志或模型上下文；授权后的候选再按相关性、权威性和新鲜度重排。
...
- `Recall@100`（前 100 召回率）：标准证据中有多少进入前 100；若每题只有一份标准证据，也可解释为多少题成功把它召回；
- `nDCG@10`（Normalized Discounted Cumulative Gain，归一化折损累计增益）：在前 10 中，相关等级更高的文档是否排得更靠前；
- `MRR`（Mean Reciprocal Rank，平均倒数排名）：每题第一个正确结果排名倒数的平均，例如第一名贡献 `1`、第二名贡献 `1/2`；
- 引用支持率：答案主张是否真被引用内容支持。

若 `Recall@100` 已经失败，调生成 Prompt 通常救不回来；应先修 query、索引或召回。

<details>
<summary><strong>拓展：向量索引的实现与生命周期</strong></summary>
...
HNSW（Hierarchical Navigable Small World，分层可导航小世界图）可以想成城市道路：上层像高速公路，节点少但跨得远；下层像街道，节点多且连接局部邻居。查询先在上层快速接近目标区域，再到下层细找。

- 图连得更密、搜索时探索候选更多，通常 recall 更高；
- 代价通常是更多内存、构建时间和查询计算；
- 新向量可插入图中，但大量删除和长期更新可能使图质量下降，需要 tombstone（逻辑删除标记）、压缩或重建，具体取决于实现。

具体引擎的参数名应在实际使用时查对应版本。扩大搜索范围一般会用延迟换 recall，HNSW 的图边本身也占内存。
...
1. 先建带相关性标注的数据集，查询必须覆盖实体、时间、权限和多跳类型；同时保存语料快照版本，避免索引变化后指标无法复现。
2. 召回阶段计算 Recall@k：所需证据是否进入候选集；排序阶段再看 MRR/NDCG 或首个相关结果位置。候选都没有时，不能把失败归给生成模型。
3. 逐项核对 query rewrite、过滤条件、稀疏/稠密分数和 rerank 分数。权限过滤必须在返回上下文前强制执行，不能靠 prompt 要求模型忽略越权文档。
4. 上下文打包记录文档 ID、chunk ID、版本、token 数和截断位置；检查关键证据是否因预算、去重或排序被移出窗口。
5. 生成后分别评估答案正确性、引用是否真的支持断言、证据覆盖率和无证据断言比例。把引用存在误当引用正确，会掩盖 attribution failure。

验算点：离线指标绑定同一语料版本；Recall 与排序指标分开；每个答案断言可追到 chunk；过期或越权文档不会进入上下文；失败能定位到 rewrite、召回、排序、打包或生成中的具体一层。
...
<details>
<summary><strong>拓展：千万级向量怎样选择 ANN 索引</strong></summary>
...
- **BM25 与向量：**BM25 擅长错误码、函数名、专有名词和精确词匹配；向量检索擅长同义改写和语义相近但词面不同的查询。混合检索先分别召回，再用验证集调融合与重排，不能先假定一方总是更好。
- **分数不能直接相加：**两种检索器的分数范围和含义不同。可在标注查询上做归一化/校准，或使用 Reciprocal Rank Fusion 等基于名次的方法；随后用 Recall@k、nDCG 和端到端答案指标验证。
- **top-k 不是越大越好：**增大 k 可能提高召回，却会加入噪声、占用上下文、增加重排和生成时间，相关证据还可能被淹没。应画 k 与召回、精度、延迟和 Token 成本的曲线，选满足目标的最小值。
- **500 ms 怎样分预算：**先为端到端留网络与降级余量，例如解析/改写 50 ms、稀疏和稠密并行召回 120 ms、融合与 ACL 30 ms、重排 180 ms、打包 40 ms，已用 420 ms，余下 80 ms 供抖动。数字只是预算方案；每段都要有 timeout，串行段总和不得超过总 deadline。
- **精确与 ANN：**精确检索适合较小集合、离线基准或需要精确 top-k 的场景，成本随向量数线性增长；ANN 用可控召回损失换大规模在线延迟。上线 ANN 前，用代表性 query 的精确结果计算索引 recall。
- **HNSW、IVF、PQ：**HNSW 用图边和候选搜索换高召回与速度，代价是内存；IVF 只搜索部分聚类桶，用漏检风险换少算距离；PQ 用短编码近似原向量，显著省内存与带宽但增加量化误差。选择要看同一数据上的 recall—延迟—内存曲线。
- **不能混新旧 embedding：**两个模型产生的坐标系和距离分布通常不可直接比较。应为新模型重建独立索引、双写或影子评测，验证后原子切换；查询向量和文档向量必须来自匹配版本。
- **权限撤销：**权威 ACL 立即在召回前和返回前拒绝访问，不等待索引重建；随后用版本化删除传播到索引、副本和缓存，并通过审计查询验证旧身份已无法命中。缓存键必须包含权限上下文。
- **多轮搜索边界：**限制最大轮数、总 Token、费用、并发和域名；每轮要求新证据或查询变化，重复状态触发停止。网页内容标记为不可信数据，不能覆盖系统指令或直接触发高权限工具；下载大小、重定向、内网地址和脚本执行均受出口策略限制。
...
- **拓展**：精确检索是 ANN 的对照基准；索引 recall 和业务 Recall@k 是两层指标。
- **拓展**：HNSW 用多层图导航，IVF 先选桶，PQ 压缩向量；选择取决于 recall—延迟—内存曲线。
- **拓展**：索引版本要绑定语料、切分、embedding、距离、参数和 ACL，并支持影子构建与回滚；增量、删除、权限撤销和缓存失效都属于索引正确性。
