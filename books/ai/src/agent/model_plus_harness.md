...
## 10. 做题方法：重放一条 Agent Loop 轨迹

1. 固定运行版本包：模型、system prompt、tool schema、策略、上下文构建器和停止条件；只记录“用了某模型”不足以复现。
2. 为每轮保存 `step_id`、不可变输入上下文的对象引用与 hash、模型响应、解析出的动作、权限决定、工具请求/结果、token 与耗时。hash 只能验真，不能还原内容；对象引用必须能取回当时的完整上下文。工具结果还要与发起它的 call ID 对齐。
3. 维护预算账：剩余 token、wall-clock deadline、模型调用数、工具调用数和累计成本。每轮开始前检查预算，停止原因必须是可枚举状态，而不是循环悄悄消失。
4. 诊断失败时先用原工具结果重放 harness：若解析、状态更新或停止仍错，问题在 harness；若固定 harness 只替换模型输出后改变，才把差异归到模型/提示层。
5. 对有副作用的工具标 `NOT_SENT / SENT_UNKNOWN / SUCCEEDED`，重放轨迹时不能再次执行 `SENT_UNKNOWN`，只能查询或用稳定幂等键恢复。

验算点：每个 observation 都有来源；每个动作都有权限证据；每个工具结果只消费一次；循环必受预算约束；最终答案能追到产生它的模型步骤和工具证据。
...
- **检测没有进展：**为每轮记录目标剩余项、artifact 版本、工具结果摘要和错误类型。如果连续多轮使用相同参数调用同一工具、状态签名不变，或完成指标没有增加，就判为停滞；随后先换策略或请求帮助，达到停滞预算再终止。最大轮数是最后保险，不是唯一信号。
- **流中断时能否执行：**只有收到完整、通过 Schema 与权限校验、并被持久化为待执行 step 的动作才可执行。半个 JSON 或缺少结束原因的流一律丢弃；若完整动作已持久化但确认丢失，则按稳定 tool-call ID 查询，不能重新生成 ID 再执行。
- **KV Cache 与业务记忆：**KV Cache 是某次模型推理为历史 Token 保存的中间张量，通常随请求结束而释放；业务记忆是带来源、权限、作用域和失效规则的持久信息。前者用于省计算，后者用于未来检索，不能用缓存命中代替事实管理。
- **谁判断完成：**模型可以提出“已完成”，但可程序验证的条件应由外部 verifier 检查，例如测试是否通过、文件是否存在、状态是否提交。主观任务也应使用独立规则或评审；不能让生成答案的同一过程无证据地给自己判满分。
- **避免工具执行两次：**把“模型调用的重试”与“工具 step 的身份”分开。Harness 为逻辑工具动作生成稳定 ID，先持久化意图，再由执行器按该 ID 去重；模型流重试只重建候选输出，不能绕过已存在的 step 再创建一次副作用。
- **判断 fallback 兼容：**比较上下文长度、工具/Schema 支持、停止原因、结构化输出、Tokenizer、精度与安全策略等能力契约；再用同请求做离线配对和 canary，检查任务成功、参数正确率、延迟与成本。HTTP 200 只证明接口可达。
- **取消后的确认：**向模型服务和工具链传播同一取消/deadline，等待各层确认或到达清理期限；检查推理请求、GPU 队列、流连接和计费记录是否结束。已经发出的副作用若无收据应进入 `UNKNOWN` 并查询，不能因为客户端断开就宣称全部停止。
