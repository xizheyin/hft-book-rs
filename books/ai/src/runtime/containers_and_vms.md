...
## 11. 做题方法：逐层追地址、CPU 与设备

1. 先写隔离目标和攻击者能力，再选择容器、用户态内核或 microVM；不能从产品名称直接推出安全结论。
2. 地址题依次画 `guest virtual → guest physical → host physical`。容器没有独立 guest physical 层；microVM 的 guest 页表与二阶段页表分别由 guest OS 和虚拟化硬件/VMM 管理。
3. CPU 题标出 guest 指令何时直接运行、何时因特权操作或设备访问 VM exit；再检查 vCPU 线程是否被宿主调度、超卖和迁核影响。
4. 设备题逐 I/O 路径比较模拟设备、virtio 与直通：请求在哪层解析、数据复制几次、DMA 能访问哪些页、IOMMU/设备身份在哪执行隔离。
5. 快照题同时冻结 vCPU、内存、虚拟设备队列、磁盘状态和网络连接语义；只有内存页一致，仍可能恢复出重复 I/O 或损坏应用状态。

验算点：每个地址属于明确层次；每个设备请求有策略执行点；DMA 不越过分配页；快照中 CPU、内存、设备和持久状态属于同一逻辑时刻；资源超卖结论同时检查 CPU、NUMA、内存和 I/O。
...
1. **不一定。** 在硬件虚拟化允许的 guest 模式中，普通算术指令通常直接由物理 CPU 执行；只有配置为敏感的操作、I/O、某些异常或外部事件才触发 VM exit。若每条加法都退出，虚拟机就无法高效运行。
2. **不是同一概念。** VM exit 是 CPU 从 guest 切到虚拟化处理环境的一次硬件事件；KVM 可能在内核中处理后直接重新进入 guest，用户态看不到。只有需要 QEMU/运行时处理、收到信号或满足其他返回条件时，`KVM_RUN` 才从内核返回用户态。
3. GVA 是 Guest Virtual Address，由 guest 页表映射为 GPA（Guest Physical Address）；GPA 再由 hypervisor 配置的 EPT/NPT 映射为 HPA（Host Physical Address）。guest OS 管第一段，宿主虚拟化层管第二段，CPU 的地址翻译硬件组合执行。
4. guest page fault 表示第一段 `GVA→GPA` 的 guest 页表权限或映射不满足，由 guest OS 按其内存规则处理；EPT/NPT violation 表示第二段 `GPA→HPA` 不满足，由 hypervisor/KVM 处理。它们发生在不同地址层，不能仅凭都叫“缺页”混为一谈。
5. 描述符告诉设备/后端缓冲区位置、长度和方向；kick 通知后端“有新工作”；used ring 记录哪些描述符已经处理；虚拟中断通知 guest 驱动查看完成。实现可以合并通知，但所有权从可用到已用的转换仍要清楚。
6. virtio 只定义高效共享队列协议。后端可能仍复制数据、转换格式、经过用户态或网络，guest 内存也可能因安全与 DMA 映射产生额外处理；是否零拷贝要逐段画缓冲区和所有权，不能由协议名推断。
7. 两条典型路径如下，具体实现可省略或增加后端层：

   ```text
   块写：guest 进程 → guest 文件系统/块层 → virtio-blk 队列
        → KVM/QEMU 或宿主后端 → 宿主文件/块设备 → 物理存储

   网络：guest 进程 → guest TCP/IP → virtio-net 队列
        → tap/用户态后端 → 宿主网络栈或虚拟交换 → NIC
   ```

   每条路径都要反向画出完成通知，写提交到队列不等于持久化或已经发到对端。
8. **不能。** dirty bitmap 全零只表示观察窗口内没有记录到 guest 内存页写入；它不说明先前内存与磁盘来自同一一致时刻，也不包含设备队列、页缓存或外部存储的未决写。创建一致快照仍要协调 vCPU、内存、设备和磁盘状态。
9. 若 guest 脏页速度 `D` 不小于迁移网络复制速度 `B`，一轮复制期间产生的新脏页不会减少，pre-copy 难以收敛。即便 `D<B`，工作集反复写热点页也会拖长多轮复制；此时需限速、短暂停机、post-copy 或业务级迁移策略。
10. 128 个持续繁忙 vCPU 争用 64 个 pCPU，平均每个 pCPU 对应 2 个 vCPU。总吞吐可能仍接近 64 个核的上限，但请求是否立即获得运行时段取决于调度；队列、锁持有者被换出和中断叠加会先扩大尾部等待，所以 p99 往往早于平均吞吐恶化。
11. 检查 vCPU 与其内存是否位于同一 NUMA 节点、virtio 后端线程和 vhost/设备队列运行在哪个节点、NIC/存储 PCIe 设备连接在哪个 NUMA 节点，以及中断和 DMA 缓冲位置。只固定 vCPU，却让内存和 I/O 都跨节点，仍会产生远端访问。
12. 建三层证据链：guest 内看文件系统/块队列与进程等待；虚拟化后端看 virtqueue 等待、QEMU/vhost 线程与宿主文件 I/O；宿主设备层看块设备队列、延迟和错误。用同一请求/时间戳关联，做一个只替换后端或迁移到另一设备的对照；哪一层等待先增长且随对照消失，才支持该层是瓶颈。
