...
## 9. 做题方法：先给数据分类，再推演挂载和恢复

1. 把根镜像、临时块设备、共享卷、checkpoint、制品和缓存分列，写 source of truth、生命周期、共享范围、一致性与丢失后果；可重建缓存不能偷偷承担唯一持久状态。
2. 画读写路径：沙箱文件系统、页缓存、虚拟块设备、宿主/远端存储、复制与持久化确认。题目说“写成功”时，必须指出已经走到哪一层。
3. 快照前记录文件系统/应用 quiesce 点、父快照 ID 和增量块范围；恢复后校验元数据与数据属于同一版本，不能只验证对象存在。
4. 让旧节点持卷暂停，新节点拿到更高 mount generation 后接管，再让旧节点恢复。所有写请求带 fencing token，存储拒绝旧 token；仅依靠 lease 持有者自觉停止不够。
5. 容量题同时计算逻辑数据、复制、快照增量、写放大、临时空间和回收滞后；删除引用后对象进入可达性扫描/宽限期，避免误删仍被 checkpoint 使用的块。

验算点：每类数据有唯一权威来源；确认语义绑定具体持久层；同一卷只有一个有效写世代；恢复 checkpoint 可校验且引用完整；回收不会删除可达对象。
...
1. 对不可变制品、Checkpoint、大对象和只需按 key 获取的结果，对象存储通常更合适：容量大、复制简单，但不提供完整 POSIX 目录、随机写和锁语义。需要现有编译工具、共享目录、原子 rename 或细粒度随机读写时，共享 POSIX 卷更自然。先列语义，再选系统。
2. 快照克隆启动快、只为修改页付费，但长期写入会增加 COW 成本；全量复制隔离直观、运行性能稳定，却增加启动时间和字节流量；远端读最省本地容量，但把运行时延迟和可用性绑定到网络/后端。应用真实读写轨迹比较启动、稳态、失败和回收四阶段，常见方案是快照起步加后台预取。
3. 可按顶级目录、租户或一致哈希分片元数据，并缓存只读查找；热点目录还需拆子目录或对写入限流。跨分片 rename 不能假装是单机原子操作：要么限制它只在同分片执行，要么由协调事务/意图日志完成并定义崩溃恢复，或改用“写新对象—原子切元数据指针—异步删旧对象”的业务协议。
4. 删除流程先撤销挂载、身份和密钥，再删除元数据引用与底层块/对象；复用前使用新租户密钥或经过证明的清零机制。验证时写入唯一哨兵数据，删除并重新分配同一类资源给另一测试租户，扫描文件、未分配区、快照、缓存和备份可见性；审计还要证明旧凭证和旧 URL 已失效。
