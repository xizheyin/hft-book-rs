...
> 先修桥梁：先读[进程与系统调用](../systems/process_threads_syscalls.md)、[虚拟内存](../systems/virtual_memory.md)和[网络与 RPC](../systems/network_rpc.md)。威胁模型必须建立在真实内核、内存和网络边界上。
...
假设每个沙箱限制 2 CPU、4 GiB 内存、2 GiB 工作卷、256 个进程和 10 分钟墙钟时间。调度器的“请求值”可用于装箱，但安全层必须执行“上限值”；只在数据库里记录配额而不在节点上强制，等于没有配额。
