...
> 难度：必会。官方 JD 明确包含应用层调度、管控面服务、混合云、可观测与节点自动上下线。

> 先修桥梁：先读[并发与异步 I/O](../systems/concurrency_async_io.md)和[分布式系统地基](../systems/distributed_foundations.md)，理解条件更新、超时不确定性、复制、共识和 fencing，再看控制面怎样组合它们。

控制面像城市规划与调度中心，数据面像真实道路、车辆和建筑。规划中心告诉系统“这里应该有一个沙箱”，节点运行时才真正创建进程、网络和卷。把两者混在一个同步 RPC 里，任何慢启动都会拖垮入口。
...
1. 一个 `(run_id, generation)` 同时最多有一个有效执行租约。
