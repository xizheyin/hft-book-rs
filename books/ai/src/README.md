...
> 这本书面向第一次系统学习 AI 与计算机系统、正在准备 DeepSeek Agent Infra 岗位的读者。它从“程序怎样在机器上运行”出发，走到 AI、Agent Harness、DSec 类沙箱云平台、可靠性和个性化面试实战。

Agent Infra 横跨两个世界：上层是模型、上下文和工具，下层是 CPU、内存、Linux、网络、存储与分布式系统。对零基础读者来说，最容易踩的坑是直接背框架和架构图，却不知道系统调用为什么会阻塞、页为何会缺失、TCP 为什么重传、模型为何吃显存，以及这些机制怎样共同制造长尾。本篇先补齐这条因果链，再进入 Transformer、训练、推理、RAG、Agent、安全评测和系统设计。
...
```mermaid
flowchart TD
    A["官方 JD 与个人证据<br/>先确定面试范围"] --> B["核心系统底座，约 60%<br/>OS、VM、网络、存储、调度、HA（高可用）"]
...
| 计算机系统地基 | CPU/内存/内核/设备、进程线程、虚拟内存、并发、网络、文件系统、数据库、分布式与性能诊断 | 能把一个慢请求从代码追到内核、网络或设备，并给出可验证证据 |
...
2. 学八章计算机系统地基，建立 CPU、内核、网络、存储、分布式和性能的共同语言。
3. 学 AI/LLM 与 Agent，理解沙箱真正承载的上层 workload：先补 AI/ML、数学、数据和神经网络积木，再按 **Transformer → 生成 → GPU → 推理 → 训练 → DeepSeek 综合架构** 学 LLM，最后进入 Agent Harness。
4. 再进入 Runtime 与 Platform，把上下两层组合成安全、可恢复的平台。
5. 最后按目录完成简历事实卡、系统设计、120 题和学习计划；真正训练时再按学习计划先验收红线题、后做白板模拟。
...
1. 先闭卷做系统地基自测，只补答不出的 P0 内容，不从第一页平均重读。
2. 先攻系统地基中的文件写入，再攻 Runtime 与 Platform，重点覆盖隔离、VM、存储、网络、租约/fencing（隔离旧主）、HA 和事故处理。
...
2. **分级而非全背**：P0 是本次岗位的最高优先内容，包括系统与 Runtime 主线，以及分布式核心、观测、容量和安全边界；其中红线知识要求 100% 正确，其余 P0 按学习计划稳定达标。P1 用于机制深挖，P2 用于高级实现和更宽的迁移能力。
...
- 一段程序怎样从用户态进入内核，数据又怎样经过缓存、网络或设备？
