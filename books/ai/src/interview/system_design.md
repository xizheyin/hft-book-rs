...
本章故意把一场 45～60 分钟白板面试能继续追问的分支都展开，所以很长。第一次练习按下面顺序，不要从 JSON 字段开始背：

| 优先级 | 先练什么 | 面试通过标准 |
|---|---|---|
| **P0：白板主线** | 需求与容量 → 控制面/数据面 → 状态机与租约 → 隔离安全 → 故障恢复 → 取舍 | 30 秒给总图，10 分钟讲完一条任务主路径，并主动说出故障和安全边界 |
| **P1：连续追问** | API 语义、幂等与外部副作用、存储职责、观测指标、灰度和容量模型 | 面试官点到某层时，能解释“为什么需要、失败会怎样、怎样验证” |
| **P2：按简历选读** | 具体 JSON/SQL 字段、跨区域灾备细节、Property Test（属性测试）与故障注入矩阵 | 只在简历真的做过或面试官要求落到实现时展开，不背教学数字 |

练习时每画一个组件，都问自己三个问题：它解决什么失败？它保存哪份状态？它挂了由谁恢复？答不出来时，继续堆组件只会失分。
