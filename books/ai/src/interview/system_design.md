...
所以 Coding Agent 平台不是一个聊天页面，而是一个**不可信任务执行平台**。模型给出意图；策略系统决定是否允许；沙箱执行动作；状态机记录进度；验证器检查结果；人类在高风险边界作最终决定。

通用的进程、内存、网络和文件机制见共享册的[程序执行总览](../../rust-hft/foundations/computer_execution.html)，部分失败、复制与共识见[分布式系统基础](../../rust-hft/distributed/index.html)。这里组合 Agent 特有的[沙箱命令执行](../systems/process_threads_syscalls.md)、[工具 RPC](../systems/network_rpc.md)、[可恢复状态](../systems/filesystem_database.md)和[Agent 分布式控制面](../platform/distributed_systems.md)。

先认四个角色：**控制面**保存期望状态并做授权、调度，**数据面**真正运行模型、工具和沙箱；**Tool Proxy** 是执行工具前的确定性权限检查点；**verifier** 是独立检查产物是否满足测试或策略的程序。`attempt` 表示一次执行尝试，`fencing epoch` 是接管时递增、用于拒绝旧 worker 的代数，`logical_effect_id` 是跨重试保持稳定的副作用身份。
...
SLO（Service Level Objective，服务目标）是团队承诺努力达到的可测指标。以下数字用于展示推导方法：

- 控制面 API 月可用性目标 99.9%。
- 99% 的已接受任务在 2 秒内进入持久队列。
- 发出取消后，99% 的沙箱在 5 秒内停止新动作并开始回收。
- 已被 API 确认接收的任务状态事件，在单节点故障假设内目标 RPO 为 0；可用区或区域灾难的 RPO/RTO 取决于复制和确认协议，必须另行声明并演练。审计事件按租户至少保存 30 天。
...
只算 2,000 个任务，就像只知道餐厅有 2,000 位客人，却不知道有人只喝水、有人要占一张大桌。可以先构造下面的**示例分类**，再用 trace 与压测替换成生产值：

| workload class | 并发占比假设 | 每个运行沙箱的示例 request | 还要单独测什么 |
|---|---:|---|---|
...
<details>
<summary><strong>深入：完整 API、字段与执行信封示例</strong></summary>
...
同一 attempt 只允许一个 worker 推进状态。Scheduler 每次分配都在数据库事务中递增单调 `fencing_epoch`，并发放带过期时间的租约；worker 定期续租。状态提交同时检查版本、epoch、取消代数和数据库看到的租约有效期。下面的 SQL 展示这些条件怎样组合：

<details>
<summary><strong>深入：条件更新 SQL</strong></summary>
...
- 系统设计顺序：**需求 → 估算 → API → 架构 → 状态机 → 故障 → 安全 → 观测 → 取舍。**
- 相关追问见[AI 与 Agent Infra 高频问题库](question_bank.md)。
