...
遇到 AI/LLM/Agent 概念题或故障题，可以按下面的顺序拆解：

1. **圈出被改变的对象**：是数据、模型参数、当前上下文，还是数据库和工具等外部状态？参数发生持久变化才属于训练；只把示例放进当前 Prompt 通常属于上下文学习。
2. **判断所处阶段**：训练题先找 Loss、梯度和参数更新；推理题先找输入、前向计算和输出；Agent 题还必须继续追踪工具、权限、状态和副作用。
3. **画出最短数据流**：`输入 → 模型 → 结构化动作 → 校验/授权 → 工具 → 外部状态 → 返回结果`。题目中的失败通常能落到其中一条边界。
4. **把“能力”和“保证”分开**：模型能够生成某个答案，不代表答案正确；模型能够提出删除命令，不代表系统允许执行；工具返回成功，也不自动代表业务目标完成。
5. **反向验算**：回答中若把 AI、ML、深度学习、LLM 和 Agent 当成同义词，或用模型输出代替真实副作用证据，就说明分层还没有完成。
