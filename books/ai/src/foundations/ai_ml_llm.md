...
> 本章目标：用一条从数据到线上请求的链路，分清 AI、机器学习、深度学习、LLM 和 Agent，并能解释训练与推理为什么是两种不同的系统负载。
...
| 主要目标 | 学到可泛化的参数 | 在 SLO 内给出结果 |
| 常见资源压力 | 算力、显存、跨卡梯度通信、检查点 | KV Cache、批处理、排队和尾延迟 |
...
| 上下文窗口 | Prompt、历史消息、RAG 文档 | 当前任务的临时信息 | 有长度上限，可能被注入恶意指令 |
