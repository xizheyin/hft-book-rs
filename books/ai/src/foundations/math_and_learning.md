...
训练过程会反复使用向量、概率、Loss、梯度、反向传播和优化器。下面从具体数字开始解释这些对象怎样共同更新模型参数。
...
模型通常先输出 Logit，也就是还没有归一化的分数。Softmax 把一组 Logit 变成总和为 1 的概率；公式中的 `exp` 是指数函数，`Σ` 表示求和：
...
分类和语言模型常用交叉熵。若正确答案的预测概率为 `p`，单样本损失如下，其中 `ln` 是自然对数：
...
其中 `θ` 表示所有参数，`η` 是学习率。SGD（随机梯度下降）直接沿当前梯度更新；Adam 一类优化器还会维护梯度的一阶、二阶统计，让不同参数获得自适应步长。它不是“永远比 SGD 好”的魔法；优化器还会与 Batch Size、学习率预热（Warmup）和后续衰减共同作用。

| 现象 | 可能原因 | 首先检查 |
|---|---|---|
| Loss 几乎不动 | 学习率太小、参数冻结、数据或标签错 | 梯度范数、可训练参数、单批过拟合 |
| Loss 剧烈振荡 | 学习率太大、Batch 太小、数值不稳 | 降学习率、检查异常样本和精度 |
| Loss 变成 NaN（非有效数值） | 溢出、除零、坏输入、梯度爆炸 | 第一个非有限值出现在哪个算子 |
...
## 9. 做题方法：先查形状，再算数值，最后解释训练含义

1. **矩阵题先写形状**：若 `X` 是 `[B,D]`、`W` 是 `[D,H]`，则 `XW` 是 `[B,H]`。相乘的内侧维度必须相等，输出保留外侧维度；不要一开始就逐元素计算。
2. **Softmax 题先平移 Logit**：所有 Logit 同减最大值不会改变结果，却能避免指数过大。算完检查每项非负、总和约等于 1；最大的 Logit 应对应最大的概率。
3. **交叉熵只取正确类别**：单样本多分类题先找到正确类别概率 `p_y`，再算 `-ln(p_y)`。`p_y` 越接近 1，Loss 越接近 0；若结果方向相反，通常是符号写错。
4. **反向传播沿计算图倒着走**：每经过一个节点就乘局部导数；多个分支汇合时梯度相加。先写符号表达式，再代数字，能避免漏乘一项。
5. **参数更新最后做方向检查**：最小化 Loss 时使用 `θ_new = θ_old - ηg`。若梯度为正，参数应减小；更新后可重新做一次前向计算，确认 Loss 是下降还是因为学习率过大而上升。
