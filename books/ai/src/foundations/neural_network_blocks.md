...
GELU 按输入大小平滑地缩放数值；SiLU 使用 `sigmoid`（把实数平滑压到 0～1 的函数）作为门，写成：
...
`⊙` 表示逐元素相乘。它比普通两层 FFN 多一条投影支路，所以参数量和 GPU Kernel（实际执行算子的程序）布局也不同。确切维度必须看模型配置。
...
## 10. 做题方法：沿着形状表和资源账逐层验算

1. **给每个维度起名字**：例如 `[B,T,D]` 分别表示批大小、序列长度和隐藏维度。Linear 通常只改变最后一维；不要只写三个没有含义的数字。
2. **参数量按矩阵加偏置**：`D_in → D_out` 的 Linear 有 `D_in×D_out` 个权重；若有偏置再加 `D_out`。FFN、门控 FFN 和 MoE 必须分别把实际存在的投影列出来。
3. **残差相加前检查形状完全一致**：主分支与直连分支的每个维度都要兼容。若 FFN 扩到 `D_ff`，必须投影回 `D` 才能与输入相加。
4. **参数、激活和缓存分三本账**：参数量回答模型有多大；激活取决于 Batch、序列和中间层；推理 KV Cache 还随历史长度与并发增长。题目问显存时不能只乘参数字节数。
5. **反向验算语义**：Attention 应改变 Token 间的信息，FFN 应逐 Token 工作。若推导让 FFN 混合了序列位置，通常是轴或 reshape 写错。
...
1. Linear 只改变最后一维，所以输出是 `[4,128,2048]`。权重形状为 `512×2048`，偏置有 2048 个数，因此参数量为 `512×2048+2048=1,048,576+2,048=1,050,624`。验算：前两维是 Batch 和 Token 数，不参与参数量计算；同一组权重会复用于全部 `4×128` 个位置。
2. 扩维给逐 Token 的非线性变换提供更大的中间特征空间，降维则把结果送回模型隐藏维度。残差执行 `x + FFN(x)`，相加的两个张量形状必须一致；若输入最后一维是 512，而 FFN 仍输出 2048，就不能逐元素相加，所以末端投影必须回到 512。
3. **Attention** 汇总其他 Token 的信息；**FFN** 对每个 Token 的特征独立做非线性变换；**Normalization** 控制激活的尺度，帮助数值和优化保持稳定；**残差**把输入直接送到后面，使网络可以在原信息上学习增量。
4. `1B×2 bytes=2 GB` 只算一份低精度权重。训练还可能需要更高精度主权重、梯度、优化器状态、前向激活、反向临时量和通信缓冲；推理还要有 KV Cache、工作区和并发请求的激活。因此应分项相加并测峰值，不能用模型文件大小代表进程显存。
5. NaN 会沿后续运算传播，最后出现的位置通常不是根因。应从输入开始逐层检查有限性、范围和数据类型，找到第一处由有限值变为 NaN/Inf 的算子，再检查溢出、除零或非法输入。全局裁剪只能隐藏症状，还可能把错误结果伪装成正常数字。
