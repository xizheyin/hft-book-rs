...
本章回答一个高频面试问题：**怎样安全启动、观察、取消并回收一次不可信的 Agent 执行？**

如果进程、文件描述符或系统调用还是陌生概念，先读第一册的[进程、线程与文件描述符](../../rust-hft/foundations/processes_fds.html)。本章不重复操作系统原理，只讨论 Agent 沙箱多出来的生命周期设计。
