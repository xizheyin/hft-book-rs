...
## 12. 做题方法：沿 RPC 阶段分配 deadline

1. 把调用拆成解析名字、取得连接、TLS、发送请求、服务端排队与执行、接收响应；给每阶段记录开始、结束、剩余 deadline 和 trace/span ID。
2. timeout 后只根据证据分类：连接前失败可较有把握地认为业务未开始；请求已写出但无响应通常是 UNKNOWN；收到带 operation ID 的成功结果才确认完成。
3. 重试前检查四项：错误是否暂时性、操作是否幂等、是否复用同一 operation ID、剩余 deadline 是否足够。每层最多尝试次数相乘，写出最坏下游放大倍数。
4. 对流式 RPC 分开记录“连接仍活着”和“业务仍推进”。没有事件不等于失败，必须由 heartbeat、进度序号或业务 checkpoint 判断；断线重连从已确认序号继续。
5. 出口策略题逐跳标出 DNS、代理、目标身份、TLS 校验和 egress policy 的执行点。工具给出的 URL 只是数据，不能自行扩大允许的网络范围。

验算点：所有子 timeout 都小于剩余总预算；UNKNOWN 不被误报失败；同一业务意图重试不换 ID；最坏重试倍数受预算约束；trace 能区分连接、排队、远端执行与响应丢失。
...
1. **本地调用只有一个进程内的控制流，RPC 跨越了可能独立失败的网络和机器。** 请求可能未发出、已发出未到达、远端已执行但响应丢失，调用方观察到的却都可能只是 timeout。因此 RPC 不能天然提供本地函数那样的单一返回点，更不能仅凭超时判断副作用是否发生。
2. **三者位于不同层。** TLS 验证通信对端并保护传输中的机密性和完整性；HTTP/gRPC 规定请求、响应、状态码、流和消息怎样表达；负载均衡在多个可用实例间选择目标并处理健康状态。TLS 不负责业务路由，gRPC 不自动解决授权或幂等，负载均衡也不能替代应用协议。
3. **因为响应缺失不等于执行缺失。** 时间线可能是“服务端完成写入 → 发送成功响应 → 网络丢包 → 客户端超时”。此时把超时当失败并生成新请求，会重复写入。应复用稳定 operation ID 查询结果；证据不足时状态是 `UNKNOWN`。
4. **依次检查四个条件：**错误是暂时性的；剩余 deadline 足够完成一次新尝试；操作本身幂等或通过同一 operation ID 去重；全局重试预算允许，且使用退避与抖动避免同时重试。任一条件不满足，就不应自动重试。
5. **“重试两次”通常表示首次尝试外再重试两次，即每层最多 3 次。** 若客户端层、服务 A 和服务 B 都独立最多尝试 3 次，最坏的最下游调用数是 `3 × 3 × 3 = 27`。若题目把“共尝试两次”定义为总共 2 次，结果才是 `2³=8`；做题时必须先写清术语口径。
6. **把 URL 当作不可信输入并逐跳校验。** 先只允许批准的协议、域名和端口；DNS 解析后拒绝回环、链路本地、私网、管理网和云元数据地址；连接必须通过受控 egress proxy。每次重定向和 DNS 重新解析都再次检查目标，防止先解析公网、后切换内网；同时限制响应大小、时间和重定向次数，并记录 run、tool call、目标身份与结果。
