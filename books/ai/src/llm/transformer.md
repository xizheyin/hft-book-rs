...
Transformer 依次组合 Token、Embedding、位置信息、Q/K/V、自注意力、因果遮罩、多头注意力和前馈网络。理解时既要跟踪张量形状，也要计算长上下文带来的时间与内存成本。
...
若一批输入写成 `X:[B,T,D]`，其中 `B` 是批大小、`T` 是 Token 数、`D` 是隐藏维度；单个注意力头的维度为 `d_k`，则可写成：

```text
W_Q, W_K: [D,d_k]    Q, K: [B,T,d_k]
W_V:      [D,d_v]    V:    [B,T,d_v]
QKᵀ:                 [B,T,T]
softmax(QKᵀ/√d_k)V: [B,T,d_v]
```

这里的转置只作用在最后两个维度，所以 `Q` 的每个位置会与 `K` 的每个位置得到一个分数。多头实现还会再增加头维度，后文给出完整 Batch 形状。
...
形状比背 API 更重要。很多实现错误都是转置、广播（自动扩展兼容维度）或 Mask 维度不对。
...
## 12. 做题方法：形状、分数、Mask、复杂度四张草稿纸

1. **先画形状表**：从 `[B,T,D]` 出发，确认 `D=H×d`；Q/K/V 拆头后是 `[B,H,T,d]`，`QKᵀ` 只交换 K 的最后两个维度，因此分数是 `[B,H,T,T]`。
2. **手算注意力按固定顺序**：点积 → 除以 `√d` → 加 Mask → 对每一行做 Softmax → 用该行权重加权 V。每行权重应非负且和为 1。
3. **Mask 题直接画方阵**：因果 Mask 的第 `t` 行只能保留 `0..t`；Padding 位置对所有有效 Query 都应不可见。不要只背“下三角”，因为左右 Padding 和布尔值含义会改变代码写法。
4. **复杂度题先找随 T 增长的对象**：标准全注意力的关系矩阵是 `T×T`；KV Cache 则按历史 Token 数近似线性增长。序列翻倍时，要分别回答算术、缓存和端到端延迟，而不是只给一个“翻倍”。
5. **最终做语义验算**：训练中的位置不能看到未来答案；逐步 Decode 的输出应与同配置的因果并行计算对齐。能运行不代表 Mask 和转置正确。
