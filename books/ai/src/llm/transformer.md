...
分词器还是模型接口的一部分：词表、特殊 Token 和 Chat Template 不匹配，即使权重正确也可能输出异常。
...
面试白板题里，形状比背 API 更重要。很多实现错误都是转置、广播或 Mask 维度不对。
...
团队训练代码补齐不同长度的序列时，把 Padding Mask 和 Causal Mask 合并错了一位。结果可能有两种：
...
维度增大时点积方差会变大，Softmax 容易饱和到接近 one-hot，梯度变差。缩放把数值保持在更合适的范围。
