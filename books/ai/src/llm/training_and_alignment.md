...
> 本章目标：理解预训练、监督微调、偏好对齐、强化学习和组相对策略优化（GRPO）的职责，能解释 DeepSeekMath 与 DeepSeek-R1 中 GRPO 的直觉，并识别奖励投机、能力遗忘与训练不稳定。

## 学习优先级

| 优先级 | 先掌握什么 | 面试要求 |
|---|---|---|
| P1 | 自监督预训练、SFT、强化学习（RL）、GRPO、奖励投机、训练状态版本化 | 能画训练链路并说清代理目标 |
| P2 | 参数高效微调、蒸馏、数据/张量/流水线/专家并行、模型状态分片 | 能解释它们分别省什么、付出什么 |
| 暂不展开 | 强化学习算法的完整推导、CUDA Kernel 实现 | Agent Infra 面试通常不要求现场推公式 |
...
<details>
<summary><strong>P2 选读：LoRA、蒸馏与分布式训练的最小模型</strong></summary>
...
### P2 追问 5：LoRA 为什么不代表训练显存只剩 0.39%？

0.39% 只描述例子中的可训练权重比例。基础模型仍要前向计算并驻留，训练还需要激活、临时缓冲和 LoRA 自身状态。

### P2 追问 6：Data Parallel 与 ZeRO/FSDP 的主要区别是什么？
...
- **P2**：LoRA 属于 PEFT，用低秩增量减少可训练参数；蒸馏把 Teacher 的部分行为迁移给 Student。
- **P2**：分布式训练可切数据、矩阵、层或专家；ZeRO/FSDP 则分片模型状态；Activation Checkpointing 用重算换显存，不等于故障恢复 Checkpoint。
