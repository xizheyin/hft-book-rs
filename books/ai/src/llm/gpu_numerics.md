...
## 13. 做题方法：所有性能题都带单位和上界

1. **先统一单位**：明确 GB 与 GiB、bit 与 byte、GB/s 与 Gb/s。量化位宽题先把每参数位数除以 8，再乘参数量；最后说明还没包含 Scale、Zero Point、对齐和运行时缓冲。
2. **带宽下界用“字节/带宽”**：若一次 Kernel 至少搬运 `M` byte、可持续带宽为 `BW` byte/s，则仅数据搬运时间下界是 `M/BW`。实际时间不应优于这个理想下界。
3. **Roofline 题先计算算术强度**：`FLOP/byte` 与机器的峰值算力/峰值带宽比较，判断更可能受算力还是带宽限制；结论仍要用真实 Kernel 计数器验证。
4. **显存题分项相加**：权重、激活、梯度、优化器、KV Cache、通信缓冲和工作区分别列账，不能把模型文件大小当峰值显存。
5. **精度题同时算范围和间距**：位数更少可能带来溢出、下溢或舍入误差。量化后必须同时比较质量、非有限值、显存、吞吐、TTFT 和 TPOT，不能只看文件缩小比例。
...
1. 使用十进制单位时，`1.5 TB/s = 1500 GB/s`，所以理论下界是 `60 GB ÷ 1500 GB/s = 0.04 s = 40 ms`。GB 在分子分母中约掉。它只是假设连续搬运且带宽完全可用的下界；Kernel 启动、计算、访存不连续和其他流量只会让实测更慢。若题目使用 GiB/TiB，也必须让两边单位一致后再除。
2. 权重只是固定显存账。每个并发请求还需要 KV Cache、临时激活、输出缓冲和调度元数据，Kernel 也需要工作区；显存碎片和安全余量还会减少可用空间。应先从可用显存扣除权重与工作区，再用剩余额度除以单请求高分位缓存，才能估算并发。
3. BF16 用 8 位指数保留接近 FP32 的范围，但尾数较短；FP16 用更多尾数位换来更窄的指数范围；FP8 再减少字节和潜在计算成本，同时必须在不同格式、缩放、溢出范围与舍入误差间权衡。它们不是简单的“8 位一定比 16 位差一半”。
4. `W8A8` 把权重和激活都量化为约 8 bit；`W4A16` 主要把权重压到约 4 bit，激活仍约 16 bit；KV Quantization 压缩推理期间保存的 Key/Value Cache。三者首先减少的是不同资源账，还会有 scale、zero point、分组和对齐开销。
5. 先查三项：第一，实际执行的 Kernel 是否原生支持该格式，还是每轮先反量化回高精度；第二，TPOT 的瓶颈是否在 KV、通信、调度或小 Batch，而不是权重带宽；第三，运行时显存和搬运字节是否真的下降，包括 scale、临时缓冲与格式转换。随后用 profiler 的 Kernel 时间与内存流量验证，不能只比较磁盘文件大小。
