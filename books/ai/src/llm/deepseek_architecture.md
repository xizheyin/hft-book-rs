...
|---|---|---|
| All-to-All 通信 | Token 要发往其专家所在设备 | 每层发送字节、链路利用率、通信等待 |
| 专家热点 | Router 可能集中选择少数专家 | 每专家 Token 数、最大/平均负载比 |
| 容量溢出 | 专家一次能接收的 Token 有限 | 丢弃、重路由、Padding 或等待 |
| 小 Batch 利用率低 | 每个专家拿到的 Token 太少 | 每专家 Batch、Kernel 启动开销 |
| 权重内存大 | 总参数仍要保存或分片 | HBM 占用、加载时间、并行布局 |
...
2,048 个历史 Token 就约为 1GiB/请求。真实模型可能使用 GQA、MQA、MLA、低精度缓存等，数字会不同；这个算例只说明为什么 KV Cache 会限制并发。
...
DeepSeek-V2 还把与位置有关的 RoPE 部分解耦。更精确地说，推理缓存不是一句“只存一个向量”就能概括：它包含压缩后的 KV 潜在表示，以及位置编码所需的相关部分。报告还利用矩阵吸收等推导减少显式恢复完整 K/V 的开销。
