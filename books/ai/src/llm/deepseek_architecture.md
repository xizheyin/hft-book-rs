...
> 本章目标：以 DeepSeek-V2 和 DeepSeek-V3 的公开技术报告为边界，讲清 DeepSeekMoE、MLA 和 MTP 的机制、收益、代价与服务含义。本文不推测未由官方技术报告说明的“V4”细节。

## 学习优先级

| 优先级 | 先掌握什么 | 面试要求 |
|---|---|---|
| P1 | MoE 为何省每 Token 计算却增加通信，MLA 为何降低 KV Cache，MTP 与推测解码的边界 | 能把三种技术分别对应到计算、缓存和训练/解码 |
| P2 | V2/V3 的精确参数、专家细分方式、负载均衡实现和矩阵吸收 | 引用时说明版本，不要求现场推导 |

下面的版本数字用于建立事实边界，不需要脱离报告死记。
...
普通 Dense FFN（稠密前馈网络）会让每个 Token 经过同一组完整参数。MoE（Mixture of Experts，混合专家）把 FFN 换成多个 Expert，再由 Router 为每个 Token 选择少数专家。
...
|---|---|---|
| All-to-All 通信 | 多个设备彼此交换 Token，把它们送到专家所在设备 | 每层发送字节、链路利用率、通信等待 |
| 专家热点 | Router 可能集中选择少数专家 | 每专家 Token 数、最大/平均负载比 |
| 容量溢出 | 专家一次能接收的 Token 有限 | 丢弃、重路由、用占位补齐（Padding）或等待 |
| 小 Batch 利用率低 | 每个专家拿到的 Token 太少 | 每专家 Batch、Kernel 启动开销 |
| 权重内存大 | 总参数仍要保存或分片 | HBM（高带宽显存）占用、加载时间、并行布局 |
| 故障域扩大 | 一次请求跨多设备 | 超时、节点失效、重试边界 |

<details>
<summary><strong>P2 选读：V3 的版本特定负载均衡边界</strong></summary>

DeepSeek-V3 报告描述了面向负载均衡的辅助损失免除策略，并保留序列级的补充平衡约束。面试时不必背公式，但应说清目标：**避免为了均匀路由而让主要语言建模目标承受过强干扰，同时仍防止专家长期失衡。** 这是 V3 报告中的版本特定设计，不应反推所有 MoE 都这样实现。
...
<details>
<summary><strong>P2 选读：MLA 缓存里不只有一句“压缩向量”</strong></summary>
...
| 轮次 | 草稿 | 主模型接受 | 实际前进 |
|---|---|---|---:|
| 1 | A B C | A、B 正确，C 被拒 | 2 Token |
| 2 | D E F | 仅 D 正确 | 1 Token |

加速取决于接受率、验证 Kernel、批处理和额外模块成本。接受率低时，草稿工作可能白做。因此正确表述是“MTP 可支持推测解码”，不是“有 MTP 就固定快 N 倍”。官方仓库也明确把 MTP 推理支持视为需要推理框架配合的能力。
