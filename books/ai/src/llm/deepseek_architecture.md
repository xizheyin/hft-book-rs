...
## 12. 做题方法：MoE、MLA 与 MTP 分三张账

1. **MoE 题先区分总参数和激活参数**：总参数影响权重存储与分片，每 Token 激活参数更接近算术量；还要单列 Router、共享专家、专家并行通信和负载均衡。
2. **专家路由题画 Token 去向**：记录每个 Token 选中了哪些专家、每个专家收到多少 Token、容量是否溢出。平均负载正常也可能存在单个热门专家拖慢 All-to-All。
3. **MLA 题从普通 KV 基线开始**：先按层数、KV 维度、序列和精度算普通缓存，再替换为报告中的潜在表示与位置相关部分。不能只用一个“压缩倍数”代替真实布局。
4. **MTP 题区分训练和推理**：训练时是更远 Token 的附加预测目标；只有推理框架实现候选提出与主模型验证时，才形成推测解码。候选数不是最终一次接受的 Token 数。
5. **加速题写期望值而非固定倍数**：至少考虑候选接受率、验证成本、额外模块、Batch 和通信。接受率下降时，更多候选反而可能浪费计算。
6. **最后核对版本**：V2、V3、R1 与具体开源权重的参数和功能并不相同；所有数字都要能回指对应技术报告或官方仓库。
