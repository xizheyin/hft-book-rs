...
1. 列出每个 key/租户的状态大小、读写速率和必须同分片执行的操作，再按候选规则把它们分配到 shard。对每个 shard 分别求状态量、请求率和最热单键，不能只看平均值。
2. 稳定吞吐由最先饱和的 shard 或共享阶段决定。若四个 shard 的容量都是 100k/s，而一个热点 shard 收到 130k/s，其他 shard 空闲不能把总系统救回来。
3. 标出跨分片操作需要的消息数、同步点和失败语义。一个原本原子的操作被拆成两条队列消息后，必须说明中途失败怎样恢复，而不是只计算并行度。
4. 对同一固定工作量，核数从 1 增到 p 时，计算 `speedup=T1/Tp` 和 `efficiency=speedup/p`，再把串行路由、共享队列、内存带宽等不随 p 扩展的部分标出来。若改用吞吐比较，必须固定负载与延迟约束。吞吐增加但核时增长更快时，扩展效率正在下降。
5. NUMA 题给线程、队列游标和主要状态标所属节点，再追每次跨节点读写。把线程迁到另一 socket 后若状态仍留在原节点，所谓“多一个核心”可能增加远端访问。

最终验算三个不变量：同一状态在任一时刻只有明确 owner；路由与 owner 版本一致；任何局部过载都能通过水位或拒绝向上游显式传播。
