...
因此，工程实现应优先使用经过审计的库并验证其语义；一百行未经模型检查的 `unsafe` 代码不能证明并发队列正确。
...
## 11. 做题方法：拓扑、容量、满载语义三张表

1. 先画每个生产者和消费者，不要先选队列名。数出谁写入预留游标、谁读取、是否允许多个消费者竞争；能拆成多条 SPSC 时，还要标出最终由谁仲裁以及是否可能饿死某一路。
2. 用 `容量 ≥ 峰值到达率 × 最长消费暂停 × 安全系数` 做第一版估算，统一秒和毫秒单位。再计算最坏排队时间约为 `容量/消费速率`，检查“大容量”是否已经让过期工作长期滞留。
3. 给每类元素写满队列动作：拒绝、丢弃并恢复、合并、阻塞或降级。动作必须能从 API 返回值和指标中观察，不能只写在运维文档里。
4. 对 MPSC/MPMC 插入“生产者预留较早位置后暂停”的故障。若后续位置已发布，消费者怎样识别洞？每槽 sequence 是否同时区分可写/可读和代次？
5. 验算基准是否真的覆盖目标拓扑：生产消费是否跨核、元素大小和批量是否相同、是否报告 Full 次数与元素 age。单线程 push/pop 结果不能回答共享游标争用。
