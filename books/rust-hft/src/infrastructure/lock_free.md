...
锁不是“性能毒药”，无锁也不是“速度魔法”。两者首先是不同的**进展保证与状态协调方式**：互斥锁（Mutex）允许线程等待持锁者；无锁（lock-free）算法要求系统整体持续完成操作，即使某个参与者暂停。这种差异会影响尾延迟、调度依赖和缓存行争用。
...
严格的 lock-free 算法不应出现“所有线程无限重试却没有任何完成”的永久活锁，否则它不满足系统整体进展定义；但它**允许个别线程饥饿**。某个线程的 CAS 连续失败只说明它可能饥饿，不能据此断定整个算法处于活锁。
