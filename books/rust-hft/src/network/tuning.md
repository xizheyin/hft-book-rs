...
## 11. 故障定位方法：建立可证伪的调优实验

1. **固定问题与基线**：写清流量模型、包长、burst、连接/组播频道、CPU 拓扑、内核/驱动/固件和目标指标，不用一次平均值概括。
2. **画四层队列**：NIC ring、softnet/backlog、Socket buffer、应用队列分别标生产率、消费率、容量、drop 与消息年龄。
3. **找第一处饱和**：在同一时间窗口读取增量计数与队列水位；上游已有 drop 时，下游“没有 drop”不能证明下游配置正确。
4. **一次只改一项**：亲和性、ring、coalescing、offload、buffer、busy poll 分开实验，预先写预期变化和回滚值。
5. **验算因果**：目标异常应随改动按预期变化，同时确认 CPU、吞吐、分位延迟和其他层 drop 没有恶化；复测回滚后问题应可重现。

常见陷阱：不同时间窗口比较累计计数；只报 p50；调大所有 buffer 掩盖排队；把某个 sysctl 当跨内核通用答案；没有真实 microburst 就评价抗突发能力。
