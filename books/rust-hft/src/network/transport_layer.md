...
覆盖序号 5000～5799，确认号为 5800，下一段第一个字节序号也为 5800。
...
有效窗口为 `min(12,000, 8,000)=8,000 byte`，还能新发 `8,000-3,500=4,500 byte`。
...
慢启动每 RTT 近似翻倍：`1 → 2 → 4 → 8 → 16 MSS`，所以 4 个 RTT 后为 16 MSS，随后进入拥塞避免。
