...
> **面试优先级：P0。** 先讲清阻塞、非阻塞、`epoll` 的“就绪通知”，再谈 `io_uring` 的“提交与完成”。DPDK、AF_XDP 等用户态收包方案只在岗位明确要求时深入。

网络程序经常不是“算得慢”，而是在**等数据**。I/O 模型要解决的问题是：数据暂时没到时，线程应该睡眠、检查别的连接，还是一直轮询？

面试官问这一章，通常不是要背接口年代，而是想确认你能否解释三件事：
...
| 协议栈处理 | TCP、路由、过滤等语义 | UDP、内核旁路 | 需要自己补回可靠性或运维能力 |
...
4. 明确要求个位数微秒、愿意独占核心和管理网卡队列：才评估 busy polling、AF_XDP、DPDK 或厂商旁路栈。
...
> 阻塞 I/O 在暂时无数据时让线程睡眠；非阻塞 I/O 立即返回 `EAGAIN`，但单独使用会迫使应用扫描或忙轮询。`select` 和 `poll` 每次传入并检查整个集合；`epoll` 持久保存关注集合，并返回就绪事件，所以大量连接但少量活跃时更合适。`epoll` 给的是 readiness，应用仍要非阻塞读到 `EAGAIN` 并处理短读；`io_uring` 更接近提交操作后收 completion。再低延迟可以忙轮询或做内核旁路，但会换来核心占用、内存管理和运维复杂度。
...
不一定。订单连接可能受交易所协议、网络距离和限频主导；成熟内核栈配合绑核、批量和正确 socket 参数常已足够。只有实测证明内核路径是关键瓶颈，且团队能承担旁路栈成本时才值得升级。
...
- [Linux io_uring 文档](https://docs.kernel.org/io_uring/)
- [Linux NAPI 文档](https://docs.kernel.org/networking/napi.html)

下一步可读 [io_uring](./io_uring.md)；只有岗位需要时，再进入 [Kernel Bypass](./kernel_bypass.md)。
