...
## 8. 代码推演方法：就绪、完成与部分 I/O

1. **先标 FD 模式**：阻塞还是非阻塞、LT 还是 ET、单线程还是多个消费者；这些条件决定返回值该怎样解释。
2. **画事件循环状态**：等待事件→收到就绪→循环读/写→成功推进 offset→遇到 `EAGAIN` 重新注册兴趣→EOF/错误清理。完成式接口则画提交→在途→CQE→回收。
3. **逐次代入返回值**：为 buffer 记录 `len` 与 `offset`，正数表示推进多少，0 在流式读上表示 EOF，`EAGAIN` 表示暂时无法推进，不能从头重做。
4. **检查并发竞态**：就绪到真正调用之间可能被其他线程消费；取消与完成可能交错，资源释放必须只发生一次。
5. **验算**：ET 处理后应读到 `EAGAIN`；发送队列的剩余字节应单调减少；每个在途操作最终只有一个明确的完成/取消归属。

常见陷阱：把 readiness 当数据预留；把非阻塞当异步；忽略 partial write；ET 使用阻塞 FD；事件循环在单个连接上无限处理导致其他连接饥饿。
