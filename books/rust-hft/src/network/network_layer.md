...
需要的子网位数 k 满足 `2^k≥6`：`2²=4<6`、`2³=8≥6`，所以从 `/24` 借 3 bit，得到 `/27`，掩码为 `255.255.255.224`。主机位 5 bit，每段 `2^5=32` 个地址，传统可用主机数 `32-2=30`。
...
验算块大小为 `256-224=32`，所有起点都是 32 的倍数，范围互不重叠；还剩 `.192/27` 与 `.224/27` 两段。
...
原始载荷 `3020-20=3000 byte`。每片还要带 20-byte 首部；非最后片载荷最大为 `floor((1000-20)/8)×8=122×8=976 byte`。

| 分片 | 原载荷字节范围 | 载荷 | 总长 | Fragment Offset=`起点/8` | MF |
|---:|---|---:|---:|---:|---:|
| 1 | `0..975` | 976 | 996 | 0 | 1 |
| 2 | `976..1951` | 976 | 996 | 122 | 1 |
| 3 | `1952..2927` | 976 | 996 | 244 | 1 |
| 4 | `2928..2999` | 72 | 92 | 366 | 0 |

验算：载荷和 `976×3+72=3000`；每片总长都不超过 1000；前三片载荷能被 8 整除；最后 offset `2928/8=366`。
...
目的末字节 `200` 的二进制是 `11001000`。逐条匹配：

| 路由 | 覆盖范围 | 是否命中 |
|---|---|---|
| `192.168.0.0/16→A` | `192.168.*.*` | 是 |
| `192.168.10.0/24→B` | `192.168.10.0–255` | 是 |
| `192.168.10.128/25→C` | 末字节最高位为 1，即 `128–255` | 是 |
| 默认→D | 全部目的 | 是 |

命中项前缀长度依次 16、24、25、0，最长为 `/25`，所以选 C。验算：若目的改为 `.100`，它不在 `128–255`，将选择 `/24` 的 B。

</details>

## 15. 做题方法：子网、分片与最长前缀匹配

1. **子网题写二进制边界**：把前缀长度 `/p` 转成掩码，IP 与掩码按位与得到网络地址；主机位全 1 得广播地址，再按题意判断网络地址和广播地址是否可分配。
2. **分配题先排需求**：按主机数从大到小选择最小可容纳前缀，网络起点必须落在块大小边界；每分一块就更新剩余区间，避免重叠。
3. **LPM 题逐项匹配**：对每条路由用其掩码匹配目的地址，列出全部命中项，再选前缀最长者；度量只在前缀同样具体时参与题目规定的比较。
4. **分片题先算载荷**：每片总长度不得超过 MTU，除最后一片外载荷通常取 8 byte 的整数倍；偏移量写“原始数据起点/8”，MF 位按是否还有后续分片设置。
5. **验算**：所有分片载荷之和应等于原始载荷；每片总长度包含自己的 IP 首部；子网范围互不重叠且覆盖题目要求。

常见陷阱：把 `/24` 当作 24 个主机位；LPM 选择数值最大的网络地址；ARP 远端通信时解析远端主机而不是默认网关的 MAC；把 NAT 当作自动提供访问控制。
...
IP 核心只尽力转发数据报，不承诺送达、顺序、唯一性或固定时延，使路由器保持通用、简单并能跨异构链路扩展。链路层可在一跳内重传，TCP 可在端到端提供可靠有序字节流，应用层还可用业务序号、校验、幂等和补发恢复；补哪一层取决于所需语义。
...
数据面对每个到达分组执行查表、排队和转发；控制面通过静态配置或路由协议计算可达路径并填充转发表。最长前缀匹配是数据面的逐包查表动作，路由协议选择哪些前缀和下一跳属于控制面。
...
TTL 每经一个路由器至少减 1，到 0 丢弃，限制环路分组寿命；Protocol 指出载荷交给 TCP、UDP、ICMP 等哪个上层协议；Header Checksum 只检测 IPv4 首部，因 TTL 会改变，路由器每跳都要更新。它不校验载荷，也不能替代传输层校验。
...
IPv4 片偏移字段只有 13 bit，以 8 byte 为单位可覆盖原数据报载荷的完整范围，所以除最后一片外载荷通常是 8 的倍数。重组需要全部分片；任一片丢失都会使整个原数据报无法交付，上层往往重传整段数据，因此分片越多，成功集齐的概率越低。
...
先写 32 bit 掩码：前 `p` 位为 1。`网络地址=IP AND 掩码`；`广播地址=网络地址 OR (NOT 掩码)`。传统普通子网可用主机范围是二者之间，即网络地址加 1 到广播地址减 1。验算块大小为 `2^(32-p)`，网络起点必须落在块边界；`/31` 点到点和 `/32` 主机路由是特殊语义，不能机械扣 2。
...
LPM 先在所有匹配目的地址的路由中选择前缀最长、范围最具体的一条。路径度量用于控制面比较到同一前缀的候选路径，或按协议规则选优。不能用较小度量的 `/16` 覆盖已经命中的 `/24`；先比较前缀具体程度，再按路由来源和度量处理同前缀候选。
...
主机先用掩码判断目的是否同网段。远端目的需要交给路由表选出的下一跳，ARP 解析的是该下一跳（通常默认网关）的 MAC，而不是远端主机 MAC。IP 目的仍写远端地址，当前帧的目的 MAC 写网关；每跳重新封装。
...
客户端刚启动时还不知道自己的 IP、DHCP 服务器地址和所在网络，所以发现与请求的早期报文常用广播让本地服务器或中继看到。租约规定地址只在一段时间内归客户端使用，使服务器能回收、续租并避免地址永久占用；客户端必须按租期状态机更新，不能把曾经获得的地址永久保留。
...
ICMP 传递网络层差错与诊断消息。`ping` 通常发送 ICMP Echo 并等待 Echo Reply；`traceroute` 利用逐步增加 TTL 后路由器返回的 Time Exceeded 消息观察路径。`ping` 通只证明某时刻 ICMP 往返可达，不能证明目标 TCP/UDP 端口监听、应用线程健康、鉴权成功或业务依赖正常。
...
PAT 把不同内网四元组改写为同一公网 IP 上不同源端口，并保存映射；返回报文按公网端口查表还原到对应主机。它的主要职责是地址/端口转换和复用，并不表达完整安全策略；静态映射、现有状态或错误规则仍可允许流量。防火墙要按独立访问控制策略决定允许与拒绝。
...
IPv6 地址为 128 bit；基本首部固定 40 byte，扩展功能放扩展首部，不含 IPv4 式首部校验和。路由器不做分片，源主机依据路径 MTU 处理；IPv6 没有广播，使用组播；邻居发现使用 ICMPv6 的 NDP，替代 IPv4 ARP 的主要职责。地址更大不自动提供防火墙或端到端安全。
