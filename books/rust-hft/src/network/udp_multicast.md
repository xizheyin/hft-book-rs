...
## 10. 故障定位方法：从序号缺口反推丢包层

1. **读缺口形态**：记录产品、频道、A/B 线路、起止序号、发生时间和是否持续；单路缺口、双路同缺口和本机所有频道同时缺口指向不同范围。
2. **画两条独立路径**：上游发送→交换网络→NIC queue→内核→Socket→解析器→订单簿，每层同时列 A、B 计数，避免只看合并后的序号。
3. **先恢复再归因**：小缺口按协议重传；跨度过大或状态不可信时取快照并追赶增量。恢复前明确标记 stale，不能用“后续包看起来正常”代替一致性。
4. **同窗对账**：比较交换机/NIC drop、softnet、Socket drop、应用接收数、解析拒绝与 sequence gap，找最早分叉点。
5. **验算恢复状态**：快照基准序号、缓存增量范围和最终期望序号必须连续；重复包不应再次改变订单簿；A/B 切换不能倒退序号。

常见陷阱：把 IP 组播当可靠广播；两路经过同一故障域却称为冗余；发现 gap 后继续使用污染状态；把大 buffer 当长期消费不足的修复；混淆 packet sequence 与一包内 message sequence。
