...
## 15. 故障定位方法：沿收发路径找第一处证据

1. **读现象**：先区分完全无包、吞吐下降、延迟上升和应用序号缺口，并固定接口、队列、CPU、时间窗口和流量方向。
2. **画路径与所有权**：RX 按 NIC ring→驱动/NAPI→协议栈→Socket 队列→应用，TX 反向画；每段标 buffer 由谁拥有、队列满时在哪里丢。
3. **逐层取同窗增量**：先看链路/NIC，再看 softnet 与协议统计，再看 Socket 队列，最后看应用消费和序号；找到最早出现异常计数的一层。
4. **做单变量验证**：固定负载，只改变一个队列、亲和性、buffer 或 offload；同时保留吞吐、分位延迟、CPU 和丢包，确认异常层随改动变化。
5. **反向验算**：应用 gap 必须能由某层 drop、解析拒绝或序号逻辑解释；若所有计数都正常，检查观测点是否绕过 fast path、抓包是否受 GRO/TSO 影响。

常见陷阱：只看累计计数；一见丢包就加 buffer；把 `send` 返回当上网线；看到抓包中的大包就断言线缆传了超 MTU 帧。
...
NIC 校验并用 DMA 把帧写入 RX buffer，完成描述符；中断通知后 NAPI 轮询取走描述符，驱动构造内核数据对象并补充 buffer；协议栈依次处理链路、IP、传输层并按 Socket 分用，数据进入接收队列；等待线程被唤醒、经调度运行，`recv` 再按接口语义复制或引用数据。每一步都可能排队，不能把 DMA 完成等同于应用已读。
...
描述符是 NIC 与驱动共享的元数据槽，保存 buffer 地址/长度、完成状态及校验等标志；接收 buffer 才保存帧字节。所有权通常在“驱动准备→NIC 填充→驱动回收”之间转移。只增加描述符却没有可用 buffer，队列仍不能接包。
...
低负载时由硬件中断及时通知有包；进入处理后暂时抑制该队列中断，由 NAPI 在预算内批量轮询，摊薄每包中断成本。队列排空后重新启用中断。它是“中断负责唤醒、轮询负责批处理”的组合，不是永久忙等。
...
协议栈先把 Socket 标为可读并唤醒等待队列；线程从睡眠转为可运行后，还要等待调度器分配 CPU，恢复上下文，再从系统调用继续并按 `recv` 规则取数据。CPU 忙、优先级或锁竞争都可能让“已入 Socket 队列”到“应用运行”之间仍有等待。
...
流式 Socket 的 `recv` 只返回当前可取得的部分字节，队列可能仍有数据，后续字节也可能继续到达。它证明本地应用拿到了这些字节，不证明消息已经完整解析，更不证明远端业务事务成功；业务完成必须由应用协议的完整响应和状态语义确认。
...
常规内核 Socket 中，`send` 成功通常只表示本机接受了相应字节。之后还要经过 Socket 发送缓冲、传输/IP 处理、排队规则与设备队列、驱动 TX ring，NIC 用 DMA 读取 buffer，完成校验/分段等卸载后才在线路发送；拥塞、调度和网卡队列都会推迟它。
...
GSO 在发送路径让协议栈携带较大对象，稍后软件分段；TSO 把 TCP 分段交给 NIC；GRO 在接收路径把符合条件的多个包合并为较大对象以减少处理次数。若抓包点位于分段前或聚合后，就会看到比线上帧更大的对象，因此必须注明抓包位置和 offload 状态。
...
NIC drop/missed 常指 RX ring 或硬件来不及接收；softnet backlog drop 指包已到内核但每 CPU backlog/处理预算无法容纳；Socket drop 指协议已分用但应用读取慢或接收缓冲不足。按同一时间窗比较接口/每队列、softnet、协议栈/Socket 和应用 gap，第一处同步增长的计数最接近故障起点。
...
若到达率 `λ` 长期大于消费率 `μ`，队列净增长率为 `λ-μ>0`；容量从 `B` 增到 `2B` 只把溢出时间从约 `B/(λ-μ)` 推迟到 `2B/(λ-μ)`，不会改变斜率。必须降低输入、提高处理率或定义丢弃/背压策略，缓冲只吸收有限突发。
...
画出应用 buffer、内核 buffer、页缓存、NIC ring/buffer 等实际对象；每条箭头标 CPU 字节复制、DMA、映射还是仅传元数据；每个阶段标谁能读写、何时完成、何时可复用。然后分别统计复制、系统调用、页固定和生命周期成本，不能只用“零拷贝”标签下结论。
...
至少说明哪个具体 buffer/范围完成、通知表示“已被内核接收”还是“设备不再访问”、通知前谁可以读写、通知后谁负责回收或重新提交，以及失败/取消时所有权回到哪里。若应用在设备 DMA 完成前复用 buffer，即使 `send` 已返回也可能破坏数据。
