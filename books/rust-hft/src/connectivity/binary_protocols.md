...
## 6. 做题方法：从线上的 byte offset 推到业务字段

1. **读规范做布局表**：每个字段列 offset、长度、端序、符号、缩放、枚举和可选条件；包头长度与消息长度的包含范围单独写清。
2. **拿十六进制样例逐字段解码**：先验证剩余长度，再按端序读整数，之后才做价格/数量缩放和枚举转换；游标每步增加的字节数都记录。
3. **建立 framing 状态机**：输入不足包头、包头完整但 body 不足、完整多消息、未知模板和坏长度分别有确定行为，不从未验证长度分配大内存。
4. **做 round-trip 与差分**：规范允许时执行 encode→decode；用官方样例、另一实现或抓包作 oracle，并对截断、翻转位、极值和未知字段 fuzz。
5. **验算**：最终游标恰等于声明边界；缩放后的值能无损或按规则回编码；未知枚举不会被误当默认合法业务值。

常见陷阱：`transmute` 结构体造成对齐、填充和端序错误；先读后查长度；价格用浮点累积；把一个协议版本的模板套到另一个版本；未声明单位和 null sentinel。字段布局必须以具体交易所/SBE/ITCH/OUCH 版本规范为准。
