...
## 9. 做题方法：用订单状态表处理竞态与未知结果

1. **读题分三个身份**：客户端订单 ID、场所订单 ID、每次请求/执行 ID 分栏记录；重试必须复用还是新建标识由协议规定，不能临时猜。
2. **画命令与事件两条线**：New/Cancel/Replace 是本地意图，ACK/Reject/Fill/CancelAck 是外部事实；`send` 返回只更新传输进度，不直接跳到业务终态。
3. **维护状态转移表**：对每个当前状态列允许事件、数量变化、风险占用变化和重复事件行为，重点推演 Fill 与 CancelAck 交错、Replace 与 Fill 交错。
4. **超时进入 Unknown**：保存请求与字节/会话证据，优先查询、等待回报或 drop copy 对账；只有协议证明安全时才重发。
5. **验算不变量**：`filled + leaves = order_qty`（按该场所修改语义调整）；累计成交不倒退；终态不能被旧事件复活；每个执行标识只计入一次持仓。

常见陷阱：发出撤单就释放全部风险；ACK 当 Fill；重连后无条件重发 New；只按到达顺序处理而不识别重复/旧事件；把某一场所的 PendingReplace 与优先级规则当通用 FIX 规则。
