...
## 6. 做题方法：先确认 L1/L2/L3 信息粒度

1. **读题列可见字段**：L1 只有最优价量，L2 是价位聚合，L3 才有订单身份与队列事件；先判断题目是否真的提供队列位置所需信息。
2. **画买卖两侧结构**：买盘价格降序、卖盘升序，每档写总量；L3 再在档内按场所规则排列订单。
3. **逐事件更新**：新增、减量、删除、成交、改价分别推一次；L2 只改聚合量，L3 同时维护订单索引与价位容器。
4. **列不变量**：数量非负、空档立即删除、best bid/ask 指向首档、订单只属于一个价位、聚合量等于档内订单量之和。
5. **验算**：事件前后总量变化可由事件解释；同一序列重放得到相同 book hash；L1/L2 输出与内部更细视图投影一致。

常见陷阱：用 L2 推断精确排队；删除最后一单后保留零量价位；把修改规则跨场所统一；浮点价格做 map key；忽略拍卖、隐含单和 crossed/locked 状态的场所语义。
