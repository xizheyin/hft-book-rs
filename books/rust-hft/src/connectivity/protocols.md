...
1. **读题标通道职责**：行情、订单入口、执行回报和 drop copy 分别承载什么；再标 TCP/UDP 只提供的传输语义，不能把应用序号交给网络层解释。
2. **画行情与订单两条时间线**：行情线写登录/订阅、增量序号、gap、重传/快照；订单线写登录、发送、ACK/Reject/Fill/Cancel 和重连对账。
3. **列协议状态**：传输连接状态、会话序号、业务订单状态分开记录。连接恢复不等于会话已同步，会话同步也不等于订单结果已知。
4. **用故障事件推演**：丢包、重复、乱序、断线、进程重启和交易所日切各走一次，指出权威事实源与恢复入口。
5. **验算**：行情处理后序号连续且订单簿可证；订单生命周期没有把发送成功当成交；drop copy 与主回报差异能进入对账流程。

常见陷阱：把 FIX 当传输协议；认为 UDP 必然等于组播；所有交易所共用同一消息和恢复规则；把行情 channel sequence、消息数和业务订单 ID 混成一个计数器。具体字段与日切规则必须以目标场所和版本规范为准。
