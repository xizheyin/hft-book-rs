...
闭包类型题先为每个被捕获变量建立一行记录：

| 变量 | 闭包体中的操作 | 最小捕获方式 | 对闭包 trait 的影响 |
|---|---|---|---|
| `x` | 只读 | `&x` | 可满足 `Fn` |
| `count` | 修改 | `&mut count` | 至少需要 `FnMut` |
| `s` | 移出或消耗 | 按值 | 只能保证 `FnOnce` |

推演时先忽略 `move` 关键字，根据闭包体实际怎样使用变量求出最小捕获方式；再加入 `move`，判断捕获的是值本身还是某个引用值。随后检查调用次数：`Fn` 可重复且不要求可变闭包，`FnMut` 调用需要可变访问闭包，`FnOnce` 消耗闭包状态。

选择接口时再比较三条路径：泛型 `F: Fn...` 会静态分发并为具体闭包类型实例化；`dyn Fn...` 通过 trait object 动态分发且需要指针；`fn` 只能装函数项和不捕获闭包。验算点是调用参数与返回类型完全一致、捕获值活得足够久，以及调用方式没有强于闭包实际实现的 trait。
