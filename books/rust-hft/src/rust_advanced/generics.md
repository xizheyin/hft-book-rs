...
泛型不只是“少写几遍代码”。在低延迟系统里，它还决定了调用方式、指令缓存占用和接口能否做到零拷贝。

学完本章，你应该能回答三个问题：

1. 泛型为什么常常很快，又为什么可能让程序变大？
2. `dyn Trait` 什么时候值得用，什么时候应该换成泛型或枚举？
3. 标准 `Iterator` 能返回什么引用，GAT 又解决了什么问题？
...
二进制文件大一点不只是多占磁盘。热路径机器码如果膨胀过多，还可能挤压 CPU 的指令缓存（I-cache），反而让尾延迟变差。
...
```mermaid
flowchart TD
    A["这个调用是否位于已确认的热路径？"] -->|否| B["优先选择更清晰、可维护的设计"]
    A -->|是| C["实现类型集合是否固定且很少？"]
...
- 行情逐条解析、订单簿更新等高频内层循环，先考虑泛型或封闭枚举；
- 启动配置、监控、管理接口、低频插件边界，`dyn Trait` 往往完全合理；
