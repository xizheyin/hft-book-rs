...
泛型与 trait 约束题先写“待证明的义务”：对每个泛型函数调用，把具体类型代入 `T`，列出编译器必须找到的每个 `T: Trait`、关联类型等式和生命周期约束。

1. 先进行类型代入，不要用“泛型什么都能传”跳过约束。
2. 对方法调用列出候选实现，检查接收者类型、trait 是否在作用域、`where` 条件是否成立；若有多个候选，再看是否需要完全限定语法。
3. 关联类型题把 `<T as Trait>::Item` 当成由实现唯一选定的类型；trait 泛型参数则允许同一类型拥有多组不同参数的实现，二者不能互换。
4. 单态化题按“被实际使用的具体类型组合”统计实例，而不是按源代码中泛型函数个数统计；再判断代码体积与静态分发收益。
5. trait object 题检查 trait 是否可作为动态对象、方法调用需要哪些接收者信息，以及数据指针与虚表如何配合。
6. GAT 或 lending iterator 题逐次调用给返回借用标注与 `&mut self` 同源的生命周期，确认上一次借用结束后才能再次可变借用迭代器。

验算点是：每个关联类型都能追溯到一个具体 `impl`，每个 trait 义务都有唯一可用的实现，返回引用没有比其来源活得更久。
