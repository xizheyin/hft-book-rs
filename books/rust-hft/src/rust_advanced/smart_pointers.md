...
“智能指针”不是一种固定实现，而是一组像指针一样访问数据、同时携带所有权规则的类型。`Box<T>` 表示独占堆所有权，`Rc<T>`/`Arc<T>` 表示共享所有权，`Cow<'a, T>` 表示借用或按需拥有。选择时要分别考虑 **分配、间接访问、引用计数、释放时机和线程边界**。
...
智能指针题先画有向所有权图：节点是对象，实线是拥有边，虚线是借用或 `Weak` 边，并在 `Rc/Arc` 节点旁记录 strong/weak 计数。

1. 每次 `clone`、`downgrade`、`upgrade` 和 `drop` 后更新计数；strong 归零时销毁值，控制块还要等 weak 计数条件满足才释放。
2. 图中若实线形成环，逐个删除外部根后检查 strong 是否仍非零；若是，说明仅靠引用计数无法回收，应把非拥有反向边改成 `Weak`。
3. `Box` 题确认唯一所有者与稳定堆地址是否真有必要；`Cow` 题逐操作标记当前是 Borrowed 还是 Owned，第一次写入才可能克隆。
4. Arena/Slab 题同时记录槽位代数或版本，释放后旧 handle 不得误指向复用的新对象。
5. `Arc` 只让引用计数线程安全，不自动让内部 `T` 的复合修改安全；若需要共享修改，继续画锁或原子状态。

验算点是每个堆对象都有明确释放条件，任何借用边都不比其来源长，所有环都至少有一条不拥有的边。

## 问题与推演

### Q1：为什么频繁跨核 `Arc::clone` 可能成为问题？
...
智能指针表达的是所有权策略，而不是速度等级。先画清数据由谁创建、谁使用、跨不跨线程、何时释放，再评估分配、间接访问和计数成本；性能优化应改变真实生命周期，而不是机械替换类型。
