...
内部可变性题先分两层权限：外层引用是 `&` 还是 `&mut`，内层机制在运行时或原子层面又允许什么操作。可用状态表逐步记录：

| 机制 | 当前状态 | 本次操作 | 新状态/结果 |
|---|---|---|---|
| `RefCell` | 0 个共享、0 个独占 | `borrow()` | 1 个共享 |
| `RefCell` | 1 个共享 | `borrow_mut()` | panic / `try_borrow_mut` 返回错误 |
| `Mutex` | 未持有 | `lock()` | 当前线程持有 |

具体步骤是：先判断数据是否跨线程；单线程动态借用可考虑 `Cell/RefCell`，跨线程共享修改需要原子或锁。再判断操作是单个可原子更新的值，还是必须让多个字段共同保持不变量。对 `RefCell` 模拟共享计数与独占标志，对锁模拟持有者和等待者，并在每个提前返回点检查 guard 是否释放。

验算点有三个：不能存在两个同时活跃的独占访问；跨线程类型必须满足相应 `Send/Sync` 条件；读出—修改—写回若跨多个原子步骤，整体并不会自动成为原子事务。
