...
`Result` 控制流题把每个可能失败的表达式展开成显式分支：

```text
成功 Ok(v)  -> 取出 v，继续下一步
失败 Err(e) -> 执行 From::from(e)，立即返回 Err
```

按下面步骤推演：

1. 给函数写出完整返回类型，确定所有 `?` 最终要汇入哪一个错误类型。
2. 从左到右列出每个 `Result<T, E>` 的成功值类型与错误类型；遇到 `?` 就画成功边和提前返回边。
3. 在错误边上检查 `From<E>` 转换是否存在；`?` 不会凭空把任意错误变成字符串或另一个枚举。
4. 对 `map`、`and_then`、`map_err` 分别标明改变成功值、串联另一个可失败操作、改变错误值，避免嵌套出 `Result<Result<...>>`。
5. 每条提前返回路径都要检查已创建局部值的析构；panic 的 unwind 与 `Result` 的普通返回是两套控制流。
6. 在系统边界决定一次错误应重试、上报、降级还是终止，依据错误语义而不是依据它来自哪个库。

验算时枚举所有出口：每个出口都必须返回声明的同一类型，且错误上下文没有在一次无信息的 `map_err(|_| ...)` 中被抹掉。
