...
在 HFT 系统里，关键问题不只是“怎样返回错误”，还包括：失败是否预期、谁负责恢复、交易所中的活动订单怎么办，以及错误处理是否影响成功路径。

> **面试优先级**：P0 是失败分类、`Result`/`Option`/panic 的边界、`?` 的错误传播和进程退出后的外部状态恢复；P1 是错误类型大小与热/冷路径拆分；某个 niche 布局的官方特例属于 P2，不值得在基础没稳时死记。
...
<details>
<summary><strong>P2 选读：<code>Result&lt;&amp;T, ()&gt;</code> 的特定布局保证</strong></summary>

下面是标准库文档列出的窄特例，适合库作者或布局审计时查阅。普通面试更重要的是知道：不要凭“最大变体加一字节”猜枚举大小，应对具体类型用工具检查。
...
所以面试中可以说“`?` 是零成本风格的控制流抽象，通常不会比等价 `match` 多出固有成本”，不要说“`?` 保证没有成本”。
...
5. 再根据故障域与恢复流程选择策略，而不是套用“所有 HFT 都必须 abort”。
