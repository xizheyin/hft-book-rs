...
宏题按“输入 token → 匹配 → 展开 → 再做名称解析和类型检查”四阶段推演：

1. `macro_rules!` 先逐个分支匹配 token tree，标出每个 metavariable 使用的片段类型与重复层级。
2. 展开重复时检查 `$()*`、`$()+` 和分隔符能否处理 0 个、1 个和多个输入；嵌套重复中的 metavariable 必须处在兼容层级。
3. 把展开结果写成普通 Rust 代码，再检查所有权、类型和控制流；宏通过匹配不代表展开代码能编译。
4. 检查卫生性：宏内部临时名字不应意外捕获调用方名字，调用方传入的标识符则在正确上下文解析。
5. 过程宏区分读取的 TokenStream、生成的 TokenStream 和错误 span；对无效输入应产生定位清晰的编译错误，而不是 panic。
6. 使用 `cargo expand` 或等价展开工具观察结果，并为正常、边界和应当编译失败的用例分别测试。

验算点是展开后的代码在不依赖“宏魔法”的情况下语义完整，且输入表达式不会因展开被意外求值多次。
