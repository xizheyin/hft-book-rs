...
## 15. 做题方法：用价格档、队列和剩余量三张表

1. **先判断能否成交**：买单价格不低于当前最优卖价、卖单价格不高于当前最优买价时才发生交叉；市价单则按可用对手盘继续扫描。
2. **再选择价格档**：主动买单从最低卖价开始，主动卖单从最高买价开始。没有吃完当前最优档之前，不应跳到更差价格。
3. **档内按明确规则分配**：FIFO 逐订单扣减；pro-rata 先算理论份额，再按交易所规定处理取整和剩余量。不要把两种规则混用。
4. **每笔成交同时更新两侧剩余量**：`trade_qty = min(taker_remaining, maker_remaining)`；归零的订单移出队列，未完成的 Taker 继续处理下一订单或下一档。
5. **最后检查不变量**：成交量不能超过任一方原始数量；限价单不能成交在其限制价格之外；同一数量不能被成交两次；FOK 要么全成要么不成，IOC 未成交部分必须取消。

修改和撤单题还要先查交易所规则：减量是否保留队列位置、改价或加量是否重新排队，不能用统一答案代替具体市场规范。
...
买 50.01 低于 best ask，不成交；买 50.02 达到 ask，可成交。卖 50.01 高于 best bid，不成交；卖 50.00 达到 bid，可成交。结论假设连续交易且无 post-only、停牌等额外指令。
...
按更优价格和同价 FIFO：A 成交 5，B 成交 8，剩余需求 4，再与 C 成交 4；买单全成，C 剩 6。均价 `(5×10+8×10+4×10.01)/17≈10.00235`。
...
总量 100，理想份额分别为 `33×0.6=19.8`、`9.9`、`3.3`。整数手数不能直接分配小数；若先向下取整得到 19/9/3，只分了 31，剩余 2 如何按最大余数、时间优先、最小分配量或随机规则分配必须由场所规定。例如最大余数给 A、B 后为 20/10/3，但不能把它当所有场所规则。
...
`Working --partial fill--> PartiallyFilled --partial fill--> PartiallyFilled`；两者收到 Cancel 请求可进 PendingCancel；Working/PartiallyFilled/PendingCancel 收到最终 Fill 均可进 Filled；PendingCancel 收到 CancelAck 进 Canceled，若期间 PartialFill 可保留 PendingCancel 并减少 leavesQty。Canceled/Filled 是终态，重复回报幂等；场所 reject/replace 状态需扩展完整机器。
...
性质一数量守恒：每次撮合 `入场量=成交量+剩余量`，所有 resting qty 非负。性质二限价边界：买成交价不高于 buy limit，卖成交价不低于 sell limit。性质三优先级：同价 FIFO 中后到订单不能在前单仍有量时先成交。FOK 测试先构造限价内总量 16，再提交 FOK 买17，应零成交且订单簿 hash 完全不变；改为16则一次全部成交。场所若有隐藏量/特殊优先级应替换相应 oracle。
