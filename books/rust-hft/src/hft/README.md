...
不同市场、司法辖区和交易场所有不同规则。系统必须遵守适用法规、交易所规则、经纪商限制和公司内部控制；技术上能够发送某种订单，不代表业务和法规允许这样做。
...
先沿一笔订单从行情输入到成交对账的链路，认识各模块怎样交接事实：
...
stateDiagram-v2
    ...
    PendingNew --> PartiallyFilled: Fill before/separate from ACK
    PendingNew --> Filled: Full Fill before/separate from ACK
...
只画“行情 → 策略 → 发单”是不完整的。一个可运营系统至少有三条路径：
