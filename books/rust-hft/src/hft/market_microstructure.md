...
市场微观结构（Market Microstructure）研究的不是“公司值多少钱”，而是：交易规则、参与者和订单流怎样共同形成可见价格与成交。它是理解电子交易业务的基础。
...
这些指标把某一时刻的可见买卖数量压缩成一个数；使用前既要会计算，也要知道它丢掉了哪些信息。
...
现实定义会处理最优价变化、成交、多个档位和时间衰减。计算前必须声明符号约定和观察窗口；“OFI 大于零”只描述该定义下的订单流，不能直接推出价格必然上涨。
...
执行大单时，可以拿最终成交结果与决策时参考价比较，并纳入费用。这个差异会混合市场移动、自身冲击、等待成本和费用，不能把所有差异都粗暴归因于系统延迟。
...
## 12. 做题方法：先还原订单簿，再按事件逐条推进

1. **画出初始订单簿**：买盘按价格从高到低，卖盘按价格从低到高；同价位再按市场规则排队。先标出 Best Bid、Best Ask、Spread 和各档数量。
2. **每次只处理一个事件**：新增、撤销、修改和成交分别更新对应订单或价位。处理后立即重算最优价，不能把一串事件凭直觉一次跳到结果。
3. **区分订单、价位和成交**：L3 事件作用于具体订单，L2 只保留价位聚合，L1 只有最优档。题目给的信息粒度决定你能否推断队列位置或成交概率。
4. **计算指标前写清口径**：不平衡度使用哪些档位、Microprice 的权重方向、OFI 统计哪些事件，都必须先定义。相同名字在不同论文或交易所规则中可能口径不同。
5. **做守恒验算**：若没有隐藏流动性或数据缺口，某档数量变化应能由新增、撤销、修改和成交解释；Spread 通常不应为负，出现 crossed book 时要检查拍卖、不同步或跨市场视图。
...
spread=`100.02-99.98=0.04`；mid=`(99.98+100.02)/2=100.00`；imbalance=`(120-80)/(120+80)=0.20`；按本章定义，microprice=`(100.02×120+99.98×80)/200=100.004`。数量权重交叉到对侧价格；不同资料若定义不同必须先声明。
...
先在 10.01 成交 10，再在 10.02 成交 15；加权均价 `(10×10.01+15×10.02)/25=10.016`。买单全部成交；10.02 卖档剩 5，10.03 保持 50。忽略费用、隐藏量和场所特殊规则。
...
可选：bid 价格降序、ask 升序；正常连续簿 `best_bid<best_ask`；档位总量非负且等于订单量求和；同一 order_id 唯一；已应用 sequence 连续。任一失败都将该 symbol/generation 标 Stale，停止基于它新增风险，保留原始消息并从冗余 feed/重传/快照恢复；不能“修个负数继续交易”。锁盘/竞价等允许 crossed book 的场景须按场所状态另定不变量。
...
设撤量前你前方可见量估计为 F。最乐观边界是假设 30 全在你前方，前方量变为 `max(F-30,0)`；最保守边界是假设全在你后方，你前方仍为 F。若撤量超过后方可见量或含隐藏/改单，边界还需按协议修正。L2 无订单身份，不能知道真实位置，只能维护假设区间。
...
本机接收时间等于场所产生时间加各自网络、网关、排队和解码延迟；两条路径延迟不同且会抖动，所以先到本机不推出先发生。即使两场所提供源时间戳，也受各自时钟同步误差约束。可证明的是各 feed 内 sequence/因果顺序；跨场所只能在已知误差界内给区间判断，或明确按本机可见顺序决策。
