...
## 13. 做题方法：从输入语法、不变量到最小反例

1. **读目标选方法**：任意字节解析器适合 coverage-guided fuzz；有结构输入用生成器；跨大量输入恒成立的规则用 property test；安全边界再配 sanitizer/Miri 等工具。
2. **定义有效与无效域**：协议长度、枚举、序号、价格数量范围写成生成策略；保留一部分畸形输入，不能因生成器过强只测 happy path。
3. **写独立 oracle**：不崩溃只是最低要求，还要检查 round-trip、数量守恒、状态机合法、优化版等于简单基线和重复事件幂等。
4. **保留状态序列**：状态机 fuzz 生成 New/Fill/Cancel/Reset 等事件列表，每步断言不变量；崩溃后保存 seed、版本和完整最小化输入。
5. **验算修复**：先把反例变成固定回归测试，再修代码；重新运行语料库并确认反例不再触发，同时没有通过放宽断言掩盖问题。

常见陷阱：只 fuzz 单条消息不 fuzz 跨消息状态；panic 被捕获就算通过；oracle 与被测实现共用同一错误函数；语料含秘密；不可重现的线程/时间随机性没有记录。
