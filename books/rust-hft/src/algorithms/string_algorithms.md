...
本章先定义字节、ASCII 与 UTF-8 的边界，再学习频次统计、分词、精确解析、KMP、Trie 和滚动哈希。每个母题都先写语言无关伪代码，再翻译成可独立运行的 C++20；使用哈希筛选时还要保留精确复核或明确碰撞政策。
...
- 是否允许引号和转义？若允许，普通 `split(',')` 不足以解析 CSV（Comma-Separated Values，逗号分隔值格式）；
...
Trie 不是“字符串题必用结构”。当数据量不大，或只做少量查询时，排序后的 `vector<string>`、哈希集合甚至线性扫描可能更简单、更省内存。
...
## 6. 进阶：滚动哈希筛选子串，并做精确复核

滚动哈希适合快速比较许多等长子串，或在二分 LCP（最长公共前缀）时重复查询子串哈希。但它是概率工具：**不同字符串可能得到相同哈希，这叫碰撞。哈希相等不能单独证明字符串相等。**
...
## 9. 章末做题方法：先定义字符单位，再推匹配状态

1. **读题明确单位**：输入是 ASCII byte、UTF-8 字节、Unicode 标量值还是用户看到的字素；大小写、规范化和分隔符规则必须来自题意。
2. **解析题画游标**：逐字段写起点、终点、允许字符和失败位置；整数解析同时检查符号、空字段、前导零与溢出。
3. **KMP 先算前缀表**：对每个位置写最长真前后缀长度，失配时沿前缀表回退而不移动已确认文本；用有重叠前后缀的模式串手推。
4. **Trie/哈希题画状态**：Trie 区分“路径存在”和“单词结束”；滚动哈希写移出、乘基数、加入的顺序，并对哈希命中做精确比较。
5. **验算**：空串、重复匹配、重叠出现、分隔符连续、非 ASCII 输入和数值边界；匹配位置单位必须与输出契约一致。

常见陷阱：把 UTF-8 byte 下标称为字符下标；KMP 回退到 `j-1` 造成重复扫描；Trie 前缀当完整单词；滚动哈希相等就认定字符串相等；`isdigit(char)` 遇负 `char` 未转换。
...
只允许替换时长度必须相等，双指针同步扫描并统计不同位置，超过 1 即假，时间 `O(n)`。扩展为一次编辑时先保证短串长度不大于长串且长度差≤1；遇首个不同，等长则两边都前进，差 1 则只前进长串，第二次不同即假。不变量是指针前缀可用至多一次编辑匹配；空串、首尾差异和完全相同都要测。
...
若约定保留空字段，从左到右在逗号处分割，`A,,B,` 输出 `['A','','B','']`；末尾逗号产生最后空字段。扫描每个 byte 一次为 `O(n)`。它不是 CSV，因为 CSV 还允许引号内逗号、双引号转义、可能的换行字段和严格语法；例如 `"A,B",C` 不能按每个逗号直接切。
...
前缀表构造不变；扫描文本时一旦 `j==pattern.size()` 就返回 `i+1-j`，结束而不继续沿前缀表找重叠匹配；扫描完返回空值/`npos`。时间仍 `O(n+m)`，前缀表空间 `O(m)`，但结果从位置数组变为一个可选下标，额外输出空间从 `O(matches)` 降到 `O(1)`。
...
节点保存孩子、`is_word`，若允许重复还保存计数。删除先沿路径记录父节点；目标不是完整词则不变，清除词标志后从叶向上，只删除“无孩子且非词尾”的节点，不能删掉其他单词共用前缀。补全先走到前缀节点，再 DFS 按规定顺序收集，达到 k 立即停止。时间为前缀长度加实际访问节点数，边界含删除不存在、单词是另一词前缀和 `k=0`。

</details>

5. 给定多个模式，先用逐个 KMP 给基线，再研究 Aho–Corasick 解决了哪些重复工作。

<details><summary>参考答案</summary>

若模式总长 M、模式数 p、文本长 n，逐个 KMP 预处理 `O(M)`，但会扫描文本 p 次，约 `O(M+pn)`。Aho–Corasick 把模式共享前缀建成 Trie，并用失败链接在失配时复用后缀状态，一次扫描文本，复杂度通常写 `O(M+n+输出数)`（按固定/高效字符转移实现）。它减少重复扫描与共享前缀工作，不消除输出大量匹配的成本。
...
预计算 `H[i+1]=H[i]*B+s[i]` 与 `powB`，半开区间 `[l,r)` 哈希为 `H[r]-H[l]*powB[r-l]`（按无符号溢出或指定模数语义）。等长子串先比哈希，命中时若要求确定正确再逐字节比较。随机生成短串与区间，把哈希判断同逐字节 oracle 对比，覆盖空段、重复字符和边界。预处理 `O(n)`、查询哈希 `O(1)`；有限随机样本只会发现已遇到的碰撞，不能证明不存在数学上的不同串同哈希。
