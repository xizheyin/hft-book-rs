...
本章先把边界说清，再学习频次统计、分词与精确解析两组主线；KMP、Trie 和滚动哈希作为 P2 选读保留。每个母题都先写语言无关伪代码，再翻译成可独立运行的 C++20。
...
| 优先级 | 内容 | 学完应能做到什么 |
|---|---|---|
| P0 主线 | 字节/ASCII/UTF-8 边界、频次数组、线性扫描、tokenize | 系统会解析协议与日志；先定义“字符”和失败语义，避免边界与生命周期错误 |
| P2 算法选读 | KMP、Trie、滚动哈希 | 适合搜索/文本算法或明确的算法专项轮，不是普通 HFT/基础设施岗位的统一门槛 |
| 专项深入 | Unicode 规范化、分词库、后缀结构、Aho–Corasick | 需要专门语义和库支持；只有岗位明确需要文本处理时再进入 |
...
## 4. P2 选读：KMP 查找所有出现位置

KMP 属于 **P2 选读**。第一次刷字符串题时，先把普通扫描、频次和解析练稳；需要确定的线性最坏时间精确匹配，或面试明确要求手写字符串匹配时，再深入这一节。
...
1. **P0**：判断两个小写 ASCII 字符串是否至多相差一次替换；先写双指针，再考虑长度不同的插入/删除。
2. **P0**：编写逗号分词器，明确 `A,,B,` 的输出；再说明为什么它还不是完整 CSV 解析器。
3. **P2**：修改 KMP，只返回第一处匹配；比较接口和空间是否需要变化。
4. **P2**：为 Trie 增加安全删除和最多返回 `k` 个补全结果。
5. **P2**：给定多个模式，先用逐个 KMP 给基线，再研究 Aho–Corasick 解决了哪些重复工作；不要求第一次完成。
6. **P2**：把滚动哈希改成前缀哈希，回答任意等长子串比较；再写一个逐字节 oracle 做随机对拍，并解释为什么对拍不能证明永远无碰撞。
