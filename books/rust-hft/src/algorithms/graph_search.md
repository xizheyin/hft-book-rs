...
DFS、BFS、拓扑排序、并查集、Dijkstra 和回溯采用不同的扩展顺序，却都必须明确“哪些状态已经确定、哪些仍待处理”。

## 0. 先修回顾

图由顶点和边组成，边可以有方向与权重；路径、环、连通分量和 DAG 的严格定义见[图：表示关系、遍历网络](graph_foundations.md)。稀疏图通常用邻接表保存已有边，稠密小图可以使用邻接矩阵。分析复杂度时用 `V` 表示顶点数、`E` 表示边数，不能把建图空间漏掉。开始任何搜索前都要先说明点和边的含义、方向、权重，以及节点在入队还是出队时标记。
...
初始化 `dist=-1,parent=-1`，令 `dist[s]=0` 并在入队时标记；弹出 u 后，对每个未访问 v 设置 `dist[v]=dist[u]+1,parent[v]=u` 并入队。不变量是队列按非递减距离处理，首次入队已得到最少边数。终点不可达时 dist 为 -1；可达时从 t 沿 parent 到 s 再反转。时间 `O(V+E)`、空间 `O(V)`。用菱形图和不可达点验算 parent 链每步距离减 1。
...
对每个未染色分量做 BFS/DFS，起点染 0，每条边另一端应染 `color[u] xor 1`；若已染色邻居与 u 同色则返回假。伪代码核心是 `if color[v]==-1: color[v]=1-color[u]`，否则检查不同色。不变量是所有已检查边两端异色；时间 `O(V+E)`、空间 `O(V)`。三角形应失败，树和断开的偶环应通过；自环立即失败。
...
用 Kahn 算法维护零入度集合。每轮取点前若集合大小大于 1，当前至少有两个合法选择，顺序不唯一；取点、删除出边并把新零入度点加入。若最终输出少于 V 则有环、无拓扑序；否则返回顺序和 unique 标志。不变量是集合恰含尚未输出且入度为 0 的点。时间 `O(V+E)`；链唯一，两个互不依赖点不唯一。
...
把每格映射为并查集节点并保存 active。新增未激活格时 `count++`，再看四邻；每与一个已激活且不同根的邻居 union 成功，`count--`。重复新增不改变 count。不变量是 count 等于 active 格子的连通分量数；路径压缩+按秩合并后每次近摊还常数，n 次空间 `O(rows×cols)` 或用哈希只存激活格。用单格、相邻两格、桥接两个岛、重复添加验算。
...
Dijkstra 维护 `dist`、`parent` 和最小堆 `(distance,node)`；弹出记录若不等于当前 dist 就跳过，松弛 `dist[u]+w<dist[v]` 时更新 parent。目标的有效最小记录弹出后可停止。沿 parent 从 t 倒推到 s 再反转；dist 仍无穷则不可达。不变量是已有效弹出的点距离已最终确定，依赖所有边权非负。复杂度邻接表+堆为 `O((V+E)log V)`，空间 `O(V+E)`；加法前防溢出。
...
逐行回溯，每行尝试每列；用三个布尔集合记录已占列、主对角线 `row-col+n-1`、副对角线 `row+col`，任一占用就剪枝。选择后写入方案并标记，递归下一行，返回前撤销。不变量是前 row 行各放一皇后且互不攻击；row==n 时复制方案。最坏指数级、状态空间 `O(n)`（不计答案）。测试 n=1 有 1 解、n=2/3 无解、n=4 有 2 解，并检查每解行列数及对角线唯一。

</details>

## 10. 章末做题方法：按边权、状态和撤销需求选搜索

1. **读题定状态图**：一个状态包含哪些信息，转移是什么，目标条件是什么；显式图和迷宫/字符串变化等隐式图使用同一思路。
2. **选择搜索器**：无权最短步数用 BFS；非负权最短路用 Dijkstra；只判连通/合并用并查集；枚举组合且要撤销选择用回溯。
3. **画 frontier**：BFS 写每层队列，Dijkstra 写 `(dist,node)` 堆并跳过过期项，并查集画根，回溯画选择树和剪枝条件。
4. **逐步更新并记录不变量**：首次入队即得最短层数；Dijkstra 弹出的有效最小距离才确定；并查集只维护分组不维护路径；回溯退出前撤销本层修改。
5. **验算**：小图手算全部路径；不可达点保持无穷；并查集组件数变化正确；剪枝不能删除仍可能产生解的分支。

常见陷阱：带权图用普通 BFS；Dijkstra 接受负边；访问标记时机错误；并查集被误用于求最短路径；回溯忘记撤销或重复选择。
...
3. 拓扑排序输出不足 `V` 个点说明什么？
4. 并查集能回答什么，不能回答什么？
5. Dijkstra 的非负权前提用在证明的哪里？
6. 回溯中的“选择—递归—撤销”分别改变什么状态？
7. 一条剪枝为什么不会漏掉合法答案？
