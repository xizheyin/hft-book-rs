...
1. **读题锁定规模**：列出每个输入维度、值域和调用次数，区分最坏、平均、摊还以及预处理后单次查询。
2. **数执行次数**：顺序循环相加，嵌套且独立的循环相乘，规模每轮按常数比例缩小才是对数；递归写出参数怎样减小和每层额外工作。
3. **写不变量/归纳对象**：循环题说明每轮前后保持什么；递归题说明更小规模成立后怎样推出当前规模；贪心题准备交换论证。
4. **构造反例与基线**：先用小规模暴力实现作 oracle，再随机差分；专门测试边界和能击穿错误直觉的输入。
5. **验算资源**：别漏输出、排序、哈希退化、递归栈和临时数组；最终复杂度变量必须与题目输入对应。

常见陷阱：看到一层循环就写 `O(n)`；把哈希查找说成绝对 `O(1)`；只证明算法终止不证明结果正确；忽略摊还结论不代表每次操作都便宜。
