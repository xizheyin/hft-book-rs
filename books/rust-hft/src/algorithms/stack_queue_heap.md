...
栈、队列、双端队列和堆都在保存“暂时还要用的数据”，但取出顺序不同：

| 结构 | 取出规则 | C++20 常用类型 | 典型问题 |
|---|---|---|---|
| 栈 | 后进先出 | `std::stack`，或把 `vector` 当栈 | 括号、撤销、下一更大元素 |
| 队列 | 先进先出 | `std::queue` | BFS、按到达顺序处理事件 |
| 双端队列 | 两端都能进出 | `std::deque` | 单调队列、窗口候选 |
| 堆 | 每次取优先级最高者 | `std::priority_queue` | Top-K、多路合并、调度 |

“单调栈”和“单调队列”不是新的标准库容器，而是给普通栈或 `deque` 增加一个不变量：内部元素按值保持单调。这个不变量让我们安全丢掉以后永远不会成为答案的候选。

> **本章目标**：能根据取出顺序选择结构；能解释单调结构中元素为何可以永久弹出；能独立实现 Top-K 和多路合并；能正确使用 C++ `priority_queue` 的比较器。

学习范围分三层：栈/队列的取出顺序、堆、Top-K 和多路合并是 **P0**；单调栈与单调队列是 **P1**；完整表达式词法、优先级和求值器是 **P2**。表达式一节用于展示“栈怎样承载一个小语法”，不是 HFT/基础设施岗位的通用必背题。
...
`top()`、`front()` 和 `back()` 都要求容器非空。面试代码中应先检查 `empty()`，不能把未定义行为交给测试运气。

`std::priority_queue<int>` 默认是**大顶堆**，`top()` 返回最大元素。小顶堆可以写成：

```cpp,ignore
std::priority_queue<int, std::vector<int>, std::greater<int>> min_heap;
```

堆只保证堆顶最优，并不保证内部整体有序。依次弹出所有元素才能得到优先级顺序，每次弹出需要 `O(log n)`。

## 2. P1 母题：每个位置右侧的第一个更大值
...
- **P2**：若学习了表达式求值，栈在什么时候归约运算符，如何拒绝除零和溢出？
