...
1. **读题找“下一位”关系**：最近未匹配、右侧第一个更大、窗口极值、全局 Top-K 或多路当前最小，分别对应栈、单调队列、堆。
2. **定义容器不变量**：写清保存值还是下标、从底到顶/队头到队尾的单调方向、堆根代表最大还是最小。
3. **逐元素推演**：先删除已失效元素，再弹出破坏单调性的元素，再加入当前元素，最后在窗口形成后读答案；堆题写每次 push/pop 后候选集合。
4. **验算摊还次数**：单调结构中每个元素最多进出一次；多路合并的堆节点要携带来源与下一下标；表达式栈逐 token 检查操作数个数。
5. **测试边界**：重复值、窗口 1/等于数组长、`k=0`、空输入、负数与多位数字。

常见陷阱：窗口题只存值导致无法判断过期；相等元素弹出规则与题意不一致；Top-K 堆方向相反；多路合并忘记推进来源；表达式的一元负号当二元减法。
