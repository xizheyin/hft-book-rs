...
稳定任务排序、归并、三路分区和 Quickselect 分别训练怎样定义平局、合并有序区间、维护分区边界，以及只继续搜索目标排名所在的一侧。

## 1. 先修回顾

完整排序确定所有元素的顺序，选择问题只要求第 `k` 个或前 `k` 个，因此不一定需要完成全部排序。稳定排序保留比较器等价元素的输入相对顺序；比较器必须形成严格弱序。`sort`、`stable_sort`、`nth_element`、`partial_sort`、堆排序以及各类经典排序的原理和复杂度统一见[排序基础：插入、交换、选择、归并与外部排序](sorting_foundations.md)。
...
## 13. 章末做题方法：先判断是否真的需要全排序

1. **读题确定输出强度**：只要第 `k` 小、任意最小 K 个、排好序的最小 K 个，还是完整有序；四种要求对应不同工作量。
2. **画 partition 或堆**：Quickselect 标 `<pivot`、`=pivot`、`>pivot` 三段及 `k` 落点；Top-K 堆始终标根代表当前候选中的哪个极值。
3. **逐轮缩小问题**：Quickselect 只进入包含目标排名的一侧；堆题每读一个元素决定忽略、插入或替换根。
4. **验算多重集合与排名**：重复值必须计入排名；结果数量恰为 `k`；若题目要求有序，再单独排序输出部分。

常见陷阱：第 `k` 小与下标 `k` 的 0/1 基差异；Quickselect 递归两侧退化成排序；最大堆/最小堆方向用反；`nth_element` 后误以为两侧各自有序；`k=0` 或 `k>n` 未定义。
