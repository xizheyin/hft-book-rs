...
本章从稳定性和比较器开始，完整实现归并排序、三路快速排序、Quickselect，并把它们与 `sort`、`stable_sort`、`nth_element`、`partial_sort` 放到同一张选择地图里。

> **本章目标**：理解稳定与不稳定的准确含义；能证明归并和三路分区的不变量；能写随机化 Quickselect；不会把 `nth_element` 误认为完整排序；能写满足严格弱序的 C++ 比较器。
