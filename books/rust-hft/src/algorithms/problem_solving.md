...
1. **重述输入输出**：用自己的话写对象、约束、重复/顺序语义和一个最小例子，确认没有把业务描述中的条件漏掉。
2. **先写基线**：给出必然正确的枚举或直接模拟，算清它为何超时；再从重复工作、单调性或可维护状态中寻找优化。
3. **写不变量后编码**：每个循环/数据结构写一句一直成立的性质，变量名与性质对应；先处理空输入和边界，再写主循环。
4. **手推与差分**：按代码真实顺序推一个含重复和边界的例子；可行时让优化版与基线随机对拍。
5. **最后验算**：检查时间、额外空间、整数范围、输入输出格式，以及最坏输入是否触发退化。

常见陷阱：没读完约束就选模板；只复述代码不说明为何正确；优化时删除唯一的正确性基线；测试都来自同一种样例；复杂度不包含排序或输出。
