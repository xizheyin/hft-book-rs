...
1. **读题列对象与操作频率**：元素是否有顺序/层级/关系，查找、插入、删除、最值和遍历各有多少次，数据是否持续变化。
2. **先定逻辑结构，再定存储结构**：线性、树、图是对象关系；连续、链式、索引是内存表示。不要用“数组”和“线性表”互相代替。
3. **画一次操作**：连续表画移动区间，链式结构画指针变化，树/图画访问路径；据实际访问节点数推时间和额外空间。
4. **验算 ADT 契约**：操作前置条件、返回值、空结构行为和更新后不变量都明确；摊还分析要给一串操作而非只看最贵一次。

常见陷阱：只凭单次复杂度选结构；忽略缓存局部性与内存开销；把递归辅助栈算成零空间；把接口语义和某一种实现绑定。
...
数据对象是“某校全部学生记录”的集合；一名学生记录是一个数据元素；学号、姓名、专业、成绩是数据项；能唯一标识学生的学号可作为主关键字。验算：若姓名会重复，就不能单独充当唯一关键字。
...
逻辑结构描述元素间的抽象关系，线性表只要求除两端外每个元素有唯一前驱和后继；存储结构描述这些关系怎样落在内存。连续数组可用相邻地址表示次序，链表可用 `next` 指针表示同一次序，所以一种逻辑结构可有多种表示。
...
集合只关心成员资格；线性结构形成一条有先后次序的序列；树除根外每个结点有唯一父结点并形成层级；图允许任意顶点间有边，可有环、多条路径和多个连通分量。先画关系再选存储，避免把“数组”误当逻辑类型。
...
顺序存储用地址位置隐含次序；链式存储为每个节点保存一个或多个链接；索引存储另建“键→位置/块”的索引项；散列存储借哈希函数把键映射到桶并保存冲突处理结构。空间不能只算数据本身，还要算空容量、指针、索引和桶。
...
ADT（抽象数据类型）规定值域、操作及语义，不规定内部表示。栈要求 `push/pop/top` 遵守后进先出，可用动态数组、链表或其他结构实现。验算实现时只看操作序列返回值是否符合 LIFO，不应让客户端依赖内部下标。
...
前置条件：`0≤i≤size` 且有足够资源；后置条件：新长度为旧长度加 1，新位置 `i` 为 `x`，旧 `[0,i)` 不变，旧 `[i,size)` 整体后移一位。数组实现的不变量可写 `0≤size≤capacity` 且 `[0,size)` 都是有效元素；空表和尾插 `i=size` 也必须成立。
...
只有已知插入位置的节点或前驱，且不把分配成本另行放大时，改常数条指针才是 `O(1)`。若先从头按值查找，最坏访问 `n` 个节点为 `O(n)`，随后接链 `O(1)`，总成本仍为 `O(n)`；空表和尾部还要维护 head/tail。
...
连续执行成本相加：`cn+dn=O(n)`；独立嵌套时外层每次执行 `n` 次内层，总次数 `n×n=n²`，为 `O(n²)`。若内层界限随外层变化，应写求和式，例如 `1+...+n=Θ(n²)`，不能只看有几个 `for`。
...
平均复杂度对输入或随机选择的概率分布取期望；摊还复杂度对任意一串操作分摊总成本，不需要假设输入分布。倍增动态数组偶尔扩容搬移 `O(n)`，但连续 `n` 次尾插总成本 `O(n)`，因此每次摊还 `O(1)`。
...
倍增时搬移量构成 `1+2+4+...<2n`，几何级数为 `O(n)`，加上 `n` 次写入仍是 `O(n)`。若每次满后只增加一格，第 `i` 次扩容要搬约 `i` 个元素，总搬移 `1+...+(n-1)=Θ(n²)`，尾插摊还退化为 `Θ(n)`。
...
仅局部状态就是 `32n` byte，渐近为 `O(n)`；真实栈帧还包含返回地址、保存寄存器、对齐和其他临时量，常数更大。深度过大会栈溢出，且编译器不保证一般递归一定尾调用优化；应测试最大输入或改显式栈。
...
基线是哈希表 `id→记录/节点` 提供平均 `O(1)` 等值查询，再用按注册时间有序的树或列表提供遍历。插入、删除和时间修改必须原子地更新两份结构；不变量是两份索引包含同一组用户且都指向同一版本记录。若注册时间只追加不改，数组/链表可更简单。
...
不一定。还要看 `n` 是否很小、哈希计算和键比较成本、负载因子/冲突、连续数组的缓存局部性、内存开销、最坏时延要求以及是否已按批次顺序扫描。比较应在真实键和规模上测量；复杂度只描述增长趋势。
...
客户端可能保存节点地址、依赖地址稳定或直接修改链接；换成数组后扩容会搬移元素，旧地址失效，数组也没有相同节点布局。解决方向是暴露值、稳定句柄或抽象迭代器，并明确失效规则；不变量和复杂度契约也要重新核对。
