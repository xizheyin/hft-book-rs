...
> **本章定位**：这是 DP 专项补强页，不是 P0 主线。P1 读者从背包、零钱或网格中按缺口选择两题；最长递增子序列的 `O(n log n)` 优化和区间 DP 属于 P2。面试准备时间有限时，能把一个基础 DP 的“状态为何足够”讲清，比看完五份代码更重要。
...
- **P2**：若学习了 LIS，`tails` 为什么保持有序，它为什么不一定是一条真实 LIS？
- **P2**：若学习了区间 DP，为什么按长度计算，最后一次操作怎样导出切分转移？
