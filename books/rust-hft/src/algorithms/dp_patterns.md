...
本章用五道母题把这些问题串起来。不要先背代码；先把状态定义写在纸上，再检查循环顺序是否真的符合定义。

本章用背包、零钱、网格、最长递增子序列和区间合并展示不同依赖形状。每一节都从状态为何足够开始，再决定初始化、遍历方向、空间压缩和答案恢复。
...
1. **读题找重叠子问题**：答案由哪些更小决策组成，是否需要记住位置、容量、上一步选择或区间端点；先写一句 `dp[state]` 的精确定义。
2. **从最后一步推转移**：枚举最后选择来自哪个前驱，写出合法条件、取 min/max/计数，并明确不可达状态。
3. **画依赖箭头定顺序**：0/1 背包容量倒序，完全背包常正序；二维/区间 DP 按依赖的长度和方向遍历，不能先压缩再猜顺序。
4. **填小表**：用 3～5 个元素逐格计算，记录前驱；需要恢复答案时另存选择或从相等关系反推。
5. **验算**：与小规模枚举比较；初始状态只开放真正可达项；答案位置和空输入语义符合题目。

常见陷阱：状态定义含糊；把 0 当不可达导致合法零值冲突；同一轮错误复用刚更新状态；LIS 严格/非严格比较写反；区间长度和分割点边界少一格。
...
- LIS 中的 `tails` 为什么保持有序，它为什么不一定是一条真实 LIS？
