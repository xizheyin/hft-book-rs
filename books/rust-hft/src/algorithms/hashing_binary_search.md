...
许多算法题都在问“怎样更快地找到东西”。常见答案并不只有一种：

- 哈希表用额外空间换取平均意义下的快速查找；
- 排序先建立全局顺序，让后续扫描或搜索更简单；
- 二分搜索利用单调性，每次排除一半候选范围。

这三类方法经常组合出现。关键不是记住某道题用了什么容器，而是先问：**我需要保留原顺序吗？数据是否已经有序？判断条件是否单调？最坏时间是否重要？**

本章属于 **P0 主线**。哈希索引对应订单 ID、请求 ID 和状态表，区间对应任务占用和时间段，二分对应有序数据与“某个阈值是否可行”；岗位真正想看的是你能否从需求推出结构，并说出它牺牲了什么。

> **本章目标**：会使用集合与映射表达“见过什么”；会通过排序合并区间；能从 `first true` 不变量独立写出二分搜索；能说明平均复杂度和最坏复杂度的区别。
...
| 工具 | 是否有序 | 典型查找成本 | 重要边界 |
|---|---|---|---|
| `std::unordered_set<T>` | 否 | 平均 `O(1)`，最坏 `O(n)` | 依赖哈希质量，会扩容 |
| `std::unordered_map<K, V>` | 否 | 平均 `O(1)`，最坏 `O(n)` | 不保证迭代顺序 |
| `std::set<T>` / `std::map<K, V>` | 按比较器有序 | `O(log n)` | 通常是节点结构，局部性较弱 |
| 有序 `std::vector<T>` | 有序 | 二分查找 `O(log n)` | 中间插入/删除通常 `O(n)` |

“哈希表查找是 `O(1)`”是一句省略了前提的话。更准确的表达是：在哈希分布合理、负载因子受控时，查找的**平均或期望**成本为常数；大量碰撞时最坏可退化到线性。

排序也是一种投资。`std::sort` 对 `n` 个随机访问元素的比较次数是 `O(n log n)` 量级。排序以后，去重、区间合并、双指针和二分搜索都可能更容易。若只查一次，排序未必划算；若之后要查很多次，它可能很值得。
