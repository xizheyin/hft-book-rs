...
本章不是毕业必做清单。卷 C 用于 P0/P1 综合迁移；卷 D 中 D1、D3 是 P0 窗口题，D2 要求线性字符串匹配，属于 **P2 字符串专项**。普通 HFT/基础设施岗位没有这类信号时，可以跳过 D2，换成一题协议字段解析；评分时按两道题重新折算，不要为了“完成整卷”临时背 KMP。
...
| D2（P2） | KMP + 行读取 | 重叠匹配、空格、空模式政策；非字符串岗位可替换 |
| D3 | 窗口 + 哈希频次 | 离开时不删零频键、非法 `k` |

若同一模式连续两次失分，回到[覆盖矩阵](coverage_matrix.md)把对应 C/D 格清空，完成第 2、7 天复测后再打勾。
