...
本章不要求你背模板。我们会从暴力解开始，找出它重复做的工作，再一步步得到双指针、滑动窗口和前缀和。

这些模式被系统/HFT 岗位反复使用，不是因为面试官偏爱谜题，而是因为缓冲区扫描、时间窗口、滚动指标和区间聚合都在做同类工作。本章属于 **P0 主线**：重点是知道每个指针或累计量代表什么，而不是背某一题的代码。

> **本章目标**：能根据约束识别三种线性结构；能说清指针或窗口的不变量；能写出边界明确、可测试的 C++20 实现。
...
- 双指针移动一端时，为什么不会排除正确答案？
- 嵌套 `while` 的滑动窗口为什么可能仍是 `O(n)`？
- 哪类条件破坏滑动窗口需要的单调性？
- 为什么前缀数组通常开成 `n + 1`？
- `std::string::size()` 为什么不一定等于人眼字符数？
- 哪些测试能暴露下标、空输入和整数溢出问题？
