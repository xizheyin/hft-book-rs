...
| 优先级 | 内容 | 为什么 |
|---|---|---|
| 必须掌握 | 无符号掩码、设置/清除/查询、异或、移位边界 | 常见，而且 C++ 写错后可能不是“答案差一点”，而是未定义行为 |
| 建议掌握 | `popcount`、子集掩码、`gcd`、安全 `lcm`、快速幂 | 能覆盖一批常见组合与数值题 |
| 进阶选读 | 大整数、Montgomery reduction、专用 SIMD 位操作 | 依赖岗位和性能目标，不应阻塞基础训练 |

第一次阅读时，先完成前四个母题。模乘的便携实现可以在第二遍学习；先理解它为什么安全，再考虑更快的平台专用写法。
