...
本章不要求你背“位运算技巧大全”。我们先建立一个安全的最小模型，再学习几种能从定义推导出来的常用模式：标志位、异或抵消、子集枚举、最大公约数、安全最小公倍数和模快速幂。

> **事实边界**：这些是通用算法训练题，不是 DeepSeek、九坤或任何公司的官方真题。不同岗位是否考查位运算、数论或特定语言标准，应以当次岗位说明和笔试通知为准。

## 0. 先确定学习优先级

| 优先级 | 内容 | 为什么 |
|---|---|---|
| P1 岗位常用 | 无符号掩码、设置/清除/查询、异或、`popcount`、移位边界 | 标志位、协议字段和整数安全与系统代码直接相关 |
| P2 算法选读 | 子集掩码、`gcd`、安全 `lcm`、模快速幂 | 能覆盖组合与数值题，但不是普通 HFT/基础设施面试的统一门槛 |
| 专项深入 | 大整数、Montgomery reduction、专用 SIMD 位操作 | 只有密码学、数值或特定性能岗位出现明确信号时再学 |

第一次阅读只需完成位模型、标志位、移位规则和异或示例。子集枚举之后的数值题都是 P2；岗位没有相关要求时可以跳过，不影响主线验收。
...
1. **P1**：给定两个只出现一次的值、其余值出现两次，返回这两个值并证明分组不会把它们放到同一组。
2. **P1**：用 `mask & (mask - 1)` 每次清除最低的 1，手写一个 `popcount`，并与 `std::popcount` 随机对拍。
3. **P2**：只枚举某个掩码的所有非空子掩码；解释为什么循环会终止。
4. **P2**：计算 `base^exponent`，若完整结果超出 `uint64_t` 就返回空；不要借助浮点数猜测。
5. **P2**：把 `checked_lcm_u64` 扩展到一组非负整数；中途出现溢出就停止。
