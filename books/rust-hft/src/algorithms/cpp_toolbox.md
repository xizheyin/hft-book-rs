...
正确算法如果在比较器、整数类型、空容器或迭代器上写错，仍然会得到错误程序。本章的 Top-K 示例专门展示怎样把伪代码翻译成可靠 C++；结构不变量和更多变体见[栈、队列、双端队列与堆](stack_queue_heap.md)。
...
1. **读题先定元素身份**：是否允许重复、需要稳定顺序、保存值还是下标、比较器是升序还是“优先级更高”。
2. **写操作契约**：为 `vector`、哈希表、堆或有序容器列出插入、删除、查找和迭代器失效条件，再落成类型定义。
3. **手推三步容器状态**：特别是 `priority_queue` 比较器、`lower_bound` 返回位置、哈希计数增减和 erase 后迭代器。
4. **编译并差分**：用最小 C++20 程序开启警告，和排序/暴力基线对比随机输入；同时用 sanitizer 检查越界与生命周期。
5. **验算输出契约**：确认顺序、重复项、空结果、整数宽度与换行严格符合题目。

常见陷阱：比较器不满足严格弱序；保存 `vector` 元素地址后扩容；把 `reserve` 当改变 size；读取不存在的哈希键意外插入；堆中元素更新后仍认为会自动重排。
