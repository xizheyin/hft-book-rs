...
本章从固定窗口和序号检测开始，再讲在线均值/方差、事件时间、在线中位数、近似摘要和水库抽样。所有实现都要用不变量说明正确性，并尽量保留朴素算法作为随机对拍基线。
...
## 9. 章末做题方法：逐事件维护而不是重算全量

1. **读题定义流语义**：事件是按到达时间还是事件时间，窗口按数量还是时间，是否可能乱序、重复、缺口和撤回。
2. **写最小状态**：滚动和保存队列与当前和；在线方差保存计数、均值和平方偏差；序号检测保存期望值；在线中位数保存两个堆。
3. **逐事件推演状态转移**：正常、重复、旧事件、跳号、窗口淘汰和水位线推进分别写一次，确保每个分支只更新应更新的状态。
4. **验算守恒/不变量**：窗口长度不超限且和等于窗口元素和；两堆大小差不超过 1 且所有下半元素不大于上半；序号不因旧包倒退。
5. **与离线基线对比**：对小流保存全量并重算结果，比较每一步而不只比较最终值；浮点统计使用容差和极端数量级输入。

常见陷阱：事件时间与到达时间混用；重复事件再次累计；均值/方差用不稳定的“大数相减”；水位线当作绝不再来迟到数据的事实；近似结构不说明误差与内存界。

## 10. 本章自测

1. 批处理、全历史流式统计和固定窗口统计有什么区别？

<details><summary>验收参考</summary>

批处理能看到有限完整输入后统一计算；全历史流式统计逐项更新且状态代表从开始到现在；固定窗口还要在新项加入时撤销过期项贡献。画同一序列 `1,2,3,4`、窗口 2：最终全历史和 10，窗口和 7，可直接验出状态语义不同。

</details>

2. 怎样为滚动和写朴素版、优化版和随机对拍？

<details><summary>验收参考</summary>

朴素版对每个窗口重新求和 `O(nk)`；优化版先求首窗，右移时 `sum+=a[r]-a[r-k]`，总 `O(n)`，不变量是 sum 等于当前窗口逐项和。随机生成小数组及所有 `k=0..n+1`，逐个比较完整输出；另固定空、负数、极值防溢出样例。

</details>

3. Welford 在线方差中的 `count`、`mean`、`m2` 分别是什么？

<details><summary>验收参考</summary>

`count` 是已处理样本数，`mean` 是当前均值，`m2` 是相对当前均值的平方偏差和。加入 x 时先 `delta=x-mean`，增加 count，更新 `mean+=delta/count`，再 `m2+=delta*(x-new_mean)`；总体方差 `m2/count`，样本方差 `m2/(count-1)`。用 `[1,2,3]` 验算 mean=2、m2=2。

</details>

4. 简化序号监控器通常没有处理哪些生产语义？

<details><summary>验收参考</summary>

至少包括重复/旧包、乱序缓冲、缺口重传、快照切换、会话或交易日重置、序号回绕、缓冲上限、超时与不可恢复状态。用 `10,12,11` 推演：不能一见 12 就永久把 expected 跳到 13，否则 11 无法按序应用。

</details>

5. 在线中位数的两个堆需要保持哪两个不变量？

<details><summary>验收参考</summary>

顺序不变量：下半区最大堆的每个值都不大于上半区最小堆的每个值；平衡不变量：两堆大小差不超过 1（常约定下半多 1）。加入后先按边界放入再搬堆顶恢复平衡。奇数时中位数取较大堆顶，偶数按契约取两顶平均并防加法溢出；每次 `O(log n)`。

</details>

6. 为什么摊还 `O(1)` 不代表每一次操作都严格只做一步？

<details><summary>验收参考</summary>

动态数组某次扩容会搬 `O(n)` 个元素，单调队列某次也可能连续弹很多项；但连续 n 次操作的总工作仍 `O(n)`，平均分摊上界为常数。证明要按整个操作序列计数，例如每个下标至多入队、出队各一次，而不是声称最坏单次为常数。

</details>

7. 面对无限流，必须确认哪些内存、乱序、过期和近似语义？

<details><summary>验收参考</summary>

明确状态条目/字节上限和满载政策；使用事件时间还是到达时间、允许多晚；过期由 TTL、水位线还是数量窗口决定；精确还是允许误差，误差界和置信度是什么。再模拟永不结束、热点键、长期缺口和迟到事件，确认状态不会无限增长且输出符合声明语义。
