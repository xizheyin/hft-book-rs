...
> 本章目标：理解批处理与流处理的区别；会设计固定窗口、在线统计、序号检测和在线中位数；能用不变量说明增量算法为什么正确，并用朴素算法做随机对拍。
...
| 长期近似去重 | Bloom filter | 可能误判“见过”，普通版本不方便删除 |
| 近似分位数 | KLL、t-digest 等摘要 | 存在可描述的误差 |
| 从未知长度流均匀抽样 | reservoir sampling | 只保留固定数量样本，不保存全部数据 |
...
- [ ] 能为滚动和写出朴素版、优化版和随机对拍。
- [ ] 能说清 Welford 状态中 `count`、`mean`、`m2` 的含义。
