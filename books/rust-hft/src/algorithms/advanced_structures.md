...
## 9. 章末做题方法：先辨认“更新—查询—图性质”

1. **读题列操作**：写出数据是否在线变化、更新是点还是区间、查询是前缀/区间/连通/最短路，以及值是否可逆或可合并。
2. **按约束选结构**：前缀可差分且单点更新优先 Fenwick；需要任意结合信息或区间更新考虑线段树；边排序后连接组件考虑 Kruskal；有向环缩点考虑 SCC；含负边再判断 Bellman-Ford。
3. **画最小样例推演**：Fenwick 写每个下标覆盖范围，线段树标节点区间，Kruskal 标并查集合并，SCC 标两次 DFS 顺序，最短路表列每轮松弛。
4. **验算不变量**：树节点聚合值等于子区间合并；MST 恰有 `n-1` 条边且无环；缩点图无环；负环判断只在从源点可达的边上成立。

常见陷阱：下标从 0 直接套 Fenwick 的 `lowbit`；线段树区间端点混乱；Kruskal 忘记图可能不连通；把 SCC 当无向连通分量；有负边仍使用 Dijkstra。
