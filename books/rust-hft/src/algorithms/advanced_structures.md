...
本章整体是 **P2 按岗位选学**，不是通用 HFT/基础设施面试的毕业要求。第一次准备时，应先让数组、哈希、树图遍历、堆、流式窗口和基础系统知识稳定，再根据岗位说明或模拟中的真实失分进入本章。
...
> **本章目标**：能用坐标压缩把稀疏键映射到稠密下标；能实现 Fenwick Tree 和迭代线段树；能说清 MST、SCC 与负边最短路的选择边界；不会把算法名称当作适用性证明。

只有出现下列触发信号才建议展开对应部分：

| 触发信号 | 选读内容 | 没有信号时 |
|---|---|---|
| 岗位/笔试明确出现动态区间查询 | 坐标压缩、Fenwick Tree、Segment Tree | 跳过，不影响 P0/P1 验收 |
| 图算法专项轮或网络优化类题目 | MST、SCC | 只需知道它们与最短路不是同一问题 |
| 明确出现负边、负环或差分约束 | Bellman-Ford 与选择边界 | 保持“Dijkstra 不能处理负边”这一条即可 |
...
Tarjan 算法只需一轮 DFS 且不需要显式转置图，但 `discovery/low-link/on-stack` 不变量更精细。先把 Kosaraju 讲清，再按岗位需要学习 Tarjan。
...
如果岗位没有要求，不必为了“高级”二字抢先背这些实现；但一旦选择使用，就必须同时说清不变量和适用边界。
