...
区间调度用来练贪心交换证明，一维题用来练状态定义，二维编辑距离展示完整 DP 与答案恢复。更多背包、零钱、网格、最长递增子序列和区间 DP 见[动态规划核心模式](dp_patterns.md)。
...
按结束时间排序，二分求 `previous[i]`：第 i 个区间前最后一个结束≤其开始的下标。定义 `dp[i]` 为前 i 个区间最大收益，转移 `max(dp[i-1],profit[i-1]+dp[previous[i-1]+1])`。恢复时比较两来源，选中则跳到 previous。状态不变量是 dp 已覆盖前 i 项全部合法组合；预处理+DP `O(n log n)`、空间 `O(n)`。测空、相接区间、负收益是否允许和并列策略。
...
`dp[a]` 为组成金额 a 的最少枚数，`dp[0]=0`，其余为无穷；对每个 a 尝试最后一枚 coin，若 `coin≤a` 且前态可达，用 `dp[a-coin]+1` 更新并记录 chosen[a]。从 target 反复减 chosen 恢复。不变量是计算 a 时所有更小金额已最优；时间 `O(amount×coins)`、空间 `O(amount)`。拒绝非正硬币，目标不可达返回空，检查加 1 不从无穷溢出。
...
二维 `dp[i][c]` 表示前 i 件、容量 c 的最大价值：不选取 `dp[i-1][c]`，能装时选 `value[i-1]+dp[i-1][c-weight]`。压一维后 c 必须从大到小，确保本轮读取的仍是上一轮，不能重复同一物品。不变量是处理完 i 件后数组恰代表这些物品的最优值。时间 `O(nC)`、空间 `O(C)`；容量 0、重量 0 的政策和总价值溢出需声明。
...
`dp[r][c]` 为到该格最小和，只能从上或左：`grid[r][c]+min(dp[r-1][c],dp[r][c-1])`，首格、首行、首列按唯一前驱初始化；同时记录 parent。终点沿 parent 回到起点再反转。不变量是按行/列计算时两个依赖均已完成。时间/空间 `O(RC)`；空网格、单行列、负值（无环时仍可算）和加法溢出需测试。
...
二次 DP 定义 `dp[i]` 为以 i 结尾的严格递增子序列最大长度，枚举 `j<i,a[j]<a[i]` 转移。优化维护 `tails[len]`：长度 `len+1` 的递增子序列可取得的最小结尾；对 x 用 `lower_bound` 替换首个 `≥x`，若不存在则追加。tails 始终递增但不一定是最终 LIS；长度正确，恢复需保存元素下标与前驱。复杂度 `O(n log n)`、空间 `O(n)`，重复值验证严格/非严格边界。
...
维护旧状态：`buy1=max(buy1,-p)`，`sell1=max(sell1,buy1_old+p)`，`buy2=max(buy2,sell1_old-p)`，`sell2=max(sell2,buy2_old+p)`；buy 表示持有后的最大现金，sell 表示完成相应次数卖出后的最大现金。用旧副本避免同日更新语义歧义。不变量是处理到当天后各状态是该约束下最优。时间 `O(n)`、空间 `O(1)`；空/下降序列返回 0，金额加减防溢出。

</details>

## 11. 章末做题方法：先证明局部选择能否锁定

1. **读题列决策序列**：每一步选什么、选择后剩余问题是否仍与原问题同形、是否存在容量/历史状态依赖。
2. **尝试贪心证明**：写出候选规则，用交换论证说明任一最优解可把第一步换成贪心选择而不变差；证明失败或能构造反例时转 DP。
3. **DP 写状态与最后一步**：明确 `dp` 含义、初值、不可达值、转移来源和遍历方向；二维依赖先画箭头再压缩空间。
4. **用小反例验算**：为贪心专找“眼前最好却堵住后续”的输入；为 DP 与暴力枚举对比，并恢复一条实际方案检查。

常见陷阱：用几个样例正确代替贪心证明；把排序后扫描一律叫贪心；滚动数组覆盖尚未使用的旧状态；编辑距离漏掉插入/删除/替换之一。
