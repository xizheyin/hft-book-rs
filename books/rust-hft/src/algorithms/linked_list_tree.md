...
链表和树看起来像两类题，实质上都在考同一件事：**能否在指针不断变化时，仍然说清楚每个对象在哪里、下一步会访问谁**。

本章不要求你背下一百段代码。每个母题都按同一条路线展开：先用白话说题意，再看最直接的办法，找到不变量，写伪代码，证明它为什么对，最后才落到 C++20。面试时也可以沿着这条路线表达。

> 本章题目是根据通用岗位能力设计的训练题，不是任何公司的真题或题库。

本章的面试价值在于指针安全和递归边界，而不是收集树题名称。**P0** 是反转/合并/判环以及一种深度优先和一种层序遍历；**P1** 是验证二叉搜索树（Binary Search Tree，BST）；完整列出四种遍历和最近公共祖先（Lowest Common Ancestor，LCA）属于补强内容。岗位没有算法专项要求时，P0 能闭卷写对后就可以回到所有权、缓存和并发主线。
...
1. 它可以是 `nullptr` 吗？
2. 它指向的对象由谁拥有，什么时候销毁？
3. 修改 `next`、`left` 或 `right` 后，还有没有办法找到原来的后继？

本章的链表代码使用裸指针表示**不拥有对象的连接关系**。测试程序用 `std::unique_ptr` 容器拥有节点，因此算法不能 `delete` 节点，也不能在所有者销毁后继续使用返回的指针。这与面试平台常见的函数签名一致，同时把所有权边界说清楚了。

```cpp,ignore
struct ListNode {
    int value{};
    ListNode* next{}; // 只表示“下一个是谁”，不表示拥有它
};
```

真实项目还可能让 `std::unique_ptr<ListNode>` 直接拥有下一个节点。那样所有权更明显，但反转和合并时必须移动智能指针，题目的重点会从算法转向所有权操作。面试前先确认题目给出的节点定义，不要擅自混用两套模型。
...
- 前序：根、左、右；适合复制结构或输出前缀表达式；
- 中序：左、根、右；二叉搜索树的中序结果应严格递增；
- 后序：左、右、根；适合先处理子树再汇总；
- 层序：从上到下逐层访问，本质是 BFS。

下面用“非递归中序遍历”作为母题。掌握栈里保存什么之后，前序和后序就不再只是背模板。
