...
在原头前放哨兵 dummy，`fast` 先走 k 步；若不足 k 步则输入非法。然后让 `fast,slow` 同步走到 `fast->next==nullptr`，此时 slow 是待删节点前驱，执行绕链。间距不变量是 fast 始终领先 slow k 个节点。时间 `O(n)`、空间 `O(1)`；k≤0、空链、k=n（删头）、k=1（删尾）都要测。
...
`slow=head,fast=head`，循环条件 `fast && fast->next`，每轮 slow 走 1、fast 走 2；结束时 slow 为中点，偶数长度落在后半第一个。以 `[1,2,3,4]` 推演：slow 为 3；`[1,2,3]` 为 2。时间 `O(n)`、空间 `O(1)`；空链返回空，题设无环，否则循环可能不终止。
...
队列先放根。每轮先记 `level_size=queue.size()`，恰弹出这些节点收集为一层，并把它们的孩子入队；新孩子留给下一轮。不变量是每轮开始时队列前 `level_size` 项恰为同一深度。时间 `O(n)`，空间为最大宽度 `O(w)`；空树返回空，不能边弹边用不断增长的 queue.size() 作本层界限。
...
用显式栈做中序遍历：不断压左链，弹出一个节点就把计数加 1，等于 k 时返回，再转右子树。BST 的中序按非降序，因此第 k 次弹出就是答案。不变量是栈保存尚未访问自身的左链祖先；时间最坏 `O(n)`、空间 `O(h)`。k≤0、k>节点数返回空/错误，重复键按树的约定计数。
...
后序函数返回高度或失败哨兵：空树 0；递归取左右高度，任一失败或差绝对值>1 就返回失败，否则返回 `1+max(left,right)`。不变量是返回非失败值时，该子树已平衡且值为真实高度。每节点一次，时间 `O(n)`、递归空间 `O(h)`；退化链应失败。不要对每节点另调用求高，否则会 `O(n²)`。

</details>

## 12. 章末做题方法：指针图与递归返回值

1. **链表题先画身份**：节点按地址区分而不是按值；画 `prev/current/next` 每一步，改链前先保存下一节点。
2. **树题定义递归函数**：一句话写清参数代表哪棵子树、返回值是什么、空节点返回什么；再按“左、右、合并”推演。
3. **遍历题画调用栈或栈帧**：标节点何时第一次到达、何时输出、何时回退；迭代写法同时记录显式栈内容。
4. **BST/LCA 题写不变量**：BST 使用祖先传下来的上下界；普通二叉树 LCA 看左右子树是否分别找到目标，不能偷用 BST 大小关系。
5. **验算**：空树、单节点、退化链、重复值规则、目标不存在；访问节点数与输出序列长度匹配。

常见陷阱：节点值相同就认为是同一节点；反转链表后丢失剩余部分；只与父节点比较来验证 BST；递归空间漏算；LCA 默认两个目标必然存在。
