...
## 16. 章末做题方法：用操作序列检查栈与队列

1. **读题写抽象操作**：只用 push/pop/top，还是 enqueue/dequeue/front，是否要求循环容量、最小值或双端操作；不要先被底层数组/链表带走。
2. **画容器状态**：栈标栈顶，队列标队头队尾；循环队列统一空/满判定，并写清数组中一个槽位是否留空。
3. **逐操作推演**：使用包含空结构、装满、绕回、出空再入队的序列；两个栈模拟队列时只在输出栈空时搬运。
4. **验算**：逻辑元素数与 `size` 一致，返回顺序符合 LIFO/FIFO，所有下标始终落在容量范围内，每个元素摊还搬移次数有界。

常见陷阱：空和满使用同一 `head==tail` 却没有额外状态；取模前下标负数；先移动指针再读取错误槽位；链式队列删完最后节点后忘记同步 tail。
...
连续数组中 `a[i]` 地址可由 `base+i×element_size` 一次算出，故随机访问 `O(1)`。在位置 `i` 插入要保持原顺序，需把 `[i,n)` 的 `n-i` 个元素右移，最坏 `i=0` 时移动 `n` 个，为 `O(n)`；尾插在容量足够时才是 `O(1)`。
...
设矩阵有 `rows` 行、元素大小 `w`，使用 0 基下标。列 `0..j-1` 各有 `rows` 个元素，列 `j` 内先有 `i` 个元素，所以线性偏移为 `j×rows+i`，地址为 `base+(j×rows+i)×w`。验算 `A[0][0]=base`，同列相邻行相差 `w`。
...
下三角含对角线共 `5×6/2=15` 项。按行、0 基压缩时，第 `i` 行前有 `i(i+1)/2` 项，故 `A[4][2]` 下标为 `4×5/2+2=12`，即第 13 个槽。验算第 4 行占下标 `10..14`，列 2 对应 12。
...
`values` 保存非零值，`col_idx` 保存每个值的列号，`row_ptr` 长度为 `rows+1`，指出每行在前两数组中的半开区间。第 `r` 行枚举 `k∈[row_ptr[r],row_ptr[r+1])`，元素是 `(r,col_idx[k],values[k])`；验算 `row_ptr[rows]=nnz`。
...
函数调用严格后进先出：最新调用必须先返回，所以栈帧保存返回地址、参数和局部状态最自然。递归深度 `d` 通常使用 `O(d)` 栈空间，超过线程栈限制会栈溢出；不能假设编译器一定做尾调用优化。
...
该方案用 `head==tail` 表示空，用 `(tail+1)%capacity==head` 表示满。若把最后一槽也占用，满时 `tail` 绕回与 `head` 相等，就和空状态无法区分，所以必须牺牲一槽。有效元素数为 `(tail-head+capacity)%capacity`，最大 `capacity-1`。
...
保存 `size` 可使用全部 `capacity` 个槽，但每次入队/出队都要同步更新额外状态，在并发实现中也增加一致性要求。空一槽方案不保存 size，却损失一个槽。两者都能 `O(1)`，关键是不混用空满条件。
...
只有头指针时，要找到末节点需沿 `next` 走完整条链，最坏 `O(n)`；保存 tail 可直接执行 `tail->next=new; tail=new`，为 `O(1)`。不变量是空队列 `head=tail=null`，插入首节点两者同时指向它，删掉末节点也必须同时清空。
...
入队只压入 `in`；出队时若 `out` 空，才把 `in` 全部弹出并压入 `out`，再从 `out` 弹出。每个元素一次进入 `in`，至多一次从 `in` 搬到 `out`，然后一次离开，不会搬回。因此 `m` 次操作总搬移 `O(m)`，单次摊还 `O(1)`，尽管某次倒栈是 `O(n)`。
...
若新元素下标更靠右且值不小于旧队尾，那么在旧元素仍处于未来任一窗口时，新元素也仍在窗口中并至少同样大；旧元素还会更早过期，所以永远不可能优于新元素。队列保持“下标递增、值递减”，每个下标至多进出一次，时间 `O(n)`。
