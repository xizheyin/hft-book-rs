...
## 22. 章末做题方法：画每一趟和判定排序性质

1. **读题先问目标**：全排序、部分最小项、稳定排序、原地、外存还是特定值域；数据规模和初始有序程度会改变选择。
2. **按算法画一趟**：插入写已排序前缀，冒泡写已就位后缀，选择写当前最小，归并写两个有序段，快排写 partition 后左右关系。
3. **性质题用元素身份测试**：给相等键附上 `a/b` 标签观察相对顺序判断稳定性；统计额外数组/递归栈判断空间；按最坏输入推比较次数。
4. **外部排序题分阶段**：先算初始归并段数量，再由归并路数算趟数；I/O 量按读写完整数据的次数估算，败者树只降低选择开销而不消除 I/O。
5. **验算**：每一趟保持声称的不变量；输出非降序且是原输入的同一多重集合；边界包括空、单元素、全等、逆序。

常见陷阱：快排永远 `O(n log n)`；稳定性只看数值看不出相等元素交换；原地等于零额外空间；归并漏算尾段；外部排序只算 CPU 比较不算磁盘读写。
...
稳定性保留“关键字相等记录在输入中的相对先后”。例如成交按价格排序，输入 `(100,A早),(99,B),(100,C晚)`；稳定升序应为 B、A、C。若输出 B、C、A，价格数值仍有序，却打乱同价时间顺序。判断时给相等键附 A/B 身份，不能只看数值。
...
比较排序的决策树每次二元比较最多分两支，要区分 `n!` 种排列，树高至少 `ceil(log₂ n!)=Ω(n log n)`，即某输入至少做这么多比较。计数排序直接用有限整数键索引计数，不只通过两两比较获取信息，时间 `O(n+K)` 依赖值域 K，因此不受比较模型下界约束。
...
初始 `[5]` 有序。插 2：`[2,5,4,6,1,3]`，前 2 项有序；插 4：`[2,4,5,6,1,3]`，前 3；插 6：不动 `[2,4,5,6,1,3]`，前 4；插 1：`[1,2,4,5,6,3]`，前 5；插 3：`[1,2,3,4,5,6]`，前 6。不变量是每趟开始前 `[0,i)` 已有序，移动大于 key 的元素后插入。
...
在有序前缀中找插入位置，比较次数从线性降为 `O(log i)`；但连续数组仍要移动最多 i 个元素。全部趟移动和最坏为 `1+...+(n-1)=Θ(n²)`，所以总时间仍 `O(n²)`。它优化比较常数，不改变搬移主项。
...
按各 gap 对每条子序列做插入排序。gap=5：比较 `(0,5),(1,6),(2,7),(3,8)`，得 `[3,1,4,2,5,9,6,8,7]`。gap=3：子序列索引 `0,3,6` 得 `2,3,6`；`1,4,7` 已 `1,5,8`；`2,5,8` 为 `4,9,7→4,7,9`，总数组 `[2,1,4,3,5,7,6,8,9]`。gap=1 做普通插入，得 `[1,2,3,4,5,6,7,8,9]`。每趟验算所有相距 gap 的子序列有序。
...
一趟相邻比较若发生逆序就交换，并记录 `swapped=true`；整趟没有交换说明全序列已有序，可提前结束。升序冒泡从左向右时，每趟把当前未排序部分最大值送到右端，所以末尾已有序后缀扩大一项。最坏 `O(n²)`，已有序加提前退出为 `O(n)`。
...
采用 Lomuto 变体：先把基准 4 交换到末尾，得 `[6,2,7,3,1,4]`；`i=0` 表示 `<4` 区右端。扫 j：6 不动；2 与位置0交换→`[2,6,7,3,1,4]`,`i=1`；7 不动；3 与位置1交换→`[2,3,7,6,1,4]`,`i=2`；1 与位置2交换→`[2,3,1,6,7,4]`,`i=3`。最后基准与位置3交换→`[2,3,1,4,7,6]`。验算基准左都 `<4`、右都 `≥4`；左右内部无需有序。
...
若每次取最左/最右且输入已有序，partition 可能分成 0 与 `n-1`，递推 `T(n)=T(n-1)+Θ(n)=Θ(n²)`，递归深度 n。随机基准让任意固定输入出现连续极端划分的概率降低，改善期望；三路分区把 `<,=,>` 分开，避免大量重复键被反复递归。随机化不提供最坏绝对保证。
...
第一趟在后缀找到最小值 1，与首项 2A 交换，数组变 `[1,2B,2A]`。两个相等键原来 A 在 B 前，排序后 B 在 A 前，故不稳定。数值序列 `[1,2,2]` 看不出问题，必须保留记录身份标签。
...
自底建堆：i=1 把 10 保持在 5/1 上；i=0 与 10 交换，再把 4 与 5 交换，得 `[10,5,3,4,1]`。取 10：与末尾换、下滤，得有效堆 `[5,4,3,1]`，后缀 `[10]`；取 5→有效 `[4,1,3]`，后缀 `[5,10]`；取 4→有效 `[3,1]`，后缀 `[4,5,10]`；取 3→有效 `[1]`，最终 `[1,3,4,5,10]`。每轮验算有效前缀满足父≥子，后缀有序且均不小于前缀。
...
给两个 4 标为 `4L,4R`。双指针比较时相等必须先取左段元素，即用条件 `left[i] <= right[j]`，输出为 `[1,2,4L,4R,6,7]`。若相等时取右段，就会让原来位于左段、在全局更早的记录落后，破坏稳定性。时间 `O(m+n)`。
...
前者 `K=11`，计数数组很小，时间 `O(n+K)`、空间 `O(K)`，很适合；用偏移 `key+5` 映射下标。后者若直接开十亿级计数数组，初始化和内存远超 n=20，通常用比较排序 `O(n log n)` 更合适。计数排序选择取决于值域而不只是元素数。
...
需要键在桶范围内较均匀分散，使每桶期望元素数有界，并让桶内排序总成本接近线性。若全落同一桶，问题退化为对 n 个元素做一次桶内排序：插入排序可到 `O(n²)`，比较排序为 `O(n log n)`。还要算桶数组空间和边界映射。
...
每轮必须稳定。按个位：`720,355,436,457,657,329,839`；按十位（读取上一轮顺序）：`720,329,436,839,355,457,657`；按百位：`329,355,436,457,657,720,839`。验算第三轮整体升序；若某轮不稳定，例如颠倒同一数字桶内顺序，后续高位不能保住低位次序。
...
`2 GiB=2048 MiB`，初始 run 数 `2048/256=8`。4 路归并第一趟把 8 段并成 `ceil(8/4)=2` 段，第二趟把 2 段并成 1 段，因此需 2 趟归并；等价于 `ceil(log₄8)=2`。验算每趟段数必须严格下降且最终为 1；题目已忽略输入/输出缓冲占用。
...
直接扫描 k 个段首每次 `O(k)` 比较；最小堆取最小并替换后调整为 `O(log k)`；败者树更新从叶到根也约 `O(log k)` 且比较组织常数较小。三者都必须读入每条记录并把结果写出，败者树只减少“从候选段首选谁”的 CPU 比较，不跳过数据读写。
...
近乎有序小数组先用插入排序（移动少、简单）；稳定大数组用归并排序（`O(n log n)`，需 `O(n)` 缓冲）；内存紧且要最坏保证用堆排序（原地、最坏 `O(n log n)`，但不稳定）；小而稠密整数键用计数排序（需可承受值域 K）；远大于内存用外部归并（生成 runs 后多路归并）。验算每个选择都写清稳定性、额外空间、值域或外存前提，而非只报名称。
