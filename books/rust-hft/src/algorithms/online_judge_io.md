...
## 13. 章末做题方法：把样例协议变成输入状态机

1. **读题圈定用例边界**：固定数量、首行 `T`、哨兵、空行分组还是读到 EOF；同时标字段是 token 还是整行文本。
2. **画读取顺序**：逐行写“读取什么→成功条件→存到哪里”，混用 `>>` 与 `getline` 时明确谁消费行尾换行。
3. **用原始文本推演**：覆盖前导/尾随空格、空行、最后无换行、多个用例和提前 EOF；每次读取失败立刻停止使用旧变量。
4. **校验输出协议**：空格、换行、大小写、精度和用例间分隔完全按题意；调试信息只能写到本地或 `stderr`，提交前移除。
5. **验算环境**：按指定语言标准编译，检查整数宽度、递归深度和输入规模；本地使用重定向运行完整样例而不是只调函数。

常见陷阱：`while(!eof())` 多处理一次；读取失败仍使用上轮值；`getline` 读到遗留换行；把字段内空格的文本用 `>>` 截断；本地依赖非标准扩展。
