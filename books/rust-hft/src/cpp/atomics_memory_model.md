...
`std::atomic` 和内存顺序（memory order）就是 C++ 用来回答这个问题的工具。正确的原子代码必须用一条清楚的因果链说明哪次写发布数据、哪次读接收数据，以及它们怎样建立先发生关系；汇编只能帮助核对实现，不能替代语言层证明。
...
假设控制线程偶尔更新一组路由规则、模型参数或风险限额，工作线程频繁读取。一个可解释的设计是：
