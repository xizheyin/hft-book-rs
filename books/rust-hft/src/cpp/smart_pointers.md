...
智能指针在后端、AI Infra 和交易系统中常出现在两个不同位置：

- **控制面配置**：更新频率低，工作线程读取频率高。可发布不可变 `shared_ptr<const Config>` 快照，但避免逐条消息复制所有者；还要规划旧配置何时析构。
- **数据面请求/张量任务/订单**：所有权通常沿流水线单向移动。`unique_ptr` 表达清楚，但每个对象动态分配可能昂贵；预分配槽位、值类型 ring buffer 或对象池可能更合适。
...
智能指针题先画所有权图，节点是对象，边标 `unique`、`shared`、`weak` 或非拥有观察：

1. `unique_ptr` 移动时删除源的拥有边并给目标添加；任意时刻同一对象只能有一条 unique 拥有边。
2. `shared_ptr` 每次复制/销毁更新 strong count，`weak_ptr` 更新 weak 观察关系；strong 归零销毁对象，控制块释放条件另行判断。
3. 检查 `shared_ptr` 的创建方式，避免由同一裸指针独立构造多个控制块；`enable_shared_from_this` 也要求对象已经由合适的 shared ownership 管理。
4. 删除所有外部根后，若 shared 边仍成环且计数不归零，把反向或观察关系改为 `weak_ptr`。
5. 自定义 deleter、数组和跨动态库边界题核对释放函数、完整类型和分配器家族。
6. `shared_ptr` 的引用计数线程安全不等于被指对象的成员访问线程安全；另行画同步边。

验算点是每个对象的销毁触发条件唯一可达，任何裸指针观察都短于拥有者，并且所有环都至少切断一条 shared 拥有边。
