...
设解析、状态更新和业务判断的耗时分别为 `P`、`U`、`D`，一次动态分配及相应初始化的耗时为 `A`。没有逐事件分配时，所讨论路径为：

\[
T_{base} = P + U + D
\]

若每个事件都在这条路径上额外分配，路径才变成：

\[
T_{event} = P + U + D + A
\]

若同一个对象在启动阶段分配，运行阶段只是访问它，那么 `A` 属于启动成本，不应加进逐事件路径。实际的 `A` 还可能包含分配器、初始化、缺页和同步，必须在目标环境测量，不能用一个固定纳秒数代表所有分配。
...
编译、链接与内存区题用“名字在每个阶段是什么”来追踪：源文件声明、目标文件符号、链接后地址、装载后映射地址。

1. 对每个全局名字标出定义或声明、内部或外部链接、所在翻译单元；同一需要唯一定义的实体不得出现冲突定义。
2. 编译后把函数、只读常量、已初始化全局、零初始化全局分别放入相应节，并标出未解析符号与重定位项。
3. 链接时为每个未定义符号寻找一个可见定义，再根据最终节地址修补重定位；找不到或多重冲突都应在此阶段暴露。
4. 装载时区分文件节与进程内存段，记录读/写/执行权限；栈与堆是运行时区域，不是直接从同名目标文件节复制而来。
5. 局部自动对象按控制流进入作用域时开始生命周期，动态对象由分配与释放控制；“位于栈上”不能替代生命周期分析。
6. 动态库题再加入位置无关代码、导入符号和运行时绑定步骤，不能把链接时符号值当成所有进程固定虚拟地址。

验算点是每个符号引用最终有唯一解释、每次重定位的基准和目标明确，运行时写入不会落到只读映射。
