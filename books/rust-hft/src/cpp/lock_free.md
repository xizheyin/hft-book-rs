...
这里的“步骤”不是墙上时间。即使算法是 wait-free，线程也可能被操作系统暂停很久；算法只承诺它重新得到 CPU 后，不需要无限等待其他参与者。
...
SPSC 环形队列题先写三条不变量：只有生产者写 `head`，只有消费者写 `tail`；槽位在发布前归生产者，在释放后才能再次归生产者；索引回绕不改变逻辑先后。

1. 选择并坚持一种索引定义，例如 `head` 指向下一个写入位置、`tail` 指向下一个读取位置；先据此写空与满条件。
2. 生产者按“acquire 读取 tail 并检查空间 → 构造/写槽位 → release 发布 head”推演；消费者按“acquire 读取 head → 读取/移动并结束槽位对象生命周期 → release 发布 tail”推演。自身维护的索引通常可用更弱顺序读取，但跨线程观察对方索引必须与发布协议配对。
3. 在事件图中确认消费者读槽位 happens-after 生产者完成写入，生产者复用槽位 happens-after 消费者完成读取。
4. 对容量边界、索引回绕和整数溢出逐次模拟；若牺牲一个槽位区分满空，有效容量应为数组长度减一。
5. 若槽位存放非平凡对象，标出 placement construction 与 destruction，不能只更新索引而遗漏对象生命周期。
6. 线性化点、无锁进展和等待自由分别判断；SPSC 正确并不自动推广到 MPSC/MPMC。

验算时枚举空队列、仅一个元素、满队列和回绕四种状态，确保任何槽位不会同时被读写，已发布元素不会被跳过或读取两次。
