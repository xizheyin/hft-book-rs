...
比较方案时至少记录：吞吐、P50/P99/P999、CPU 占用、队列满/空次数、CAS 失败次数、消息年龄、线程亲和性和 NUMA 拓扑。一个单线程平均耗时无法说明并发结构是否合适。
...
一个多生产者 CAS 队列平均延迟更低，但 P999 更高、CAS 失败数随线程数剧增。可以先尝试什么结构调整？

<details>
<summary>参考答案</summary>

考虑减少共享：按 symbol/account 分片，或给每个生产者独立 SPSC，再由消费者聚合。同时保持相同负载、线程亲和性和测量边界，重新比较吞吐、P50/P99/P999 与 CPU 使用率。
