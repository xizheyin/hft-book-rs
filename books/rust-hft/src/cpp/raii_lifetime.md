...
> 本章目标：能解释构造函数、析构函数和 RAII；能用作用域推演资源释放顺序；理解异常或提前返回时为何仍能清理；优先组合标准 RAII 类型，而不是手写 `new`/`delete`；知道确定析构时机不等于析构成本为零。
...
假设一个批次对象拥有 1,000 个子对象，每个子对象析构平均花 8 ns。这是**教学数字**，真实成本受析构逻辑、缓存、分配器和硬件影响。

离开作用域时集中清理的估算成本是：

\[
1{,}000 \times 8 = 8{,}000\ \text{ns} = 8\ \mu s
\]

RAII 保证“会清理”，却不会把 8 微秒变没。如果这个大对象恰好在逐条消息关键路径尾部析构，就可能形成尾延迟尖峰。可考虑：
