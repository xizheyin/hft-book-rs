...
RAII 构造析构题画“构造栈”：每成功构造一个子对象就压栈，正常离开或异常展开时按规则逆序弹栈。

1. 构造派生对象时依次记录虚基类、直接基类、成员（按声明顺序）和构造函数体；初始化列表书写顺序不能改变成员实际顺序。
2. 若某一步抛异常，只析构已经完成构造的基类和成员；最外层对象本身尚未构造完成，不调用其析构函数。
3. 普通作用域中记录局部对象完成构造的先后，离开作用域时逆序析构；动态对象则由拥有它的 RAII handle 生命周期控制。
4. 移动 RAII 对象后更新资源所有者，源对象析构必须安全且不能再次释放同一资源。
5. 析构函数通常不能让异常逃出，尤其在已有异常展开时；错误应通过显式操作提前报告。
6. 锁、文件和事务 guard 题在每条 return、throw、break 路径检查 guard 何时离开作用域。

验算点是每个成功获得的资源恰有一次释放，析构次序严格是成功构造依赖的逆序，部分构造失败也没有泄漏或重复释放。
