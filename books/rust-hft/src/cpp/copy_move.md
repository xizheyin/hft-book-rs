...
> 本章目标：能判断常见表达式会复制还是可能移动；理解 `std::move` 只是转换表达式类别；知道被移动对象仍然存在但值通常不能假设；能用 Rule of Zero 设计业务类，并解释何时需要 Rule of Five 与 `noexcept`。
...
## 13. HFT 联系：队列边界上的所有权转移

生产者把消息批次交给消费者时，常见选择包括：
