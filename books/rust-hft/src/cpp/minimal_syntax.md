...
引用捕获不会自动延长外层变量的存在时间，也不会让多线程读写自动安全。如果 lambda 被保存到外层变量已经消失之后，或交给另一个线程并同时修改共享数据，仍可能出错。[C++ 线程与同步](threads_sync.md)会专门解释生命周期、`std::thread`、锁和条件变量；本节只要求你会辨认捕获列表并调用 lambda。
...
这里的类型名是 `Side`，合法的命名值是 `Side::buy` 和 `Side::sell`。枚举不会自动替你验证外部字节或整数；解析输入时仍要拒绝未知编码，因此示例没有把所有“非 buy”状态都当成 sell。`::` 可以先读作“到这个名字所属的区域中查找”：
...
下面的对照只说明两种语言怎样表达相近意图；C++ 的规则仍以本章前面的对象、引用和类型语义为准，不能从 Rust 语法机械类推。
...
语法推导题对每个表达式写四项：静态类型、值类别、读取的对象、可能修改的对象。

1. 从声明开始分清对象本身与指向/引用的对象，读取 `const` 应检查 const 作用在哪一层。
2. 对字面量和算术先做整型提升、通常算术转换，再计算结果；不要先按期待的目标类型计算后再“塞进去”。
3. 函数重载题列出候选，依次检查名字可见、参数个数、模板推导/约束和隐式转换等级，再选择最佳可行函数。
4. 条件与循环逐次写出条件值和变量状态；范围 `for` 再判断循环变量按值还是引用，按值修改不会回写元素。
5. 返回局部对象时区分返回值对象与局部对象生命周期；返回局部引用或指针会悬空。
6. 最后沿作用域标出构造和析构，检查所有读取都发生在初始化后、销毁前。

验算可把复杂表达式拆成若干有名字的中间变量；若拆分后类型或副作用与原式不同，说明忽略了转换、值类别或求值规则。
...
面向 HFT 数据建模时，可以用 `enum class` 表示有限业务状态，用 `struct` 组织简单记录，用 `class` 划分公开接口和内部表示。价格常以协议规定的整数刻度表达，但整数、访问权限和类型名称都不会自动保证业务正确，仍需明确范围与不变量。接下来先用[基础复习：重新找回手感](basics_refresher.md)把这些语法写进完整小程序，再进入编译、内存和生命周期主线。
