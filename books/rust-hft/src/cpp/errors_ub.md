...
失败可以分成可预期结果、异常和未定义行为三类。前两类是程序设计的控制流；未定义行为则表示程序已经违反语言规则，不能再假设后续行为正常。三者需要不同的处理边界。
...
未定义行为审查用“对象—存储—访问”三层清单：

1. 对每次访问确认对象生命周期已经开始、尚未结束，指针/引用指向该对象允许访问的字节范围且满足对齐。
2. 确认读取前值已初始化，类型访问符合别名与对象表示规则；`memcpy` 可搬字节不代表任意字节都构成有效对象值。
3. 算术题分别检查有符号溢出、除零、移位宽度和转换范围；无符号回绕有定义，不能据此推断有符号也一样。
4. 容器和字符串操作后更新所有迭代器、指针和引用的有效性；释放或重分配后旧地址即使数值没变也不能继续使用。
5. 异常题沿每个抛出点展开栈，确认已构造子对象会逆序销毁，并检查 `noexcept` 边界是否导致终止。
6. 工具验证组合使用编译警告、sanitizer、静态分析和测试；一次运行没报错不能证明不存在 UB，因为 UB 本身不承诺稳定表现。

验算时要求每个解引用都能回答“哪个仍活着的对象、哪段合法范围、哪种允许的访问类型”。任何一项只能靠“通常如此”回答，都应继续修正设计。
