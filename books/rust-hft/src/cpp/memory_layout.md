...
1. 按成员声明顺序，把当前偏移向上补齐到成员对齐要求，记录前置 padding、成员大小和结束偏移。
2. 全部成员放置后，把对象总大小补齐到最大对齐，确保数组中下一个对象也正确对齐；基类、虚函数和 `[[no_unique_address]]` 等情况要以实现/ABI 观察为准。
3. 用 `sizeof`、`alignof` 和允许使用时的 `offsetof` 验证具体实现，不把一次 ABI 结果写成所有平台的语言保证。
4. 将成员地址区间映射到 Cache 行，判断热循环真正读取了多少有效字节；多个线程写同一行的不同字段才是伪共享。
5. 比较 AoS/SoA 时计算给定访问模式每行承载的有用元素数，同时考虑索引、间接访问与批处理方式。
6. 性能结论用同一数据规模和访问序列的基准、Cache miss/带宽计数器建立证据，结构体更小不必然让整条路径更快。

验算点是所有成员区间不重叠且满足各自对齐，总大小是最大对齐的整数倍，性能解释能对应到测得的内存行为。
