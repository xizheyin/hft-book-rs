...
容器选择与失效题建立操作表：每一步记录 size、capacity/bucket count、复杂度、被移动元素范围和仍有效的指针/迭代器。

1. 先按访问模式筛选：是否需要连续内存、随机访问、稳定引用、有序遍历、按键查找或两端操作。
2. 对 `vector` 插入前判断是否超过 capacity；重分配会使所有指针/引用/迭代器失效，不重分配时仍要按插入位置规则判断。
3. 对哈希容器跟踪负载因子与 rehash；平均常数查找依赖哈希分布，不能当作最坏常数。
4. 对树容器区分节点稳定性和迭代器失效规则，并把 `O(log n)` 与实际分配、间接访问成本分开。
5. 排序/算法题检查迭代器类别是否满足要求，比较器必须维持严格弱序；否则结果不由“偶尔看起来有序”保证。
6. 摊还复杂度题把一段操作的总成本除以操作数，不把某一次扩容的线性成本错误写成每次都常数。

验算时让表中每个仍被使用的迭代器都能证明未被对应操作失效，并用实际元素数与移动/构造计数核对复杂度解释。
