...
本章从最简单的函数模板开始。你不需要预先了解泛型、虚函数表或元编程。

> **本章目标**：读完后，你能写出受约束的 C++20 模板，解释 `constexpr` 的边界，并能根据热路径、扩展性和代码体积选择虚函数或静态分发。

> **面试优先级**：P0 必会模板、Concept、`constexpr` 与虚函数分别解决什么问题，以及静态/动态分发的取舍；P1 理解单态化、内联与代码体积之间的关系；CRTP 是 P2 岗位选读，除非面试明确偏 C++ 模板或低延迟基础设施，不必背它的写法。
...
它存在的原因，是让基类复用一段公共流程，同时在编译期调用派生类提供的“钩子”。先能说清这个动机和它不适合运行时异构对象即可；下面的实现细节按岗位选读。

<details>
<summary>P2 岗位选读：CRTP 的最小实现与边界</summary>
...
## 7. HFT 场景：热路径和控制面用不同抽象

一个常见设计是：

- 行情逐条解码、订单簿更新等已经确认的热循环，用模板让消息类型在编译时确定；
