...
1. 为每种非法输入写“状态不变”断言：价格非正、数量非正、`bid>=ask` 时处理返回拒绝，订单簿哈希、持仓和输出订单数均与调用前一致。
2. 状态轨迹可写为 `Live(seq=n) → 收到 n+2 → Stale(gap=[n+1,n+1])`；Stale 时只允许恢复/受控撤单，不生成新意图。补齐或应用快照后验证序号屏障和盘口不变量，再回 Live。
3. 风险账使用 `worst_long=position+open_buy`、`worst_short=position-open_sell`。发新单先原子预占，成交把未成交占用转成持仓，只有权威撤单/拒单确认才释放剩余占用；验算 `初始占用+新增-成交转移-确认释放=当前占用`。
4. 示例迁移为 `PendingNew→Working→PendingCancel→Cancelled`，并包含 `Rejected/Filled`；每个回报按 order ID 和版本只应用一次。取消与成交竞态中，Fill 可从 PendingCancel 更新数量，不能因已发取消就提前释放风险。
5. 固定输入、配置和初始状态，逐事件更新确定性哈希；两次运行的事件数、最终订单簿/持仓/订单状态哈希应一致。若包含时钟或随机数，把它们作为显式输入记录。
6. 先保存语义等价基线，再只替换数据结构；固定硬件、编译器、输入、深度、预热和线程，逐事件比对输出，最后比较 P50/P99、吞吐、分配与 Cache 指标。结果不同的两次运行不能比较性能。
...
贯穿系统题用事件回放表推演，而不是只检查最终输出：

| 序号 | 输入事件 | 订单簿/状态变更 | 风控决定 | 输出事件 | 不变量 |
|---|---|---|---|---|---|

1. 给输入建立严格序号与解析结果，非法消息应在改变状态前被拒绝。
2. 每个事件只执行一次状态转移，记录旧值、新值和产生的下游消息；重放相同前缀应得到相同状态。
3. 在订单产生前计算并记录风控所依据的快照，保证限额检查与实际提交数量一致。
4. 队列满、组件失败和取消时分别推演，明确事件是阻塞、拒绝、重试还是进入恢复流程，不能静默丢失。
5. 为所有权边界标注消息在哪个组件创建、移动和销毁，检查复用缓冲区不会在消费者完成前被改写。
6. 性能测量只包住目标路径，并同时保存正确性计数；更快但事件数或结果不同的运行不可比较。

验算使用两个守恒关系：接受数加拒绝数应等于进入风控的有效请求数；任何时刻持仓和挂单暴露应能由已经处理的事件前缀重算得到。
