...
结构体布局题按声明顺序维护一个“当前偏移”：

1. 放置字段前，把当前偏移向上补齐到该字段对齐值的整数倍；补出的字节是字段前 padding。
2. 放置字段并增加字段大小；所有字段结束后，再把总大小补齐到结构体最大对齐值的整数倍，得到尾部 padding。
3. 用“数组中第二个元素的起始地址也必须满足最大对齐”验算尾部 padding 是否必要。
4. Cache 行题把对象起始地址和每个字段范围除以行大小，写出各自落在哪一行；只有不同线程反复写同一 Cache 行中的不同位置，才构成伪共享。
5. 比较 AoS 与 SoA 时，先圈出循环真正访问的字段，再计算每个 Cache 行中有多少有效字节。不能只凭“字段更紧凑”断言一定更快。

Rust 默认布局不承诺稳定字段顺序，涉及固定 ABI 或协议时必须先看 `repr` 约定；`#[repr(align(N))]` 提高对齐也会增加对象和数组步长，不能只算单个字段。
...
1. 字段必须从满足自身对齐的地址开始，因此前一字段后可能补 padding；对象数组中每个元素都要满足最大对齐，所以结构体末尾也可能补齐步长。
2. 时间局部性是短时间重复访问同一数据，如循环累加器；空间局部性是访问相邻地址，如顺序遍历 `Vec`。
3. 每次处理一条记录的大多数字段时 AoS 自然；批量只处理同一字段、便于向量化时 SoA 常有更高有效字节比例。最终取决于访问模式。
4. Cache coherence 以整行转移所有权；两个变量即使不同，只要同处一行且被不同核心反复写，就会让该行来回失效。
5. 每字段对齐到一行会扩大对象和工作集，增加 Cache/TLB miss 与内存带宽，还可能破坏批量布局；只应隔离有证据的跨核写热点。
6. 先核对字段偏移、对齐和数组步长，再检查每行有效字节、工作集是否跨级 Cache、访问是否新增间接层/分支、是否破坏向量化，以及 false sharing/NUMA。用同输入基准与 Cache/TLB/带宽计数器建立证据。
