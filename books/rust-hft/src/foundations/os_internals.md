...
分析操作系统执行路径时，先做一张事件表，列为“当前执行者、CPU 特权级、正在运行还是等待、下一次唤醒来源”：

1. 普通函数调用仍在用户态；系统调用通过受控入口进入内核；设备中断由硬件事件进入处理程序。三者的触发原因不同。
2. 对每次阻塞写明等待条件，例如“等待数据到达”或“等待锁可用”，并标出由谁在条件满足后把线程变为 runnable。
3. 区分模式切换与线程切换：进入内核不必然更换线程，线程阻塞或调度器选择别人时才可能发生上下文切换。
4. I/O 路径把提交与完成分开；线程提交请求后可以阻塞，也可以继续做别的工作，完成事件之后还要经过唤醒与调度才重新运行。
5. 最后检查保护目标：特权指令、地址空间和内核同步分别防止什么错误。若答案只是“为了安全”，应继续指出具体资源和失败方式。

验算整条链时，任意相邻两步都应有明确事件连接：是谁调用、谁返回、谁中断或谁唤醒。无法解释的“然后”通常就是漏掉的内核机制。
...
1. 页表和设备寄存器控制隔离、物理内存与硬件；若普通应用可直接修改，就能读取他人内存、绕过权限或破坏设备。CPU 特权级和内核接口把这些操作收口。
2. 系统调用由应用执行陷入指令主动触发；异常由当前指令的执行条件同步触发；中断由定时器或外设等外部事件异步触发。
3. Running 正在 CPU 上执行；Ready/Runnable 已具备运行条件、正在等 CPU；Sleeping/Waiting 尚缺事件，即使 CPU 空闲也不能继续。
4. 进入内核只是模式切换，仍可在同一线程上下文执行并返回。只有它阻塞、被抢占或调度器选择其他线程时才发生任务上下文切换。
5. `设备完成 → 中断/完成队列 → 驱动确认并更新请求 → 内核把等待线程置为 runnable → 调度器选择它 → 恢复上下文 → 系统调用返回`。设备完成不等于线程立即运行。
6. 阻塞/非阻塞描述调用在条件未满足时是否等待；同步/异步描述完成结果由当前调用等待还是稍后通知。同步非阻塞、异步提交等组合都可能存在。
7. 等待极短且可预测、线程可独占核心、睡眠唤醒更贵时轮询可能合适；代价是 CPU、功耗、总线/缓存流量和公平性。
8. `固件初始化硬件 → 选择并装载 bootloader/内核 → 内核建立内存、中断、驱动和根文件系统 → 启动第一个用户态进程 → 服务管理器按依赖启动普通服务`。
9. 容器通常共享宿主内核，以 namespace/cgroup 等隔离；VM 运行 guest kernel，由 VMM/硬件虚拟化形成额外内核边界。VM 边界更强但启动和资源成本通常更高，仍取决于配置。
