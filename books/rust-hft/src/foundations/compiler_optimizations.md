...
在低延迟系统中，性能问题经常不是“算法写错了”，而是“编译器无法证明你的意图”。同样的业务逻辑，若写法让优化器看不清数据依赖与别名关系，最终机器码和性能可能明显不同。理解 `rustc` 与 LLVM 的协作方式，本质上是理解一件事：哪些语义信息能够在编译期保留下来，并转化为更激进的优化。
...
> **面试主线**：先会解释“语义越清楚，优化器越敢内联、删分支和重排访存”，再会用 release 构建、汇编和基准验证。HIR/THIR/MIR 的逐阶段职责和 PGO 命令属于 P2；普通面试不要求背编译器内部 pass 名单。
...
<details>
<summary><strong>P2 选读：HIR、MIR 与 LLVM IR 分别承担什么</strong></summary>
...
<details>
<summary><strong>P2 选读：发布参数与 PGO 操作示例</strong></summary>

这些参数会随工具链、部署 CPU 和发布策略变化。面试中优先讲“固定构建条件、一次改变一个变量、用代表性负载做回归”，不要把下面配置背成万能模板。

编译参数是性能工程的一部分，不是“统一模板”。下面是常见发布配置，它强调运行期性能而非编译速度。

```toml
[profile.release]
opt-level = 3       # 较积极的常用数值优化等级；不保证比 2 更快
lto = "fat"         # 链接时全局优化
codegen-units = 1   # 减少当前 crate 的代码生成分区，通常会增加编译时间
panic = "abort"     # panic 时直接终止，不进行栈展开；恢复语义也随之改变
debug = false       # 不在主产物中加入完整调试信息；应另行保存可匹配符号
rpath = false
```

### 4.1 CPU 架构优化

若构建机器与所有部署机器属于已确认兼容的 CPU 集合，可以评估针对目标机器编译；否则 `native` 可能生成旧机器不能执行的指令，不应直接用于通用发布物。
...
第二个误区是只看平均延迟。编译器优化往往会改变分支布局与指令缓存行为，最终影响的是尾部延迟分布。评估时应同步观察 P99/P99.9 与硬件事件计数，而不是只看一次跑分。
