...
1. 圈出所有“读—改—写”操作，找出可能交错的位置；只说“两个线程同时执行”还不能指出竞争结果。
2. 临界区方案逐项检查互斥、空闲让进和有限等待。证明互斥时假设两个参与者同时进入，寻找与原子操作或状态条件的矛盾。
3. 信号量题先说明计数代表什么资源，再为每次 `P` 写“消耗一个许可”、每次 `V` 写“归还一个许可”。按调度序列维护计数与等待队列，初值错一位会改变整个结果。
4. 条件变量始终使用“持锁检查谓词 → `while` 中 wait 原子释放锁并睡眠 → 醒来重新持锁并检查”。`signal` 表示状态可能改变，不承诺被唤醒者运行时条件仍成立。
5. 生产者—消费者题用 `empty + full = N` 验算缓冲槽数量；读者—写者题说明偏向策略和饥饿风险；哲学家题检查是否主动破坏循环等待。
6. 最后为每条失败路径检查锁和许可是否成对释放，尤其是提前返回、超时与取消。

常见陷阱是把 mutex 当作事件通知，或把 semaphore 的数值机械理解为“正在等待的线程数”。计数的业务含义必须先定义。
...
1. 两线程都先读 `count=0`，各自算 1，再都写 1；执行两次加一却只增加 1。问题来自 read-modify-write 不是一个原子动作。
2. 互斥防止同时进入临界区；进展保证无人占用时需要进入者能作决定，不能永久拖延；有限等待防止某个请求在其他人反复插队下饥饿。
3. 若分开，A 检查到空闲后被切走，B 也检查到空闲并设置占用，A 恢复后再次设置，二者都认为持锁。test-and-set/CAS 等把判断和修改做成一个不可分割操作。
4. 初值 `10`，表示 10 个可用连接。前 10 次 P 各消耗一个许可；第 11 个在计数为 0 时等待，直到有人 V 归还连接。
5. 若“释放锁”和“入等待队列”之间有缝隙，生产者可能在缝隙中 signal，随后消费者才睡眠，造成 lost wakeup。原子 wait 让状态改变和睡眠注册不可错过。
6. 必须用 while：其一可能 spurious wakeup；其二被唤醒到重新获得 mutex 之间，其他线程可能已消费条件。醒来必须重新检查谓词。
7. 先 P(empty) 保证有槽位，再拿 mutex 修改缓冲。若先拿 mutex，在缓冲满时生产者持 mutex 等 empty；消费者需同一 mutex 才能取走元素并 V(empty)，形成等待环。
8. 持续到来的读者可让 reader count 永不归零，写者一直进不去。写者优先可在有写者等待时阻止新读者；公平方案用到达队列/turnstile 按序授予。
9. 五人各持左叉并等右叉，形成资源等待环。`room=4` 限制最多四人尝试拿叉，阻止五个单叉占有状态同时成立，因此至少有人可取得第二把并释放资源。
10. mutex 保护某一时刻只有一个参与者修改共享状态；屏障让一组参与者都到达某阶段后再一起继续。generation 区分连续两轮，避免上一轮迟到唤醒被误算进下一轮。
