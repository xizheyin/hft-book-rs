...
在低延迟系统领域，许多现存代码仍然长期建立在 C++ 之上。将系统迁移到 Rust 不是一次性替换，而是一个渐进演化的过程。本章讨论如何安全、高效地进行 C++ 到 Rust 的重构，以及在混合编程中可能遇到的边界问题。

> **P2 岗位选读**：只有简历项目涉及 Rust/C++ 混编、遗留系统迁移或 FFI（Foreign Function Interface，跨语言函数接口）时，才需要完整阅读。本章面试主线只有四点：从小边界渐进迁移；接口必须明确所有权与销毁方；异常/panic 不可随意跨 ABI；用同输入影子运行证明语义一致。具体 `cxx`、Corrosion 和跨语言 LTO 配置应在真实项目中查当前版本文档，不是通用面试背诵项。
