...
跨 C++/Rust 边界题用一张“边界契约表”逐项推演：

| 值/资源 | 谁分配 | 谁拥有 | 谁释放 | ABI 表示 | 失败怎样返回 |
|---|---|---|---|---|---|
| 输入缓冲区 | C++ | 调用期间仍归 C++ | C++ | 指针 + 长度 | 状态码/结果类型 |

1. 为每个参数和返回值确认是复制、借用还是所有权转移；借用必须标明在回调、异步任务或缓存中能否继续保存。
2. 只让稳定、明确布局的 ABI 类型过边界；C++ 模板类型、Rust enum、异常和 trait object 不应未经桥接直接暴露。
3. 画分配与释放配对，保证同一分配器家族负责回收；两侧都释放或两侧都不负责都会出错。
4. 列出正常返回、C++ 异常、Rust panic、错误码和进程终止路径，确保异常/panic 不越过不允许展开的 FFI 边界。
5. 对字符串、容器和回调检查指针失效、编码、容量以及调用方对象寿命；“零拷贝”必须同时满足底层内存足够长寿且期间不被重分配。
6. 迁移验证同时比较输出、错误、资源计数和性能；影子流量只能证明覆盖到的输入，不能替代边界契约。

验算点是：每个跨界对象在任意时刻都有且只有一个明确销毁责任，所有借用都短于来源，所有失败路径都能回到双方约定的状态。
