...
判断一个抽象是否引入额外成本时，用“语义操作清单”对比抽象版与手写低层版：

1. 列出分配次数、复制/移动字节数、间接调用、分支、边界检查、引用计数更新和同步操作。
2. 对泛型、迭代器和闭包先代入具体类型，判断能否单态化、内联和消除中间对象；对 `dyn Trait` 则明确数据指针与虚表间接调用。
3. 对所有权包装器区分 `Box` 的堆分配、`Rc/Arc` 的计数操作、借用本身与实际数据复制，不能把“智能指针”视为同一成本。
4. 比较生成 IR/汇编，确认两版剩余的可观察操作；再用 release 构建的基准测量，而不是用 debug 结果推断语言设计。
5. 检查代码体积、编译时间和缓存行为；运行时没有额外抽象开销，不代表所有维度都免费。
6. 若两版性能不同，沿调用、分配、分支和内存访问找到具体差异，不能用“编译器没优化好”结束分析。

验算标准不是“出现了高级语法”，而是最终执行了哪些本来可以避免的操作。若抽象版与合理手写版生成等价操作，才有证据称这部分抽象为零额外运行时成本。
...
1. 泛型调用通常按具体 `T` 单态化成专用机器码，运行时无需类型判断；若类型组合很多，会生成多个实例，增加二进制和 I-cache 压力。
2. `&dyn Trait` 是数据指针加虚表指针；虚表按具体类型提供方法地址。调用多一次间接查表/跳转，且可能阻碍内联，但借用本身不要求堆分配。
3. 迭代器 adapter 惰性构造状态，`map` 只描述变换；`collect` 消费迭代器，才执行闭包并按目标容器需要分配/写入。
4. Newtype 是新的名义类型，可限制混用并实现独立 trait；类型别名只是同一类型的另一个名字，`UserId` 与 `u64` 可直接互换。
5. 用同一输入和等价输出写迭代器版/for 版，release 同 target 编译；用 `black_box` 防删除，检查 IR/汇编和分配，再重复测端到端分布。结论限定到编译器、硬件和输入，不能先假设任一版更快。
