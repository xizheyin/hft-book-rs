...
设备与磁盘调度题先判断题目是否真的采用机械磁盘模型：

1. 机械磁盘题写出初始磁头位置、请求柱面、初始方向和边界规则，再按算法逐个选择下一请求。
2. FCFS 保留到达顺序；SSTF 每一步重新选择当前最近请求；SCAN/C-SCAN 是否走到物理端点、LOOK/C-LOOK 是否只走到最远请求，必须严格按题设区分。
3. 总寻道量等于相邻服务位置差的绝对值之和。服务序列写完后反向重算一次，所有原请求应恰好出现一次。
4. 若给出寻道、旋转和传输参数，分别计算后再相加；只有在题目说明可忽略某项时才能省略。
5. 缓冲/缓存/spooling 选择题先写目的：平滑生产消费速率、复用已有数据、还是把独占设备作业排队。三个词不能互换。
6. SSD 没有机械寻道，不能照搬柱面移动计算；仍可能存在控制器队列、写放大和公平性，但应使用题目提供的模型。

分析阻塞 I/O 时再加一条事件链：请求提交、线程等待、设备完成、中断/完成通知、线程变为 runnable、调度运行。设备完成时刻不必等于线程恢复执行时刻。
...
1. 块设备按可寻址块读写，适合随机访问；字符设备常提供顺序字节/事件流；网络设备处理分组和协议状态。强行统一会丢失边界、寻址与完成语义。
2. 系统调用层提供用户接口和权限入口；设备无关层处理统一命名、缓冲、排队和错误框架；驱动把通用请求转换成具体设备命令并处理中断/DMA。
3. 阻塞表示条件未满足时当前线程等待；非阻塞表示立即返回“暂不可用”；异步表示提交后稍后通过完成事件取得结果。这三者不是简单二选一同义词。
4. 用户态日志 buffer 把许多小记录合成较大 write，平滑生产/消费速率；页缓存 cache 保存文件页副本以复用并延迟回写。前者关注传输批量，后者关注数据副本与复用。
5. SPOOL 把作业先写入队列/磁盘，由单一后台服务按序驱动独占设备；多个用户提交任务而不直接争用打印机等设备。
6. 请求为 `98,183,37,122,14,124,65,67`，磁头 53，范围 0–199，初始向高端且端点移动计入：

   | 算法 | 服务/移动路径 | 总移动量 |
   |---|---|---:|
   | FCFS | `53→98→183→37→122→14→124→65→67` | `640` |
   | SSTF | `53→65→67→37→14→98→122→124→183` | `236` |
   | SCAN | `53→65→67→98→122→124→183→199→37→14` | `(199-53)+(199-14)=331` |
   | C-SCAN | `53→...→183→199→0→14→37` | `(199-53)+199+37=382` |

   单位都是磁道；验算每个请求恰服务一次。若题设采用 LOOK/C-LOOK 或不计回卷，结果会不同，必须声明。
7. SSTF 持续选择附近新请求，远端请求可能永远轮不到；SCAN 保持方向扫过整个范围，使远端最终被访问，等待更有上界。
8. SSD 仍需合并固定开销、利用内部并行队列、控制公平/优先级、避免过载，并遵守 flush/barrier 等正确性顺序；只是“磁头距离”不再是核心模型。
9. `设备完成 → 中断/完成队列 → 驱动收割并标记请求完成 → 唤醒线程为 runnable → 调度 → 恢复线程 → 系统调用返回`。
10. 块层只看请求和屏障等接口，不知道目录、事务提交或业务不变量；上层必须明确哪些数据/元数据先持久化并发出 flush/FUA 等约束，块层才有可保持的顺序。
