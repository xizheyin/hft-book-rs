...
> **面试主线**：必须能解释流水线为什么提高吞吐、分支预测失败为什么浪费工作、依赖链为什么限制并行，以及缓存未命中为什么让 CPU 等待。TSC 汇编序列属于 **P2 选读**；除非岗位明确要求超低延迟计时或你在项目中用过，否则理解“计时也会扰动被测对象、必须校准和防重排”就够了。
...
<details>
<summary><strong>P2 选读：直接读取 TSC 时怎样约束 CPU 与编译器重排</strong></summary>

下面内容服务于需要自己实现 x86 短区间计时器的岗位。普通性能面试优先说明为什么使用成熟基准框架，并能指出迁核、频率、预热和空计时边界等误差来源，不需要默写汇编。
