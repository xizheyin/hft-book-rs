...
## 9. 做题方法：用版本矩阵和故障时间线审查发布

1. **读题列发布单元**：二进制、配置、协议 schema、内核/驱动/固件、FPGA bitstream、风险限额和恢复数据分别是否变化。
2. **画兼容矩阵**：新旧发送端、接收端、状态/快照和回滚版本两两能否共存；破坏性格式变更必须有迁移与双读/双写边界。
3. **画流量切换时间线**：准入检查→影子/小流量→健康门槛→扩大→完成，期间谁持有订单会话、行情和风险状态必须唯一。
4. **预演故障**：进程启动失败、状态恢复失败、版本指标恶化、节点断电和回滚不兼容分别写停止条件、回滚动作与权威事实源。
5. **验算**：产物可识别可复现；canary 覆盖真实消息与故障；回滚后订单、持仓、行情 generation 和审计连续；观测本身不会阻塞关键路径。

常见陷阱：只回滚二进制不回滚配置/格式；所有实例同时升级；健康检查只看进程存活；新旧实例同时发送同一会话；变更后没有做 drop copy 与持仓对账。
