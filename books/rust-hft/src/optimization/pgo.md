...
它适合分支和调用分布较稳定的程序，但也很容易“把基准答案背进二进制”。训练流量不真实，PGO 可能优化常见路径，却让低频但重要的恢复路径变慢。因此，画像怎样产生、是否具有代表性以及怎样与普通构建公平比较，都是 PGO 流程的一部分。

## 1. 先理解编译器在做什么

Rust 源码会经过多层转换，最终由 LLVM 编译器基础设施生成机器码。MIR（Mid-level Intermediate Representation）是 Rust 编译器的中级中间表示；LLVM IR（Intermediate Representation）是交给 LLVM 优化的中间表示；LTO（Link-Time Optimization，链接时优化）让编译器在链接阶段跨编译单元观察代码。
...
训练数据还要去除或保护账户、策略等敏感信息，并能够在隔离环境中复现；真实流量频率与低频高风险路径都要覆盖。
...
## 9. 做题方法：证明 profile 与生产热路径一致

1. **读题定优化目标**：吞吐、端到端分位延迟、代码尺寸还是某类请求；先建立相同 release 配置的非 PGO 基线。
2. **画完整产物链**：源码/依赖/编译器→插桩二进制→代表性训练→profile 合并→优化编译，记录每一步版本和输入摘要。
3. **检查代表性**：按消息类型、错误路径、负载比例、CPU 特性和部署配置比较训练与生产；不能只跑单一 happy path。
4. **公平 A/B**：相同机器、数据、链接方式和 LTO/codegen 选项，随机化运行顺序，多轮测正确性、吞吐、尾延迟、代码尺寸与硬件计数。
5. **验算**：编译器确实消费 profile 且无大规模陈旧/不匹配警告；收益在独立验证集存在，非训练路径没有不可接受退化。

常见陷阱：把插桩版直接当最终性能；训练集等于验证集；profile 来自另一份二进制；同时改变 PGO、LTO 与代码导致无法归因；只看平均值。
