...
## 8. 做题方法：从局部限额推到 kill switch

1. **读题画风险层级**：订单、策略、产品、账户和全局分别有哪些限制，谁拥有权威状态，局部快照允许多旧。
2. **列风险向量**：持仓、在途、名义金额、PnL、速率和数据新鲜度分开，不用一个“风险分数”掩盖单位。
3. **推演状态变化**：New/Fill/CancelAck/Reject/行情异常/断线分别怎样更新占用和降级状态；对重复与乱序事件写幂等键。
4. **kill switch 画协议**：触发→阻止新单→发起撤单→确认/超时→升级人工→对账恢复；“开关已按下”不等于场所订单都已消失。
5. **用故障证据验算**：主回报、drop copy、持仓与清算记录对账；注入状态延迟、节点隔离和撤单失败，确认最坏暴露仍在声明边界内。

常见陷阱：只在策略进程做风控；把未实现收益当可立即释放的确定资金；行情陈旧仍计算价格风险；kill switch 只设一个布尔变量；全局限额检查没有处理并发超卖。
