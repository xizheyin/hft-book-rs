...
1. **读题定义状态**：可信订单簿、当前订单、持仓、定时器、参数版本和风险反馈分别由谁更新；策略只能基于一个明确 generation 的输入快照。
2. **画事件状态机**：MarketData/Timer/Fill/Reject/CancelAck/Gap/ConfigUpdate 到达后，先更新哪部分状态，再产生何种意图；副作用通过 context/gateway 执行。
3. **推演报价生命周期**：无报价→发 New→PendingNew→Working→PendingCancel/Replace→终态，重点加入成交与撤改交错。
4. **列策略不变量**：陈旧行情不新增风险、每侧至多多少活动订单、价格符合 tick/边界、订单意图先过风控、重放同一事件不重复发单。
5. **验算**：用完整事件 trace 回放得到同样意图；更换参数时记录生效边界；断线恢复不仅重放 ACK/Fill，还恢复行情、定时器和未决请求事实。

常见陷阱：回调内直接绕过风控发网络请求；只保存最终仓位而不保存未决订单；动态分发/分配成本未经测量就武断优化；墙钟时间进入回放导致不确定；Gap 后继续引用旧信号。
