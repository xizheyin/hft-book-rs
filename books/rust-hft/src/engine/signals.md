...
## 8. 做题方法：逐样本更新并与离线公式对账

1. **读题写采样语义**：按每条事件、固定时间还是成交量采样，窗口是否未填满，缺失/重复/乱序怎样处理。
2. **列最小状态**：EMA 保存上一值与 `alpha`；滑窗保存队列、和/平方和；回归保存计数与各项和。每个状态都标单位和数值范围。
3. **逐样本推演**：先写初始化，再加入新样本、淘汰旧样本、归一化和组合；不能把“更新前”与“更新后”的均值混用。
4. **处理数值与陈旧性**：检查零方差、极端值、整数溢出和浮点消减误差；行情 gap 或市场状态异常时冻结/失效信号。
5. **验算**：小窗口每一步与离线重算一致；生产实现与研究版本在同一数据、时间和参数版本下差分；SIMD 优化前后在规定容差内一致。

常见陷阱：EMA 的 `alpha` 与时间间隔不匹配；未满窗口除以固定窗口长；回归分母为零；回测先看到未来数据；只看收益不做成本、稳定性和样本外检验。
