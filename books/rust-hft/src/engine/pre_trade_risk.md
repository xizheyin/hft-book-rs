...
## 8. 做题方法：单位表、风险快照与预占原子性

1. **读题先写单位**：价格 tick/缩放、数量单位、合约乘数、币种和限额口径逐项列出；所有乘法先判断整数范围。
2. **区分三类状态**：已成交持仓、在途订单预占、当前请求增量分别成列；新单、部分成交、撤单确认和 Reject 对预占的影响逐事件推进。
3. **画一次原子决策**：读取同一风险快照→计算订单后敞口→比较单笔/产品/账户/全局限制→成功时预占。并发请求不能都基于旧值分别通过后再相加。
4. **价格检查带新鲜度**：标参考价来源、时间和 market state；陈旧或缺失时按策略拒绝/降级，不能让默认零或旧价通过偏离检查。
5. **验算**：最坏成交情况下限额仍满足；`position + reserved` 与订单状态对得上；取消请求未确认前不提前释放；溢出和单位错误走拒绝路径。

常见陷阱：只用 `fetch_add` 跨过多字段不变量；检查和预占分离产生 TOCTOU；浮点比较金额限额；多产品共享限额却各分片独立批准；以测得的平均延迟替代正确性证明。
