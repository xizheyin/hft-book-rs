...
## 9. 做题方法：先算路由，再画迁移或缓存时序

### 9.1 分片映射题逐键计算

范围分片先在边界表中定位键，并特别测试等于边界的值；取模分片直接计算 `hash(key) mod N`，扩容时对同一批样例键分别用旧 N、新 N 计算，数出迁移比例。一致性哈希题把节点和键按位置排序，键归给顺时针第一个节点；加入节点 D 时，只有 D 的前驱到 D 之间的区间改归 D。

答案还要做负载验算：键空间均匀不代表请求均匀。把每个热点键的请求率累加到负责分片，比较最热分片与单节点容量；只比较行数会漏掉单键热点。

### 9.2 再平衡题维护 ownership epoch

画出“复制历史数据、追赶增量、切换目录、处理旧路由”四个阶段，并给旧、新所有者标 epoch。每一步都问：此刻谁能接受写？旧客户端发来的写由谁拒绝或转发？若旧节点与元数据服务隔离，它是否还能直接修改一份独立数据？最后一问能检验 epoch 是否真的由写入资源执行，而不只是路由表上的数字。

### 9.3 Cache-aside 题强制插入迟到回填

固定使用四个参与者：读请求、写请求、缓存、权威数据库。让读先 miss 并读到 v1，写再提交 v2 并失效缓存，最后让读回填 v1。然后把候选方案代入这条时间线：

- 只有“先写库再删缓存”时，v1 仍会回来；
- 只有 TTL 时，错误值只会以后过期，不会立即被阻止；
- 有版本水位或 generation 时，要明确失效怎样推进它，以及迟到回填比较哪个值后被拒绝。

分析穿透、击穿、雪崩时分别数“不同不存在键”“同一热点键的并发回源者”“同一窗口过期的键数”，再选择负缓存/Bloom filter、singleflight、TTL 抖动，避免只背三个相似名词。
...
单用户查询由 `hash(user_id)` 路由到 1 个分片，并可在片内用 `(user_id,time)` 索引。全局日汇总没有 user_id 约束，需访问全部 N 片并合并，或读取预聚合结果。后者把成本移到写入/流处理，并需要定义延迟与纠错。
...
按顺时针第一个节点负责：5→10，30→40，65→70，90 绕回→10。加入 55 后，它接管前驱 40 之后到 55（通常写作 `(40,55]`）的键，原来属于 70；其余区间不动。边界是否含端点要按实现统一。
...
余数的除数变化会让大多数键的 `h mod N` 与 `h mod (N+1)` 不同。可先固定大量逻辑分片 `p=h mod P`，再维护 `p→physical node` 映射；扩容只迁移选中的逻辑分片。代价是映射元数据、逻辑片粒度与热点仍需管理。
...
方案一把当前时间桶再按 hash(source/key) 分成 K 个子片，写分散，但一天查询要并行读 K 片并合并。方案二缩小/滚动多个时间桶并提前创建、轮转写入，减轻单片容量峰值，但跨窗口查询访问更多片。也可预聚合；必须用峰值而非日均验证。
...
四片各存 25GB，数据量完全均匀；若 celebrity 用户的一条 1KB 记录承受 80% 请求，它所在片先耗尽 CPU/网络，其余片空闲。分片容量账必须同时列 bytes、QPS、读写比和最热 key，不能只看磁盘。
...
需要等全部或足够多分片时，总延迟由最大值决定；30 次中至少一次落入单片慢尾的概率显著上升。还要加 fan-out 调度、连接池与聚合成本。应测端到端分布，并用 deadline、部分结果语义或减少 fan-out 控制，而非相加平均值。
...
读流程：查缓存；命中返回；未命中读数据库；成功后带 TTL/版本回填再返回。数据库不可用时，允许陈旧的商品展示可返回仍在 stale window 的旧值；余额/权限等强正确数据应报错或失败关闭；只有业务明确定义“缺失等于默认”时才返回默认值。选择要写进接口契约并监控。
...
读者先缓存未命中并读到 DB 旧版 v1；写者提交 v2 后删除缓存；迟到读者随后把 v1 回填，旧值复活。缓存写入携带版本并用条件写“只接受更高版本”，或在删除后让回填再次核对版本，可拒绝 v1 覆盖 v2/墓碑。版本必须来自权威单调序列。
...
穿透是反复查不存在键：用负缓存或 Bloom filter。击穿是单个热门键过期，大量请求同时回源：用 singleflight/互斥重建并可提供旧值。雪崩是大量键同时过期或缓存整体不可用：TTL 加抖动、分批预热与限流降级。措施要对应原因，单纯扩缓存容量不能解决三者。
...
假阳性只让不存在键多访问一次 DB；假阴性会把真实存在键挡掉，返回错误不存在。普通 Bloom filter 不安全删除单键，常周期重建或用 counting Bloom；数据库删除后旧假阳性可接受。负缓存要按版本失效/短 TTL，重新创建同键时必须清除，否则会遮蔽新数据。
...
仅存在缓存内的已确认写会永久丢失。关键数据至少需要确认前写入持久 WAL，复制到足够故障域并通过一致的 leader/世代控制；恢复要重放、去重和对账，刷盘积压也需背压。否则它只能承诺可丢的异步缓存语义。
...
旧节点可能暂停后恢复、重定向丢失，甚至仍接受写，造成双主与遗漏迁移。每次归属变更递增 epoch，请求携带 epoch，最终存储只接受当前或按迁移协议允许的世代；旧 token 写被拒绝并迫使刷新路由。epoch 检查必须在副作用点，而不只是客户端。
