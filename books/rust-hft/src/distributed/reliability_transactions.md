...
### 12.1 幂等与消息题画“效果—去重记录—确认”

把一次操作拆成三个事件：业务效果持久化、幂等/inbox 记录持久化、调用方观察到确认。分别在每两个事件之间插入崩溃。前两个事实若不在同一事务，就会出现“已记为处理但效果缺失”或“效果已发生但重试再次执行”。第三步不是持久提交的一部分；若确认丢失，传输可以重复，但消费者必须从持久记录返回第一次结果。

验算幂等键时检查：同一业务意图的所有重试是否复用同一键；同一键换参数是否被拒绝；记录过期后承诺是否明确；`PROCESSING` 被接管后，旧执行者是否会被版本号或 fencing 阻止。

### 12.2 重试题用乘法树和 deadline 账本

每层最多尝试次数等于“首次尝试 + 重试次数”。三层各最多尝试 3 次时，最坏调用数是 `3×3×3=27`，不是 6 或 9。再按时间线维护剩余预算：

```text
remaining = 总 deadline - 已处理时间 - 已退避时间 - 必要收尾时间
```

任何下一跳 timeout 都不能超过 remaining；若已经小于完成一次调用所需的安全预算，就应停止重试。最后检查抖动是否只打散时刻，以及重试是否针对可恢复错误并受幂等保护。

### 12.3 跨服务事务题从每个提交点切开

- 2PC：列出每个参与者的 INIT、PREPARED、COMMIT/ABORT 和协调者持久决定。在所有 YES 后让协调者失联，检查参与者为何不能自行选择结果。
- Saga：为每个本地事务写对应补偿，并指出补偿自身的失败、重试和不可撤销外部事实；不能把补偿画成时间倒流。
- Outbox：在“业务与 outbox 同事务提交、relay 发布、标记已发送、消费者本地提交”之间逐点崩溃。发布后未标记会重复，因此消费者 inbox 仍是验算不变量。

最终状态检查不应只看消息是否发过，而要比较订单、库存、支付等业务事实；这正是对账要验证的内容。
...
调用者只看到 timeout/UNKNOWN，无法由此断言扣款失败。新键会把同一意图伪装成第二次扣款；应复用原 operation_id 查询结果或重试，让服务端返回已保存结果。
...
请求带 transfer_id、源/目标、金额和参数摘要；事务先条件插入 transfer_id，首次出现才更新两边余额并保存结果，重复则返回原结果。同事务避免“记了去重却没转账”或“转账成功却没记去重”两个崩溃窗口。
...
`500-220-80-50=150ms`。这是假设没有其他调度开销的理论上限；实现还应留安全余量。若下游无法在剩余预算内有意义完成，就不应发第二次。
...
最坏为 `3×3×3=27` 次。通常把重试集中在最了解业务幂等性和总 deadline 的最高合理层，底层只做连接前等明确安全的小重试，并配 retry budget；这样避免乘法放大和各层重新计时。
...
参与者已持久化 PREPARED/YES 并锁住资源，协调者尚未把 COMMIT/ABORT 决议送达。它们不能自行提交，因为协调者可能决定 abort；也不能回滚，因为全体 YES 后可能已有 durable COMMIT 决议或其他参与者已提交。需等待协调者恢复/查询决议，因此经典 2PC 可能阻塞。
...
步骤为 reserve inventory→charge payment→create shipment。失败时逆序补偿：取消发货请求、退款、释放库存；每步与补偿都有稳定 ID 并可重试。包裹已交承运商或通知已被人看见无法真正“撤销”，只能拦截/退货/发更正并进入人工对账。
...
DB 提交后、发送前崩溃会永久漏消息；先发后提交则可能发布不存在的业务事实。Outbox 将业务行与发送意图原子提交，消除漏记意图。relay 在发送后、标记前崩溃仍会重复，消费者需 inbox/event_id 去重并与本地效果同事务。
...
它只能从更早 checkpoint/日志位置重新读，导致一段事件重复；若从过晚位置猜测则可能漏事件。处理器用源日志位置或事件 ID 去重，并让位置推进与业务效果一致提交，才能安全重放。
...
RTO 要求故障后 15 分钟内恢复服务；RPO 允许最多丢 5 分钟数据。仅每小时备份，最坏数据点约旧 60 分钟，不满足 RPO；恢复大备份也未证明能在 15 分钟完成。需更连续日志/复制，并做计时恢复演练。
...
变慢使在途与队列增长，更多请求超时并重试，额外流量让下游更慢。背压在准入/有界队列处拒绝或降级新工作，保护在途；熔断在依赖调用处短路已高概率失败的请求，给下游恢复窗口。二者都要配恢复探测和重试预算。
...
为两类任务分别设置线程/async permit、队列、连接池、内存和下游配额，支付保留最低容量；图片队列满只拒绝图片。若共享同一无界执行器或数据库池，名义分队列仍会在共享点互相拖死，需逐资源向量验算。
...
选 relay 发布 `ReserveInventory(event_id=E)` 后、标记 sent 前崩溃。恢复依据是订单 DB 中已提交的 outbox E；relay 会再次发布 E。库存服务在本地事务插入 inbox E 并预留库存，若 E 已存在就返回原结果，因此消息可重复而预留只发生一次。
...
消息存在不证明消费者成功提交效果，也不证明补偿、重复或人工操作后的最终事实。对账应按 order/payment/reservation ID 比较订单状态、实际扣款和实际预留，列出缺失、重复与金额不符，再生成修复动作并审计。
...
接管时原子把 owner generation 从 8 提到 9。所有状态更新和下游副作用携带 generation，并条件要求当前仍为 9；旧执行者 token=8 的提交被最终存储拒绝。lease 只判断何时接管，fencing 才阻止迟到旧主人。
