...
<div class="book-hero">
  ...
  <p class="book-hero__lead">从市场规则和交易链路出发，学会用 Rust 与 C++ 构建、验证并解释低延迟系统。</p>
  <div class="book-hero__badges">
    <span>市场微观结构</span><span>Rust</span><span>C++</span><span>低延迟系统</span><span>交易引擎</span><span>系统设计</span><span>面试题</span>
  </div>
</div>

欢迎来到《Rust、C++ 与 HFT 基础宝典》。这是一本独立、聚焦的学习书：你会先弄懂电子市场怎样工作，再理解行情、订单、撮合、风控和性能工程怎样组成一套真实交易系统；Rust、C++ 与计算机基础章节则负责解释“代码为什么这样写、成本在哪里、安全边界是什么”。
...
| Rust、C++ 与系统工程 | 从所有权、RAII、并发和内存模型，到队列、网络 I/O、测试、剖析与部署 | 能解释实现选择、安全边界、抽象成本以及验证方法 |

两条主线不会各讲各的。HFT 提供真实问题和正确性约束，Rust 与 C++ 提供两套实现和思考工具，测试、回放与性能分析负责把它们连成证据链。例如，理解 SPSC 队列之前先明确谁生产、谁消费；讨论“更快”之前先证明订单状态和风险检查没有被破坏。
...
先读 C++ 部分导论。完全没学过时按“最小语法 → 基础复习练习”起步；以前学过但忘了时直接做“基础复习”的 30～90 分钟诊断。随后按“编译与内存 → 指针与引用 → RAII → 拷贝和移动 → STL → 原子与无锁 → 性能工具”的顺序学习。最后完成 C++ HFT 贯穿项目，并用 Sanitizer、测试和基准检查它。
